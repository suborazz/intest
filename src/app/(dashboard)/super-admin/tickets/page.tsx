...
import { AlertTriangle, Award, Calendar, Clock as Clock_2, CreditCard, Download, ExternalLink, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageCircle as MessageCircle_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Trash2, User, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
import Link from "next/link";
import * as React from "react";
import * as React_4 from "react";
import React_3 from "react";
import { useEffect, useMemo, useState } from "react";
import { clsx, ClassValue, clsx as clsx_2, ClassValue as ClassValue_2 } from "clsx";
import { twMerge, twMerge as twMerge_2 } from "tailwind-merge";
...
import { z } from "zod";
import { Slot, Checkbox as CheckboxPrimitive, Separator as SeparatorPrimitive, Label as LabelPrimitive, Tooltip as TooltipPrimitive } from "radix-ui";
...
interface IStatusChipProps_6 {
      status?: string;
    }

function cn_20(...inputs: ClassValue_2[]) {
      return twMerge_2(clsx_2(inputs));
    }

const TICKET_STATUS_STYLES: Record<string, string> = {
      OPEN: "bg-red-500/10 text-red-600 dark:bg-red-500/20 dark:text-red-400 border-red-500/20",
      PENDING:
...
const StatusChip_21: React_4.FunctionComponent<IStatusChipProps_6> = ({ status }) => {
      ...
      return (
        <Badge
          ...
          className={cn_20(
            ...
            TICKET_STATUS_STYLES[value] ?? TICKET_STATUS_STYLES.OPEN,
          )}
        >
          {value}
        </Badge>
...
const StatusChip_22 = StatusChip_21;
...
function formatHumanReadableDate_28(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
function cn_11(...inputs: ClassValue[]) {
...
export default function SuperAdminTicketsPage() {
    ...
    interface Ticket extends TicketObject {
      subject?: string;
      name?: string;
      email?: string;
      message?: string;
    ...
    const StatusSelect_2: React.FunctionComponent<IStatusSelectProps> = ({
      ticketId,
      status,
    }) => {
      const { mutate: updateTicket, isPending } =
        SuperAdminDataHooks.useUpdateTicket();
      ...
      return (
        <div onClick={(e) => e.stopPropagation()}>
          <Select_20
            ...
            disabled={isPending}
            onValueChange={(value) =>
              updateTicket({ id: ticketId, payload: { status: value } })
            }
          >
            <SelectTrigger className="h-7 w-[120px] cursor-pointer text-[10px] font-semibold">
              <SelectValue_20 />
            </SelectTrigger>
            <SelectContent>
    ...
    const StatusSelect_3 = StatusSelect_2;

    interface ITicketActionsProps {
      ticket: Ticket;
    }

    interface IReplyDialogProps {
      ticket: Ticket | null;
      isOpen: boolean;
      onClose: () => void;
    }

    const ReplyDialog: React_3.FunctionComponent<IReplyDialogProps> = ({
      ticket,
      isOpen,
      onClose,
    }) => {
      const [replyMsg, setReplyMsg] = React_3.useState("");
      const { mutate: updateTicket, isPending } =
        SuperAdminDataHooks.useUpdateTicket();

      if (!ticket) return null;

      const handleReply = (e: React_3.FormEvent) => {
        e.preventDefault();
        if (!replyMsg.trim()) return;
        updateTicket(
          { id: ticket.id, payload: { message: replyMsg } },
          {
            onSuccess: () => {
              setReplyMsg("");
              onClose();
      ...
      return (
        <ResponsiveDialog
          ...
          title={`Ticket: ${ticket.subject || ticket.title || "Support Request"}`}
        >
          <div className="space-y-4 pb-2 pt-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="border-border/50 bg-muted/10 space-y-1 rounded-xl border p-3">
                <span className="text-muted-foreground flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider">
                  <User className="size-3" /> From
                </span>
                <span className="text-foreground block text-xs font-semibold">
                  {ticket.name || ticket.user?.name || "Unknown"}
                </span>
              ...
              <div className="border-border/50 bg-muted/10 space-y-1 rounded-xl border p-3">
                <span className="text-muted-foreground flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider">
                  <Clock_2 className="size-3" /> Status
                </span>
                <Badge
                  variant="outline"
                  className={cn_11(
                    "rounded-full text-[9px] font-bold uppercase tracking-wider",
                    TICKET_STATUS_STYLES[status] ?? TICKET_STATUS_STYLES.OPEN,
                  )}
                >
                  {status}
                </Badge>
            ...
            {ticket.message && (
              <div className="border-border/50 bg-muted/10 rounded-xl border p-3.5">
                <span className="text-muted-foreground mb-1.5 block text-[9px] font-bold uppercase tracking-wider">
                  Issue Description
                </span>
            ...
            <form
              ...
              className="border-border/50 space-y-3 border-t pt-4"
            >
              <Label className="text-foreground flex items-center gap-1.5 text-xs font-bold">
                <MessageCircle_2 className="text-primary size-3.5" /> Reply to Ticket
              </Label>
              <Textarea
                value={replyMsg}
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  <MessageCircle_2 className="size-3.5" />
    ...
    const TicketActions: React.FunctionComponent<ITicketActionsProps> = ({
      ticket,
    }) => {
      ...
      return (
        <div onClick={(e) => e.stopPropagation()}>
          <Button
            ...
            className="text-primary hover:bg-primary hover:text-primary-foreground h-8 cursor-pointer gap-1.5 rounded-lg px-3 text-xs font-bold"
          >
            <MessageCircle_2 className="size-3.5" /> Reply
          </Button>
    ...
    const columns_14 = (): ColumnDef_2<Ticket>[] => [
      {
        accessorKey: "subject",
        header: "Subject",
        cell: ({ row }) => (
          <div>
            <div className="text-foreground line-clamp-1 text-sm font-semibold">
              {row.original.subject || row.original.title || "Support Request"}
            </div>
        ...
        cell: ({ row }) => (
          <div>
            <div className="text-foreground text-xs font-semibold">
              {row.original.name || row.original.user?.name || "User"}
            </div>
      ...
      },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => <StatusChip_22 status={row.original.status} />,
      },
      {
        id: "update-status",
        header: "Update Status",
        cell: ({ row }) => (
          <StatusSelect_3 ticketId={row.original.id} status={row.original.status} />
        ),
        enableSorting: false,
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-muted-foreground text-xs font-medium">
            {formatHumanReadableDate_28(row.original.createdAt)}
          </div>
        ),
      },
      {
    ...
    const TicketsListingTable = () => {
      ...
      const [meta, setMeta] = useState<TMeta_24>({ total: 0, limit, page });

      const { data: ticketsResponse, isLoading } = SuperAdminDataHooks.useTickets({
        page,
        limit,
        search,
        status,
      ...
      const ticketsList = (ticketsResponse?.data || []) as Ticket[];

      useEffect(() => {
        const total = (ticketsResponse as { meta?: { total?: number } })?.meta
          ?.total;
        setMeta((prev) => ({
          ...prev,
          page,
          limit,
          total: typeof total === "number" ? total : ticketsList.length,
        }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [ticketsResponse, page, limit]);

      const sidebarContent = useMemo(() => <TicketsSidebarFilters />, []);
          ...
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-4">
              <Heading
                ...
                description="Manage platform support queries from users — update statuses, send replies, and close resolved issues."
              />
            </div>
          </PageHeaderLayout>
