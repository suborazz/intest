...
import { AlertTriangle, Award, Calendar, CreditCard, Download, Edit, ExternalLink, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, LucideIcon, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Trash, Trash2, User, Users as Users_2, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
import { ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, FilterIcon, FilterXIcon, TableIcon } from "lucide-react";
...
type PartnerType =
  "job-placement" | "educational-institutes" | "training-support";
...
interface EducationalPartner extends BasePartner {
...
interface TrainingSupportPartner extends BasePartner {
...
type CombinedPartner = JobPlacementPartner &
      EducationalPartner &
      TrainingSupportPartner;

type PartnerType_2 =
      "job-placement" | "educational-institutes" | "training-support";

interface PartnerActionsProps {
      partner: JobPlacementPartner | EducationalPartner | TrainingSupportPartner;
      type: PartnerType_2;
    }

function DropdownMenu_11({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_11({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
const PARTNER_TABS: TSectionItems_5[] = [
      {
        id: "job-placement",
        label: "Job Placement",
        href: "job-placement",
        link: "/super-admin/partners?type=job-placement",
      },
      {
        id: "educational-institutes",
        label: "Educational Institutes",
        href: "educational-institutes",
        link: "/super-admin/partners?type=educational-institutes",
      },
      {
        id: "training-support",
        label: "Training Support",
        href: "training-support",
        link: "/super-admin/partners?type=training-support",
      },
    ];

const PartnersSidebarFilters = () => {
      return (
        <SidebarFiltersHeader className="mr-0 px-3">
          <div className="text-muted-foreground py-4 text-center text-xs font-semibold">
            No active filters. Use search to find partners.
          </div>
...
export default function SuperAdminPartnersPage() {
    ...
    const PARTNERS_QUERY_KEYS = {
      ALL: ["partners"] as const,
      LIST: (type: PartnerType, params?: Record<string, unknown>) =>
        ["partners", type, params] as const,
      INTERESTS: (params?: Record<string, unknown>) =>
        ["partners", "interests", params] as const,
    };
    ...
    const PartnersService: IPartnersService = {
      async listPartners(type, params) {
        const response = await axiosInstance.get<
          GenericApiResponse<Record<string, unknown>[]>
        >(`/partners/${type}`, { params });
        return response.data;
      },

      async createPartner(type, formData) {
        const response = await axiosInstance.post<
          GenericApiResponse<Record<string, unknown>>
        >(`/partners/${type}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        return response.data;
      },

      async updatePartner(type, id, formData) {
        const response = await axiosInstance.patch<
          GenericApiResponse<Record<string, unknown>>
        >(`/partners/${type}/${id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        return response.data;
      },

      async deletePartner(type, id) {
      ...
      async getPartnerInterests(params) {
        const response = await axiosInstance.get<
          GenericApiResponse<Record<string, unknown>[]>
        >(`/partners/interests`, { params });
        return response.data;
      },

      async getPartnerById(type, id) {
        const response = await axiosInstance.get<
          GenericApiResponse<Record<string, unknown>>
        >(`/partners/${type}/${id}`);
        return response.data;
    ...
    const PartnersDataHook = {
      usePartners(
        type: PartnerType,
        params?: Record<string, unknown>,
        options?: TQueryOptions<GenericApiResponse<Record<string, unknown>[]>>,
      ) {
        return useQuery({
          queryKey: PARTNERS_QUERY_KEYS.LIST(type, params),
          queryFn: async () => await PartnersService.listPartners(type, params),
          ...options,
        });
      },
      ...
      usePartnerInterests(
        ...
        return useQuery({
          queryKey: PARTNERS_QUERY_KEYS.INTERESTS(params),
          queryFn: async () => await PartnersService.getPartnerInterests(params),
          ...options,
        });
      },
      ...
      usePartner(
        ...
        return useQuery({
          queryKey: ["partners", type, id],
          queryFn: async () => await PartnersService.getPartnerById(type, id),
          enabled: !!id,
          ...options,
        });
      },
      ...
      useCreatePartner(
        options?: TMutationOptions<
          GenericApiResponse<Record<string, unknown>>,
          Error,
          { type: PartnerType; formData: FormData }
        >,
      ) {
        const queryClient = useQueryClient();
        return useMutation({
          mutationFn: async ({ type, formData }) =>
            await PartnersService.createPartner(type, formData),
          ...options,
          onSuccess: (data, variables, context) => {
            queryClient.invalidateQueries({ queryKey: PARTNERS_QUERY_KEYS.ALL });
            queryClient.invalidateQueries({
              queryKey: ["public", "partners", variables.type],
            });
            toast.success(data.message || "Partner added successfully!");
      ...
      useUpdatePartner(
        options?: TMutationOptions<
          GenericApiResponse<Record<string, unknown>>,
          Error,
          { type: PartnerType; id: string; formData: FormData }
        >,
      ) {
        const queryClient = useQueryClient();
        return useMutation({
          mutationFn: async ({ type, id, formData }) =>
            await PartnersService.updatePartner(type, id, formData),
          ...options,
          onSuccess: (data, variables, context) => {
            queryClient.invalidateQueries({ queryKey: PARTNERS_QUERY_KEYS.ALL });
            queryClient.invalidateQueries({
      ...
      useDeletePartner(
        options?: TMutationOptions<
          GenericApiResponse<Record<string, unknown>>,
          Error,
          { type: PartnerType; id: string }
        >,
      ) {
        const queryClient = useQueryClient();
        return useMutation({
          mutationFn: async ({ type, id }) =>
            await PartnersService.deletePartner(type, id),
          ...options,
          onSuccess: (data, variables, context) => {
            queryClient.invalidateQueries({ queryKey: PARTNERS_QUERY_KEYS.ALL });
            queryClient.invalidateQueries({
    ...
    interface PartnerFormDialogProps {
      ...
      onClose: () => void;
      partner?: Partial<
        JobPlacementPartner & EducationalPartner & TrainingSupportPartner
      >;
      type: PartnerType;
    }

    const PartnerFormDialog: React_3.FC<PartnerFormDialogProps> = ({
      isOpen,
      onClose,
      partner,
      type,
      ...
      const { mutate: createPartner, isPending: isCreating } =
        PartnersDataHook.useCreatePartner({
          onSuccess: () => onClose(),
        });

      const { mutate: updatePartner, isPending: isUpdating } =
        PartnersDataHook.useUpdatePartner({
          onSuccess: () => onClose(),
        });
      ...
      const [website, setWebsite] = useState(partner?.website || "");
        ...
        partner?.postOpportunity || "",
      );
      const [jobNature, setJobNature] = useState(partner?.jobNature || "");
      const [fieldOfEmployment, setFieldOfEmployment] = useState(
        partner?.fieldOfEmployment || "",
      );
      const [minQualification, setMinQualification] = useState(
        partner?.minQualification || "",
      );
      const [skillsRequired, setSkillsRequired] = useState(
        partner?.skillsRequired ? partner.skillsRequired.join(", ") : "",
      );
      const [staffStrength, setStaffStrength] = useState(
        partner?.staffStrength || "",
      );

        const [naacAccreditation, setNaacAccreditation] = useState(
        partner?.naacAccreditation || "",
      );

        const [serviceProvide, setServiceProvide] = useState(
        partner?.serviceProvide || "",
      );
      ...
      const handleSubmit = (e: React_3.FormEvent) => {
        ...
        if (type === "job-placement") {
          formData.append("postOpportunity", postOpportunity);
          formData.append("jobNature", jobNature);
          formData.append("fieldOfEmployment", fieldOfEmployment);
          ...
          const parsedSkills = skillsRequired
            ...
            .map((s: string) => s.trim())
            .filter(Boolean);
          formData.append("skillsRequired", JSON.stringify(parsedSkills));
        } else if (type === "educational-institutes") {
          formData.append("naacAccreditation", naacAccreditation);
        } else if (type === "training-support") {
          formData.append("serviceProvide", serviceProvide);
        }

        if (isEdit && partner?.id) {
          updatePartner({ type, id: partner.id, formData });
        } else {
          createPartner({ type, formData });
        }
      };
      ...
      return (
        <ResponsiveDialog
          ...
          title={isEdit ? "Edit Partner Profile" : "Register Partner Profile"}
          description="Add or update onboarding partner catalog details."
        >
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            ...
            {type === "job-placement" && (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <Label htmlFor="postOpportunity">Post Opportunity</Label>
                    <Input
                      id="postOpportunity"
                      value={postOpportunity}
                      onChange={(e) => setPostOpportunity(e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <Label htmlFor="fieldOfEmployment">Field of Employment</Label>
                    <Input
                      id="fieldOfEmployment"
                      value={fieldOfEmployment}
                      onChange={(e) => setFieldOfEmployment(e.target.value)}
            ...
            {type === "educational-institutes" && (
              <div className="space-y-1">
                <Label htmlFor="naac">NAAC Accreditation Grade</Label>
                <Input
                  id="naac"
            ...
            {type === "training-support" && (
              <div className="space-y-1">
                <Label htmlFor="service">Services Provided</Label>
                <Input
                  id="service"
                  value={serviceProvide}
                  onChange={(e) => setServiceProvide(e.target.value)}
                  placeholder="e.g. Full Stack Web Development Training"
                />
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label htmlFor="logo">Partner Logo Image</Label>
                <Input
                  id="logo"
                  type="file"
                  accept="image/*"
              ...
              <div className="space-y-1">
                <Label htmlFor="photo">Highlight Display Photo</Label>
                <Input
                  id="photo"
                  type="file"
                  accept="image/*"
    ...
    const PartnerActions: React.FC<PartnerActionsProps> = ({
      ...
      const { mutate: deletePartner, isPending } =
        PartnersDataHook.useDeletePartner();

      const handleDelete = () => {
        if (
          confirm(`Are you sure you want to delete the partner "${partner.name}"?`)
        ) {
          deletePartner({ type, id: partner.id });
        }
      };
      ...
      return (
        ...
        >
          <DropdownMenu_11>
            <DropdownMenuTrigger_11 asChild>
              <Button
                variant="ghost"
                size="icon"
            ...
            <DropdownMenuContent align="end" className="w-44">
              ...
              {type === "job-placement" && (
                <DropdownMenuItem asChild className="cursor-pointer">
                  <Link href={`/super-admin/partners/${partner.id}/interests`}>
                    <Users_2 className="mr-2 size-3.5 text-emerald-500" />
    ...
    const getColumns_5 = (type: PartnerType_3): ColumnDef<CombinedPartner>[] => [
      {
        accessorKey: "name",
        header: "Partner Name",
        cell: ({ row }) => (
          <div className="flex items-center gap-3">
            {row.original.logo ? (
              <div className="bg-muted relative size-8 shrink-0 overflow-hidden rounded-md border">
                <Image
              ...
              <div className="bg-primary/10 text-primary flex size-8 shrink-0 items-center justify-center rounded-md text-xs font-bold uppercase">
      ...
      ...(type === "job-placement"
        ? ([
            {
              accessorKey: "fieldOfEmployment",
              header: "Domain Field",
              cell: ({ row }) => (
                <span className="text-foreground/80 text-xs font-semibold">
                  {row.getValue("fieldOfEmployment") || "N/A"}
                </span>
              ),
            },
            {
              accessorKey: "jobNature",
              header: "Job Nature",
              cell: ({ row }) => (
                <span className="text-foreground/80 text-xs font-semibold">
                  {row.getValue("jobNature") || "N/A"}
                </span>
              ),
            },
          ] as ColumnDef<CombinedPartner>[])
        : []),
      ...(type === "educational-institutes"
        ? ([
            {
              accessorKey: "naacAccreditation",
              header: "NAAC Grade",
              cell: ({ row }) => (
                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                  {row.getValue("naacAccreditation") || "N/A"}
                </span>
              ),
            },
          ] as ColumnDef<CombinedPartner>[])
      ...
      ...(type === "training-support"
        ? ([
            {
              accessorKey: "serviceProvide",
              header: "Services Provided",
              cell: ({ row }) => (
                <span className="text-foreground/80 max-w-[200px] truncate text-xs font-semibold">
                  {row.getValue("serviceProvide") || "N/A"}
                </span>
              ),
            },
          ] as ColumnDef<CombinedPartner>[])
        : []),
      {
        id: "actions",
        header: () => <div className="text-right">Actions</div>,
        cell: ({ row }) => <PartnerActions partner={row.original} type={type} />,
      },
    ];
    const PartnersListingTable = () => {
      const searchParams = useSearchParams();
      ...
      const activeType =
        (searchParams?.get("type") as PartnerType_4) || "job-placement";

      const [meta, setMeta] = useState<TMeta_16>({ total: 0, limit, page });
      const [formOpen, setFormOpen] = useState(false);

        const { data: response, isLoading } =
        PartnersDataHook.usePartners(activeType);
      const allPartners = useMemo<
        (JobPlacementPartner & EducationalPartner & TrainingSupportPartner)[]
      >(
        () =>
          (response?.data as unknown as (JobPlacementPartner &
            EducationalPartner &
            TrainingSupportPartner)[]) || [],
        [response],
      );
      ...
      const columns = useMemo(() => getColumns_5(activeType), [activeType]);

      return (
        <>
          <PageHeaderLayout>
            <Heading
              title="Partners Onboarding Board"
              description="Manage onboarding partner catalogs across Placement, Educational, and Training support domains."
            />
            <div className="flex w-full flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
              <Sections sections={PARTNER_TABS} activeHref={activeType} />
              <Button
                size="sm"
                onClick={() => setFormOpen(true)}
                className="h-8 w-full shrink-0 cursor-pointer justify-center rounded-lg text-xs font-semibold sm:w-auto sm:-translate-y-1 sm:translate-x-3"
              >
