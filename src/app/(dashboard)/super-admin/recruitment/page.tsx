...
import { AlertTriangle, Award, Calendar, CreditCard, Download, Edit, ExternalLink, Eye, FileText, FolderOpen, Globe as Globe_2, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Trash2, User, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
interface JobOpportunityPublic_3 {
      id: string;
      companyName: string;
      address: string;
      postOpportunity: string;
...
interface JobOpportunityPublic_2 {
      id: string;
      companyName: string;
      address: string;
      postOpportunity: string;
...
interface ActionsCellProps {
      job: JobOpportunityPublic_2;
    }

function DropdownMenu_13({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_13({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
      ...
      return (
        ...
        >
          <div className="space-y-4">
            <div className="bg-destructive/10 text-destructive flex items-start gap-3 rounded-lg p-3 text-xs font-medium">
              <AlertTriangle className="size-4 shrink-0" />
              <div>
                Are you sure you want to delete{" "}
                <span className="font-bold">"{jobTitle}"</span>? This will
...
export default function SuperAdminRecruitmentPage() {
    ...
    const ActionsCell: React_3.FC<ActionsCellProps> = ({ job }) => {
      const [deleteOpen, setDeleteOpen] = useState(false);

      const { mutate: deleteJob, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteJobOpportunity();
      ...
      return (
        ...
        >
          <DropdownMenu_13>
            <DropdownMenuTrigger_13 asChild>
              <Button
                variant="ghost"
                size="icon"
            ...
            </DropdownMenuTrigger_13>
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem asChild className="cursor-pointer font-semibold">
                <Link href={`/super-admin/recruitment/${job.id}/applications`}>
                  <Eye className="mr-2 size-3.5 text-blue-500" />
              ...
              <DropdownMenuItem asChild className="cursor-pointer font-semibold">
                <Link href={`/super-admin/recruitment/edit/${job.id}`}>
    ...
    const columns_8 = (): ColumnDef<JobOpportunityPublic_2>[] => [
      {
        ...
        header: "Advt. No. & Date",
        cell: ({ row }) => {
          const advtNo = row.original.advtNo || "N/A";
          const advtDate = row.original.advtDate || "N/A";
          return (
            <div className="flex flex-col gap-0.5 text-xs">
              <span className="text-foreground font-semibold">{advtNo}</span>
              <span className="text-muted-foreground text-[10px]">{advtDate}</span>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex max-w-[220px] flex-col gap-0.5">
            <span className="text-foreground truncate text-xs font-bold leading-snug">
              {row.original.postOpportunity}
            </span>
            <span className="text-muted-foreground text-[10px] font-medium leading-none">
              Nature: {row.original.jobNature}
            </span>
          </div>
        ),
      },
      ...
      },
      {
        ...
        header: "Attachments",
        cell: ({ row }) => {
          const { logoUrl, jdDocUrl } = row.original;
          return (
            <div className="flex items-center gap-2">
              {logoUrl && (
                <a
                  ...
                  title="Company Logo"
                >
                  <Globe_2 className="size-4 text-emerald-600 transition-colors hover:text-emerald-500" />
                </a>
              )}
              {jdDocUrl && (
                <a
                ...
                >
                  <FileText className="size-4 text-indigo-600 transition-colors hover:text-indigo-500" />
                </a>
              )}
              {!logoUrl && !jdDocUrl && (
    ...
    const RecruitmentListingTable = () => {
      ...
      const [meta, setMeta] = useState<TMeta_18>({ total: 0, limit, page });

      const { data: response, isLoading } = SuperAdminDataHooks.useJobOpportunities(
        {
          limit: 100, 
        },
      );

      const allJobs = useMemo<JobOpportunityPublic_3[]>(
        () => (response?.data as JobOpportunityPublic_3[]) || [],
        [response],
      );

      const filteredList = useMemo(() => {
        return allJobs.filter((item) => {
          const name = (item.postOpportunity || "").toLowerCase();
          const company = (item.companyName || "").toLowerCase();
          const location = (item.address || "").toLowerCase();
          const advt = (item.advtNo || "").toLowerCase();
        ...
        <>
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
              <Heading
                title="Recruitment Openings Governance"
                description="Manage and broadcast your public recruitment notices and job opportunities."
              />
              <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto">
                <Link
                  href="/super-admin/recruitment/new"
                  className="w-full sm:w-auto"
                >
                  <Button
                    size="sm"
                    className="h-8 w-full cursor-pointer justify-center rounded-lg text-xs font-semibold sm:w-auto"
                  >
