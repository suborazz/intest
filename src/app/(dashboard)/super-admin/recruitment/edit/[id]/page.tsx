...
import { Bold, Code, Edit, FileText, Globe as Globe_2, Italic, Link as LinkIcon, List, ListOrdered, Minus, Quote, Redo2, RemoveFormatting, Save, Undo2, Upload, X } from "lucide-react";
...
interface JobOpportunityPublic_4 {
  id: string;
  companyName: string;
  address: string;
  postOpportunity: string;
...
interface IEditFormProps {
  data: JobOpportunityPublic_4;
}

const createJobOpportunitySchema_2 = z_2.object({
  companyName: z_2.string().min(2, "Company name is required"),
  address: z_2.string().min(2, "Address is required"),
  postOpportunity: z_2.string().min(2, "Post opportunity is required"),
  jobNature: z_2.string().min(2, "Job nature is required"),
  fieldOfEmployment: z_2.string().min(2, "Field of employment is required"),
  minQualification: z_2.string().min(2, "Minimum qualification is required"),
  skillsRequired: z_2.string().optional(),
  totalStaffStrength: z_2.string().optional(),
  website: z_2.string().optional().or(z_2.literal("")),
  logoUrl: z_2.string().optional(),
  advtNo: z_2.string().optional(),
  advtDate: z_2.string().optional(),
  closingDate: z_2.string().optional(),
        jdDescription: z_2.string().nullish().or(z_2.literal("")),
});

type TEditJob = z_2.infer<typeof createJobOpportunitySchema_2>;
...
const EditForm_7: React_3.FC<IEditFormProps> = ({ data }) => {
  const [showDialog, setShowDialog] = useState(true);
  const [logoFile, setLogoFile] = useState<File | null>(null);
  const [jdFile, setJdFile] = useState<File | null>(null);
  const router = useRouter();

  const form = useForm<TEditJob>({
    resolver: zodResolver(createJobOpportunitySchema_2),
    defaultValues: {
      ...
      jdDescription: data.jdDescription || "",
    },
  });

  const { mutate: updateJob, isPending } =
    SuperAdminDataHooks.useUpdateJobOpportunity({
      onSuccess: () => {
        router.back();
        router.refresh();
        setShowDialog(false);
  ...
  const onSubmit = (values: TEditJob) => {
  ...
  return (
    <ResponsiveDialog
      ...
      description="Modify the details of this job opportunity."
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <FormItem>
            ...
            <FormControl>
              <Input
                value={data.id}
                disabled
                className="bg-muted text-muted-foreground h-9 text-xs"
              />
            </FormControl>
          </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g. B.Tech in CSE / Equivalent"
                      className="h-9 text-xs"
                      {...field}
                    />
                  </FormControl>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {data.logoUrl && !logoFile && (
                  <div className="text-muted-foreground bg-muted/30 flex items-center gap-2 rounded p-1.5 text-[10px]">
                    <Globe_2 className="size-3" />
                    <a
                      href={data.logoUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="hover:text-primary max-w-[150px] truncate underline"
                    >
                      View Current Logo
                    </a>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <label className="bg-background text-foreground hover:bg-muted/10 inline-flex cursor-pointer items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-bold shadow-sm">
                  ...
                  {logoFile && (
                    <div className="flex items-center gap-1 rounded bg-emerald-50 px-2 py-1 text-[11px] font-medium text-emerald-600">
                      <span>{logoFile.name.substring(0, 12)}...</span>
                      <button type="button" onClick={() => setLogoFile(null)}>
                        <X className="size-3 text-emerald-600" />
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {jdFile && (
                    <div className="flex items-center gap-1 rounded bg-indigo-50 px-2 py-1 text-[11px] font-medium text-indigo-600">
                      <span>{jdFile.name.substring(0, 12)}...</span>
                      <button type="button" onClick={() => setJdFile(null)}>
                        <X className="size-3 text-indigo-600" />
                      </button>
                    </div>
                  )}
                </div>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <p className="text-muted-foreground -mt-1 text-[11px]">
                  Write the JD here as an alternative to — or alongside — the
                  PDF above.
                </p>
                <FormControl>
                  <RichTextEditor_2
          ...
          <Button
            type="submit"
            disabled={isPending}
            className="mt-4 h-10 w-full text-xs font-semibold"
          >
...
const EditForm_8 = EditForm_7;

interface IEditPageProps {
  params: Promise<{ id: string }>;
}

const EditPage: React_2.FC<IEditPageProps> = ({ params }) => {
  const { id } = use(params);
  const { data, isLoading } = SuperAdminDataHooks.useJobOpportunityDetail(id);
  ...
  return <EditForm_8 data={data.data} />;
};
