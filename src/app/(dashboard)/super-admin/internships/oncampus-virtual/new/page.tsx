...
import { Bold, Code, Italic, Link as LinkIcon, List, ListOrdered, Minus, Quote, Redo2, RemoveFormatting, Undo2 } from "lucide-react";
...
export default function NewOnCampusVirtualInternshipPage() {
    type TCreateOnCampusVirtualInput = z_2.input<
      typeof ZCreateOnCampusVirtual
    >;
    const ZCreateOnCampusVirtual = z_3.object({
    ...
    function AddForm_14() {
      ...
      const defaultCategory =
        searchParams?.get("type") === "virtual-internship"
          ? "VIRTUAL"
          : "ON_CAMPUS";

      const form = useForm<TCreateOnCampusVirtualInput>({
        resolver: zodResolver(ZCreateOnCampusVirtual),
        defaultValues: {
          title: "",
          duration: "",
          type: "FREE",
      ...
      const { mutate: createInternship, isPending } =
        OnCampusVirtualDataHooks.useCreateInternship({
          onSuccess: () => {
            router.back();
            setShowDialog(false);
            form.reset();
      ...
      function onSubmit(values: TCreateOnCampusVirtualInput) {
            const modulesArray =
      ...
      return (
        <ResponsiveDialog
          ...
          title={
            defaultCategory === "VIRTUAL"
              ? "Admin: Create Virtual Internship"
              : "Admin: Create On-Campus Internship"
          }
          description="Register a new internship on the system. This module captures expressions of interest."
        >
          <Form {...form}>
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-1.5">
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue_18 placeholder="Select Category" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
