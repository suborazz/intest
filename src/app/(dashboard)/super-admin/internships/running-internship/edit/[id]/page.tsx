...
import { Award, Bold, BookOpen, Briefcase as Briefcase_2, Building2, Calendar as Calendar_3, Clock as Clock_2, Code, DollarSign, Edit, FileText, Italic, Layers, Link as LinkIcon, List, ListOrdered, Loader2, MapPin as MapPin_2, Minus, Quote, Redo2, RemoveFormatting, Sparkles as Sparkles_2, Undo2, UserCheck, Users as Users_2 } from "lucide-react";
...
import { LucideIcon, ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, Loader2Icon, XIcon, X, LoaderIcon } from "lucide-react";
...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>(
  ({ placeholder, value = [], onChange, className, id, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          "border-input bg-background/50 focus-within:ring-primary/20 focus-within:border-primary/50 flex min-h-10 w-full cursor-text flex-wrap items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-all duration-200 focus-within:ring-2 [&>div]:w-auto [&>div]:flex-1",
          className,
        )}
      >
        {value.map((tag, index) => (
          <Badge
            key={index}
            variant="secondary"
            className="bg-secondary/80 border-border/40 hover:bg-secondary animate-in fade-in zoom-in-95 flex items-center gap-1 rounded-lg border py-0.5 pl-2 pr-1 font-semibold transition-all duration-150"
          >
            ...
            <Button
              ...
              className="hover:bg-destructive/10 hover:text-destructive text-muted-foreground h-4 w-4 cursor-pointer rounded p-0 transition-colors"
            >
              <X className="h-3 w-3" />
        ...
        <Input
          ...
          className="h-7 min-w-[120px] border-0 bg-transparent px-0 py-0 text-sm shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
          {...props}
        />
      </div>
    );
...
const ZCreateInternship = createInternshipSchema.extend({
      ...
      contact: z_2.string().optional().nullable(),
      organizer: z_2.string().optional().nullable(),
      projectFocus: z_2.string().optional().nullable(),
      timePeriod: z_2.string().optional().nullable(),
      credits: z_2.string().optional().nullable(),
      qualification: z_2.string().optional().nullable(),
      facilities: z_2.string().optional().nullable(),
      careerOpportunity: z_2.string().optional().nullable(),
      onboardingDetails: z_2.string().optional().nullable(),
      ...
      skills: z_2.union([z_2.string(), z_2.array(z_2.string())]).optional(),
    });

type TCreateInternship = z_2.infer<typeof ZCreateInternship>;
...
interface AdminInternshipPublic_3 extends InternshipPublic {
      createdBy?: {
        role?: string;
      } | null;
...
function PopoverTrigger_7({
      ...props
    }: React_3.ComponentProps<typeof PopoverPrimitive.Trigger>) {
      return <PopoverPrimitive.Trigger data-slot="popover-trigger" {...props} />;
    }

function cn_24(...inputs: ClassValue[]) {
      return twMerge(clsx(inputs));
    }

interface InstructorBasicInfo_4 {
      id: string;
      userId?: string;
      fullName?: string;
      name?: string;
...
export default function EditInternshipPage() {
    ...
    function EditForm_11({ data }: { data: AdminInternshipPublic_3 }) {
      ...
      const form = useForm<TCreateInternship>({
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        resolver: zodResolver(ZCreateInternship) as any,
        defaultValues: {
          title: data.title || "",
          companyName: data.companyName || "",
          description: data.description || "",
          location: data.location || "",
          mode: (data.mode as "ONLINE" | "OFFLINE" | "HYBRID") || "OFFLINE",
          type: (data.type as "FREE" | "PAID" | "STIPEND") || "FREE",
          category:
            (data.category as "ON_CAMPUS" | "VIRTUAL" | "RUNNING") || "RUNNING",
          ...
          instructorId: data.instructorId || data.assignedInstructorId || "",
      ...
      const { mutate: updateInternship, isPending } =
        SuperAdminDataHooks.useUpdateInternship({
          onSuccess: () => {
            router.back();
            router.refresh();
            setShowDialog(false);
      ...
      function onSubmit(values: TCreateInternship) {
        ...
        updateInternship({ id: data.id, payload });
      }
      ...
      return (
        <ResponsiveDialog
          ...
          title="Super Admin: Edit Running Internship"
          description="Modify and update details, assigned instructor, and curriculum metadata."
        ...
        >
          <Form {...form}>
            ...
            >
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue_23 placeholder="Select Mode" />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col">
                        <FormLabel className="mb-1 text-xs font-semibold">
                          Program Start Date
                        </FormLabel>
                        <Popover_7>
                          <PopoverTrigger_7 asChild>
                            <FormControl>
                              <Button
                                variant="outline"
                                className={cn_24(
                                  "border-input bg-background h-10 w-full justify-start border pl-3 text-left font-normal",
                                  !field.value && "text-muted-foreground",
                                )}
                              >
                                <Calendar_3 className="mr-2 h-4 w-4 opacity-50" />
                                {field.value ? (
              ...
              <div className="bg-muted/30 border-border/50 space-y-4 rounded-xl border p-4">
                <div className="border-border/30 flex items-center gap-2 border-b pb-2">
                  <Users_2 className="size-4 text-blue-600" />
                  <h3 className="text-foreground text-sm font-bold">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="E.g. Non-Residential or Residential"
                            icon={Clock_2}
                            {...field}
                            value={field.value || ""}
                          />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="sm:col-span-2">
                        ...
                        <FormControl>
                          <Input
                            placeholder="E.g. B.Tech/M.Tech (CS/IT) 3rd/4th Year"
                            icon={BookOpen}
                            {...field}
                            value={field.value || ""}
                          />
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem
                        className={cn_24(
                          selectedType === "FREE" ? "sm:col-span-2" : "",
                        )}
                      >
                        <FormLabel className="text-xs font-semibold">
                          Internship Type *
                        </FormLabel>
                        <Select_23 onValueChange={field.onChange} value={field.value}>
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue_23 placeholder="Select Type" />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                            <SelectItem value="FREE">Free (No fees)</SelectItem>
                            <SelectItem value="STIPEND">
                              Stipend-based (Paid to Student)
                            </SelectItem>
                            <SelectItem value="PAID">
                              Paid (Enrollment Fee Required)
                            </SelectItem>
                          </SelectContent>
                        </Select_23>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="E.g. Access to high-performance computing lab, library subscription, and lunch coupons"
                            rows={2}
                            {...field}
                            value={field.value || ""}
                          />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="E.g. Pre-placement interview (PPI) opportunity with partner tech firms"
                            rows={2}
                            {...field}
                            value={field.value || ""}
                          />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Setup instructions, link to onboarding portal, pre-requisites..."
                            rows={2}
                            {...field}
                            value={field.value || ""}
                          />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <TagInput
                            placeholder="E.g. Microservices Architecture, Docker & Kubernetes, CI/CD Setup"
                            value={Array.isArray(field.value) ? field.value : []}
                            onChange={field.onChange}
                          />
                        </FormControl>
                        <FormDescription className="text-[10px]">
                          Press Enter or Comma after typing each module.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <TagInput
                            placeholder="E.g. Next.js, Go, Docker, Kubernetes, Prometheus"
                            value={Array.isArray(field.value) ? field.value : []}
                            onChange={field.onChange}
                          />
                        </FormControl>
                        <FormDescription className="text-[10px]">
                          Press Enter or Comma after typing each tool.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <TagInput
                            placeholder="E.g. TypeScript, Golang, Docker basics"
                            value={Array.isArray(field.value) ? field.value : []}
                            onChange={field.onChange}
                          />
                        </FormControl>
                        <FormDescription className="text-[10px]">
                          Press Enter or Comma after typing each skill.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
  ...
  const { data, isLoading } = SuperAdminDataHooks.useInternshipDetail(id);
  ...
  return <EditForm_11 data={data.data} />;
}
