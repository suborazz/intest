...
import { Activity as Activity_2, AlertTriangle, Award, Calendar, Check as Check_2, CheckCircle2, Clock as Clock_2, CreditCard, Download, Edit, ExternalLink, Eye, FileText, FolderOpen, GraduationCap as GraduationCap_2, Info, Laptop as Laptop_2, Layers, Loader2, LucideIcon, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Sparkles as Sparkles_2, Trash2, Trash2Icon, User, Users as Users_2, XCircle, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
import { ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, TableIcon, LoaderIcon } from "lucide-react";
...
const TYPE_TO_CATEGORY: Record<string, "RUNNING" | "ON_CAMPUS" | "VIRTUAL"> = {
      "running-internship": "RUNNING",
      "on-campus": "ON_CAMPUS",
...
interface ITypeBadgesProps {
      type?: string;
    }

const TypeBadges_3: React_4.FunctionComponent<ITypeBadgesProps> = ({ type }) => {
      if (type === "PAID") {
        return (
          <Badge
            variant="outline"
            className="border-emerald-500/20 bg-emerald-500/10 text-[9px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400"
          >
            Paid
          </Badge>
        );
      ...
      if (type === "STIPEND") {
        return (
          <Badge
            variant="outline"
            className="border-amber-500/20 bg-amber-500/10 text-[9px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400"
          >
            Stipend
          </Badge>
        );
      ...
      return (
        <Badge
          variant="outline"
          className="bg-muted/50 border-border/40 text-muted-foreground text-[9px] font-bold uppercase tracking-wider"
        >
          Free
        </Badge>
      );
...
const TypeBadges_4 = TypeBadges_3;

interface AdminInternshipPublic extends InternshipPublic {
      createdBy?: {
        role?: string;
      } | null;
      instructorId?: string | null;
...
function DropdownMenu_3({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_3({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
function DeleteInternshipDialog_2({
      ...
      return (
        <AlertDialog_4 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
            <AlertDialogHeader className="flex flex-col items-center text-center">
              ...
              <AlertDialogDescription className="text-center">
                ...
                {internshipTitle ? (
                  <>
                    the internship posting for{" "}
                    <span className="text-foreground font-semibold">
                      "{internshipTitle}"
                    </span>
                  </>
                ) : (
                  "this internship posting"
                )}
                ? This action is permanent and cannot be undone. All candidate
                registrations associated with this internship might be affected.
...
function formatCurrencyINR(amount?: number | null): string {
...
export default function SuperAdminInternshipsModerationPage() {
    ...
    function ApprovedActions({
      ...
      onViewDetails: (item: InternshipPublic) => void;
    }) {
      const [deleteOpen, setDeleteOpen] = React_3.useState(false);

      const isOnCampusOrVirtual =
        internship.category === "ON_CAMPUS" || internship.category === "VIRTUAL";

      const editHref = isOnCampusOrVirtual
        ? `/super-admin/internships/oncampus-virtual/edit/${internship.id}`
        : `/super-admin/internships/running-internship/edit/${internship.id}`;

      const leadsHref = `/super-admin/internships/oncampus-virtual/${internship.id}/leads`;
      const appsHref = `/super-admin/internships/${internship.id}/applications`;

      const { mutate: deleteInternship, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteInternship();
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {isOnCampusOrVirtual ? (
                <DropdownMenuItem asChild className="cursor-pointer font-semibold">
                  <Link href={leadsHref}>
                    <Users_2 className="mr-2 size-3.5 text-emerald-600" />
                    <span>View Leads & Expressions</span>
                  </Link>
                </DropdownMenuItem>
              ) : (
                <DropdownMenuItem asChild className="cursor-pointer font-semibold">
              ...
              <DropdownMenuItem asChild className="cursor-pointer font-semibold">
                <Link href={editHref}>
                  <Edit className="mr-2 size-3.5 text-amber-500" />
    ...
    function PendingActions({
      ...
      const { mutate: approvePosting, isPending } =
        SuperAdminDataHooks.useApproveInternshipPosting();
      ...
      const item = internship as AdminInternshipPublic;
      const createdBy =
        item.createdBy?.role ||
        (item.createdById &&
        item.instructorId &&
      ...
      const superAdminStatus = isInstructorCreated
        ? item.superAdminApprovalStatus ||
          (item.isApproved ? "APPROVED" : "PENDING")
        : item.superAdminApprovalStatus || "APPROVED";

      const isSuperAdminApproved = superAdminStatus === "APPROVED";
      const showApproveAction = !isSuperAdminApproved;
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              {showApproveAction && (
                <>
                  <DropdownMenuItem
                    disabled={isPending}
                    onClick={() => approvePosting(internship.id)}
    ...
    const INTERNSHIP_TABS: TSectionItems[] = [
      {
        id: "running-internship",
        label: "Running Internships",
        href: "running-internship",
        link: "/super-admin/internships?type=running-internship",
        icon: Activity_2,
      },
      {
        id: "pending-internship",
        label: "Pending Internships",
        href: "pending-internship",
        link: "/super-admin/internships?type=pending-internship",
        icon: Clock_2,
      },
      {
        id: "on-campus",
        label: "On Campus Internships",
        href: "on-campus",
        link: "/super-admin/internships?type=on-campus",
        icon: School,
      },
      {
        ...
        link: "/super-admin/internships?type=virtual-internship",
        icon: Laptop_2,
      },
    ];
    const CategoryInternshipsListingTable = () => {
      const searchParams = useSearchParams();

      const activeType = searchParams?.get("type") || "running-internship";
      const category = TYPE_TO_CATEGORY[activeType] || "RUNNING";
      const isPendingTab = activeType === "pending-internship";

      const page = Number(searchParams?.get("page")) || 1;
      const limit = Number(searchParams?.get("size")) || 10;
      const search = searchParams?.get("filter") || undefined;

      const [meta, setMeta] = useState<TMeta_5>({ total: 0, limit, page });
      const [selectedInternship, setSelectedInternship] =
        useState<InternshipPublic | null>(null);
      const [isDetailOpen, setIsDetailOpen] = useState(false);

      const handleViewDetails = (item: InternshipPublic) => {
      ...
      const allPending = useMemo<InternshipPublic[]>(
      ...
      const paginatedPending = useMemo(
        () => filteredPending.slice((page - 1) * limit, page * limit),
        [filteredPending, page, limit],
      );

      const internships = isPendingTab
        ? paginatedPending
        : catalogResponse?.data || [];

      const isLoading = isPendingTab ? isPendingLoading : isCatalogLoading;

      useEffect(() => {
        if (isPendingTab) {
          setMeta({ total: filteredPending.length, limit, page });
        } else if (catalogResponse?.meta) {
          setMeta({
            total: catalogResponse.meta.total,
            limit: catalogResponse.meta.limit,
            page: catalogResponse.meta.page,
          });
        }
      }, [isPendingTab, catalogResponse, filteredPending.length, limit, page]);

      const columns = useMemo<ColumnDef<InternshipPublic>[]>(() => {
        if (activeType === "on-campus" || activeType === "virtual-internship") {
          return [
            {
              id: "internshipId",
            ...
            },
            {
              ...
              cell: ({ row }) => (
                <div
                  className="text-primary max-w-[150px] cursor-pointer truncate text-sm font-semibold hover:underline"
            ...
            },
            {
              accessorKey: "type",
              header: "Type",
              cell: ({ row }) => <TypeBadges_4 type={row.original.type} />,
            },
            {
              accessorKey: "duration",
              header: "Duration",
              cell: ({ row }) => (
                <span className="text-xs">{row.original.duration || "N/A"}</span>
              ),
            },
            {
            ...
            },
            {
              ...
              cell: ({ row }) => (
                <div
                  ...
                  title={
                    row.original.projectFocus ||
                    (row.original as AdminInternshipPublic).project ||
                    "N/A"
                  }
                >
                  {row.original.projectFocus ||
            ...
            },
            {
              ...
              cell: ({ row }) => {
                const plainText =
                  row.original.description?.replace(/<[^>]*>/g, "") || "N/A";
                return (
                  <div
        ...
        return [
          ...
          },
          {
            ...
            cell: ({ row }) => (
              <div className="max-w-[180px]">
                <div
                  className="text-foreground hover:text-primary cursor-pointer truncate font-semibold transition-colors"
                  title={row.original.title}
                  onClick={() => handleViewDetails(row.original)}
                >
                  {row.original.title}
                ...
                <div
                  className="text-muted-foreground mt-0.5 truncate text-[10px] font-medium"
          ...
          },
          {
            ...
            cell: ({ row }) => {
              const item = row.original as AdminInternshipPublic;
              const createdBy =
                item.createdBy?.role ||
                (isPendingTab
                  ? "INSTRUCTOR"
            ...
            cell: ({ row }) => {
              const pricing =
                row.original.type === "PAID"
                  ? formatCurrencyINR(row.original.price || 0)
                  : row.original.stipendAmount
                    ? formatCurrencyINR(row.original.stipendAmount)
                    : "Free";
              const text = `${row.original.duration || "N/A"} • ${pricing}`;
              return (
                <div
                  className="text-foreground max-w-[130px] truncate text-xs font-medium"
                  title={text}
                >
                  {text}
                </div>
          ...
          },
          {
            ...
            cell: ({ row }) => {
              const mode = row.original.mode || "OFFLINE";
      ...
      return (
        <div>
          <PageHeaderLayout>
            <Heading
              title="Internships"
              description="Manage active internship postings across running, on-campus, and virtual categories — monitor applications and approve submissions."
            />
            <Sections sections={INTERNSHIP_TABS} activeHref={activeType} />
          </PageHeaderLayout>
            ...
            <DataTable
              ...
              toolbar={
                <div className="flex w-full items-center justify-between gap-2">
                  ...
                  <Link
                    href={
                      activeType === "on-campus" ||
                      activeType === "virtual-internship"
                        ? `/super-admin/internships/oncampus-virtual/new?type=${activeType}`
                        : "/super-admin/internships/running-internship/new"
                    }
                  >
  ...
  return (
    <main>
      <Suspense fallback={null}>
        <CategoryInternshipsListingTable />
      </Suspense>
    </main>
  );
}
