...
import { AlertTriangle, Award, Calendar, CheckIcon, CreditCard, Download, EllipsisVerticalIcon, ExternalLink, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Trash2, Trash2Icon, User, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, Star as Star_2, XCircle as XCircle_2 } from "lucide-react";
...
import { LucideIcon, ChevronLeftIcon, ChevronRightIcon, ChevronDownIcon, ChevronUpIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, TableIcon, LoaderIcon } from "lucide-react";
...
const RatingStars_2: React_4.FunctionComponent<IRatingStarsProps> = ({
      rating,
    }) => {
      return (
...
const RatingStars_3 = RatingStars_2;

function DropdownMenu_18({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_18({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface DeleteConfirmationDialogProps_3 {
      isOpen: boolean;
      onClose: () => void;
      onConfirm: () => void;
      isDeleting?: boolean;
...
function AlertDialog_6({
      ...props
    }: React_3.ComponentProps<typeof AlertDialogPrimitive.Root>) {
      return <AlertDialogPrimitive.Root data-slot="alert-dialog" {...props} />;
    }

function DeleteConfirmationDialog_7({
      isOpen,
      onClose,
      onConfirm,
      isDeleting = false,
      authorName,
    }: DeleteConfirmationDialogProps_3) {
      return (
        <AlertDialog_6 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent>
            <AlertDialogHeader className="flex flex-col items-center text-center">
              ...
              <AlertDialogDescription className="text-center">
                ...
                ? It will be removed from the moderation queue and cannot be
                recovered.
...
export default function SuperAdminReviewsPage() {
    ...
    const ReviewActions: React.FunctionComponent<IReviewActionsProps> = ({
      review,
    }) => {
      ...
      const { mutate: approveReview, isPending: isApproving } =
        SuperAdminDataHooks.useApproveReview();
      const { mutate: deleteReview, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteReview();

      return (
        <div onClick={(e) => e.stopPropagation()}>
          <DropdownMenu_18 open={open} onOpenChange={setOpen} modal={false}>
            <DropdownMenuTrigger_18 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <EllipsisVerticalIcon className="text-muted-foreground h-4 w-4" />
              </Button>
            </DropdownMenuTrigger_18>
            <DropdownMenuContent align="end" className="w-40">
              ...
              >
                <span>Approve</span>
                <span className="inline-flex items-center justify-center rounded-md bg-emerald-500/10 p-1 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400">
                  <CheckIcon size={12} />
                </span>
              </DropdownMenuItem>
          ...
          <DeleteConfirmationDialog_7
            isOpen={deleteOpen}
            onClose={() => setDeleteOpen(false)}
            onConfirm={() =>
              deleteReview(review.id, { onSuccess: () => setDeleteOpen(false) })
            }
    ...
    const columns_13 = (): ColumnDef_2<ReviewPublic_2>[] => [
      {
        accessorKey: "name",
        header: "Author Name",
        cell: ({ row }) => (
          <div>
            <div className="text-foreground font-semibold">{row.original.name}</div>
      ...
      },
      {
        accessorKey: "rating",
        header: "Rating",
        cell: ({ row }) => <RatingStars_3 rating={row.original.rating} />,
      },
      {
        accessorKey: "comment",
        header: "Comment / Review",
        cell: ({ row }) => (
          <div className="text-foreground line-clamp-3 max-w-sm text-xs font-medium leading-relaxed">
    ...
    const ReviewsListingTable = () => {
      ...
      const [meta, setMeta] = useState<TMeta_23>({ total: 0, limit, page });

      const { data: reviewsResponse, isLoading } =
        SuperAdminDataHooks.usePendingReviews({
          page,
          limit,
          search,
        });
      ...
      useEffect(() => {
        const total = (reviewsResponse as { meta?: { total?: number } })?.meta
          ?.total;
        setMeta((prev) => ({
          ...prev,
          page,
          limit,
          total: typeof total === "number" ? total : reviewsList.length,
        }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [reviewsResponse, page, limit]);
          ...
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-4">
              <Heading
                ...
                description="Review rating submissions and comments from students or instructors before they are approved for display on the public website."
              />
            </div>
          </PageHeaderLayout>
