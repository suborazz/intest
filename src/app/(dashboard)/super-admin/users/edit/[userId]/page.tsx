...
import { useRef, ReactNode, useEffect, useContext, useMemo } from "react";
...
const ZEditUser = updateUserSchema
  .pick({
    name: true,
  })
  .extend({
...
type TEditUser = z.infer<typeof ZEditUser>;
...
const EditForm_9: React_3.FunctionComponent<IEditFormProps_2> = ({ data }) => {
  const [showDialog] = React_3.useState(true);
  ...
  function onSubmit(values: TEditUser) {
    updateUser({
      id: data.id,
      payload: {
        name: values.name,
  ...
  return (
    <ResponsiveDialog
      isOpen={showDialog}
      setIsOpen={(open) => !open && router.back()}
      title="Edit User Details"
      description="Update profile name and role assignments."
    >
      <Form {...form}>
...
const EditFrom_2 = EditForm_9;

interface IEditUserPageProps {
    params: Promise<{ userId: string }>;
}

const EditUserPage: React.FunctionComponent<IEditUserPageProps> = ({
  params,
}) => {
  const { userId } = React.use(params);
  const { data, isLoading } = UserDataHook.useUserDetail(userId);

  return !isLoading && data?.data?.user ? (
    <EditFrom_2 data={data.data.user} />
  ) : (
    <div>Loading...</div>
  );
};
