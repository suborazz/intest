...
import { AlertTriangle, Award, Calendar, CreditCard, Download, Edit, Edit2Icon, EllipsisVerticalIcon, ExternalLink, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, PlusCircle, School, Send, ShieldCheck, Trash2, Trash2Icon, User, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
import { Select as SelectPrimitive, Slot, Checkbox as CheckboxPrimitive, Separator as SeparatorPrimitive, Label as LabelPrimitive, Switch as SwitchPrimitive, Tooltip as TooltipPrimitive } from "radix-ui";
...
function DropdownMenuCheckboxItem({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      data-inset={inset}
      className={cn(
        "outline-hidden focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-inset:pl-7.5 data-disabled:pointer-events-none data-disabled:opacity-50 relative flex min-h-7 cursor-pointer select-none items-center gap-2 rounded-md py-1.5 pl-2 pr-8 text-xs [&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      checked={checked}
      onSelect={(e) => {
...
const ROLE_FILTERS: TAlertFilter_13[] = [
      { label: "Student", value: "STUDENT", count: null },
      { label: "Instructor", value: "INSTRUCTOR", count: null },
      { label: "Immersion User", value: "IMMERSION_USER", count: null },
      { label: "Job Applicant User", value: "RECRUIT_USER", count: null },
      { label: "Super Admin", value: "SUPER_ADMIN", count: null },
    ];

const UsersSidebarFilters = () => {
      return (
...
function cn_21(...inputs: ClassValue_2[]) {
...
const ROLE_BADGE: Record<string, string> = {
      ...
      IMMERSION_USER: "bg-indigo-500/10 text-indigo-600",
      RECRUIT_USER: "bg-emerald-500/10 text-emerald-600",
    };

const RoleChip_2 = ({ role }: { role: string }) => (
      <Badge
        variant="ghost"
        className={cn_21(
          "pointer-events-none gap-1 rounded-full border-0 px-2.5 py-0.5 text-[10px] font-semibold shadow-none",
...
function DropdownMenu_19({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_19({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface DeleteConfirmationDialogProps_4 {
      isOpen: boolean;
      onClose: () => void;
      onConfirm: () => void;
      isDeleting?: boolean;
...
function AlertDialog_7({
      ...props
    }: React_3.ComponentProps<typeof AlertDialogPrimitive.Root>) {
      return <AlertDialogPrimitive.Root data-slot="alert-dialog" {...props} />;
    }

function DeleteConfirmationDialog_8({
      isOpen,
      onClose,
      onConfirm,
      isDeleting = false,
      userEmail,
    }: DeleteConfirmationDialogProps_4) {
      return (
        <AlertDialog_7 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent>
...
export default function SuperAdminUsersPage() {
    ...
    const ActiveToggle_2 = ({ userId, isActive }: IActiveToggleProps) => {
      ...
      return (
        ...
        >
          <Switch
            ...
            disabled={isPending}
            onCheckedChange={(checked) =>
              updateUser({ id: userId, payload: { isActive: checked } })
            }
    ...
    const ActiveToggle_3 = ActiveToggle_2;
    const UserActions: React.FunctionComponent<IUserActionsProps> = ({
      user,
    }) => {
      const router = useRouter();
      ...
      if (user.role === "STUDENT") {
        ...
        hasProfile =
          !!user.instructorRegistration &&
          !user.instructorRegistration.deletedAt;
      } else if (user.role === "IMMERSION_USER") {
        hasProfile = !!user.immersionParticipantProfile;
      } else if (user.role === "RECRUIT_USER") {
        hasProfile = !!user.recruitProfile;
      }

      const handleSendReminder = async () => {
        try {
          setIsSendingReminder(true);
          const res = await axiosInstance.post("/users/send-reminder", {
            userId: user.id,
          });
          toast.success(
        ...
        <div onClick={(e) => e.stopPropagation()}>
          <DropdownMenu_19>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              {!hasProfile && user.role !== "SUPER_ADMIN" && (
                <DropdownMenuItem
                  disabled={isSendingReminder}
                  onClick={handleSendReminder}
                  className="flex cursor-pointer items-center justify-between text-amber-600 dark:text-amber-400 focus:text-amber-600"
                >
                  <span>{isSendingReminder ? "Sending..." : "Send Reminder Email"}</span>
                  <span className="bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400 inline-flex items-center justify-center rounded-md p-1">
                    <Mail_2 size={12} />
                  </span>
                </DropdownMenuItem>
              )}
          ...
          <DeleteConfirmationDialog_8
            isOpen={deleteOpen}
            onClose={() => setDeleteOpen(false)}
            onConfirm={() =>
              deleteUser(user.id, { onSuccess: () => setDeleteOpen(false) })
            }
    ...
    const columns_15 = (page: number, limit: number): ColumnDef_2<UserPublic>[] => [
      {
        id: "serialNumber",
        header: "S. No.",
        cell: ({ row }) => (
          <div className="text-muted-foreground font-mono text-center text-xs font-bold">
      ...
      },
      {
        accessorKey: "role",
        header: "Role",
        cell: ({ row }) => <RoleChip_3 role={row.original.role} />,
      },
      {
        id: "profileStatus",
        ...
        cell: ({ row }) => {
          ...
          let hasProfile = false;
          let profileLabel = "N/A";

          if (user.role === "STUDENT") {
            hasProfile =
              !!user.studentRegistration && !user.studentRegistration.deletedAt;
            profileLabel = hasProfile ? "Completed" : "Not Created";
          } else if (user.role === "INSTRUCTOR") {
            hasProfile =
              !!user.instructorRegistration &&
              !user.instructorRegistration.deletedAt;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <ActiveToggle_3 userId={row.original.id} isActive={row.original.isActive} />
        ),
      },
      {
        id: "actions",
    ...
    const UsersListingTable = () => {
      ...
      const role = searchParams?.get("role")?.split(",")[0] || undefined;

      const [meta, setMeta] = useState<TMeta_25>({ total: 0, limit, page });

      const { data: usersResponse, isLoading } = UserDataHook.useUsers({
        page,
        limit,
        search,
        role: role ? (role as UserRole_2) : undefined,
      });
      ...
      const handleExport = async () => {
        try {
          ...
          const link = document.createElement("a");
          link.href = url;
          link.download = "user-accounts.xlsx";
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          window.URL.revokeObjectURL(url);
      ...
      useEffect(() => {
        const total = (usersResponse as { meta?: { total?: number } })?.meta?.total;
        setMeta((prev) => ({
          ...prev,
          page,
          limit,
          total: typeof total === "number" ? total : usersList.length,
        }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [usersResponse, page, limit]);

      const sidebarContent = useMemo(() => <UsersSidebarFilters />, []);
          ...
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <Heading
                ...
                description="Create new database user profiles, toggle active status, and modify organizational role permissions."
              />
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  className="w-full shrink-0 border-amber-500/30 bg-amber-500/10 text-amber-700 hover:bg-amber-500/20 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400 sm:w-auto"
                >
                  <Mail_2 className="mr-2 h-4 w-4" />
                  {isSendingBulk
                    ? "Sending Reminders..."
