...
import { LucideIcon, ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, TableIcon, LoaderIcon } from "lucide-react";
...
const CategoryChip_2: React_4.FunctionComponent<ICategoryChipProps> = ({
      category,
    }) => {
      if (!category) {
      ...
      return (
        <Badge
          variant="outline"
          className="bg-muted text-[9px] font-bold uppercase tracking-wider"
        >
          {category}
        </Badge>
      );
...
const CategoryChip_3 = CategoryChip_2;

function formatHumanReadableDate_15(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
function DropdownMenu_8({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_8({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
function DeleteConfirmationDialog_5({
      ...
      return (
        <AlertDialog_2 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent>
            <AlertDialogHeader className="flex flex-col items-center text-center">
              ...
              <AlertDialogDescription className="text-center">
                ...
                ? This action will permanently remove the post from the public blogs
...
export default function SuperAdminBlogsPage() {
    ...
    class BLOGS {
      private static readonly BLOGS_URL = "/super-admin/blogs";

      static readonly MAIN_FILTERS = `?page=1&size=10`;
      static readonly MAIN = `${this.BLOGS_URL}${this.MAIN_FILTERS}`;

      static withParams(params: string) {
        return `${this.BLOGS_URL}?${params}`;
      }

      static BY_ID(id: string) {
        return `${this.BLOGS_URL}/${id}`;
      }

      static NEW() {
        return `${this.BLOGS_URL}/new`;
      }
    }
    ...
    const BlogActions: React.FunctionComponent<IBlogActionsProps> = ({
      blog,
    }) => {
      const router = useRouter();
      ...
      const { mutate: deleteBlog, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteBlog();

      return (
        <div onClick={(e) => e.stopPropagation()}>
          <DropdownMenu_8 open={open} onOpenChange={setOpen} modal={false}>
            <DropdownMenuTrigger_8 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <EllipsisVerticalIcon className="text-muted-foreground h-4 w-4" />
              </Button>
            </DropdownMenuTrigger_8>
            <DropdownMenuContent align="end" className="w-36">
              <DropdownMenuItem
                onClick={() => router.push(BLOGS.BY_ID(blog.id))}
          ...
          <DeleteConfirmationDialog_5
            isOpen={deleteOpen}
            onClose={() => setDeleteOpen(false)}
            onConfirm={() =>
              deleteBlog(blog.id, { onSuccess: () => setDeleteOpen(false) })
            }
    ...
    const columns_4 = (): ColumnDef_2<BlogPublic_2>[] => [
      {
        accessorKey: "title",
        header: "Blog Title",
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: "Category",
        cell: ({ row }) => <CategoryChip_3 category={row.original.category} />,
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-foreground text-xs font-medium">
            {formatHumanReadableDate_15(row.original.date)}
          </div>
        ),
      },
      {
    ...
    const BlogsListingTable = () => {
      ...
      const [meta, setMeta] = useState<TMeta_10>({ total: 0, limit, page });

      const { data: blogsResponse, isLoading } = SuperAdminDataHooks.useBlogs({
        page,
        limit,
        search,
      });
      ...
      useEffect(() => {
        const total = (blogsResponse as { meta?: { total?: number } })?.meta?.total;
        setMeta((prev) => ({
          ...prev,
          page,
          limit,
          total: typeof total === "number" ? total : blogsList.length,
        }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [blogsResponse, page, limit]);
        ...
        <div>
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
              <Heading
                title="Blogs & Articles Board"
                description="Publish news, success stories, and educational articles for users on the platform."
              />
          ...
          <div className="w-full min-w-0">
            <DataTable
              ...
              columns={columns_4()}
              data={blogsList}
              url={(row) => BLOGS.BY_ID(row.id).slice(1)}
              meta={meta}
              setMeta={setMeta}
              toolbar={
                <div className="flex items-center gap-2">
