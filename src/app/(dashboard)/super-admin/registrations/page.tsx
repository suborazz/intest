...
import { Briefcase, Compass, GraduationCap, UserCheck } from "lucide-react";
import React_2, { Suspense, use } from "react";
import { AlertTriangle, Award, BookOpen, Briefcase as Briefcase_2, Calendar, Check as Check_2, Code, CreditCard, Download, Edit, Edit2, ExternalLink, Eye, FileDown, FileSignature, FileText, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, MoreVertical, Phone as Phone_2, Plus, School, Send, ShieldCheck, Trash2, Trash2Icon, User, Users as Users_2, X, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Phone, Search, Target, Users, XCircle as XCircle_2 } from "lucide-react";
...
import { AlertDialog as AlertDialogPrimitive, Dialog as SheetPrimitive, DropdownMenu as DropdownMenuPrimitive } from "radix-ui";
import axios from "axios";
import { toast } from "sonner";
import { ColumnDef as ColumnDef_2 } from "@tanstack/react-table";
import { LucideIcon, ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, CopyIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, ArrowUpIcon, FilterIcon, FilterXIcon, TableIcon, CopyCheckIcon, LoaderIcon } from "lucide-react";
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        data-size={size}
        className={cn(
          "group/alert-dialog-content bg-popover text-popover-foreground ring-foreground/10 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 fixed left-1/2 top-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-3 rounded-xl p-4 outline-none ring-1 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-64 data-[size=default]:sm:max-w-sm",
          className,
        )}
        {...props}
      />
...
function AlertDialogDescription({
  ...
  return (
    <AlertDialogPrimitive.Description
      ...
      className={cn(
        "text-muted-foreground *:[a]:underline *:[a]:underline-offset-3 *:[a]:hover:text-foreground text-balance text-xs/relaxed md:text-pretty",
        className,
      )}
      {...props}
    />
...
function AlertDialogHeader({
  ...
  return (
    <div
      ...
      className={cn(
        "has-data-[slot=alert-dialog-media]:grid-rows-[auto_auto_1fr] has-data-[slot=alert-dialog-media]:gap-x-4 sm:group-data-[size=default]/alert-dialog-content:has-data-[slot=alert-dialog-media]:grid-rows-[auto_1fr] grid grid-rows-[auto_1fr] place-items-center gap-1 text-center sm:group-data-[size=default]/alert-dialog-content:place-items-start sm:group-data-[size=default]/alert-dialog-content:text-left",
        className,
      )}
      {...props}
    />
...
function AlertDialogMedia({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-muted *:[svg:not([class*='size-'])]:size-4 mb-2 inline-flex size-8 items-center justify-center rounded-md sm:group-data-[size=default]/alert-dialog-content:row-span-2",
        className,
      )}
      {...props}
    />
...
function AlertDialogTitle({
  ...
  return (
    <AlertDialogPrimitive.Title
      ...
      className={cn(
        "font-heading sm:group-data-[size=default]/alert-dialog-content:group-has-data-[slot=alert-dialog-media]/alert-dialog-content:col-start-2 text-sm font-medium",
        className,
      )}
      {...props}
    />
...
const TABS = [
      {
        ...
        href: "student",
        link: "/super-admin/registrations?tab=student",
        icon: GraduationCap,
      },
      {
        id: "instructor",
        ...
        href: "instructor",
        link: "/super-admin/registrations?tab=instructor",
        icon: UserCheck,
      },
      {
        id: "immersion",
        label: "Immersion Registration",
        href: "immersion",
        link: "/super-admin/registrations?tab=immersion",
        icon: Compass,
      },
      {
        id: "recruit",
        label: "Recruit Registration",
        href: "recruit",
        link: "/super-admin/registrations?tab=recruit",
        icon: Briefcase,
      },
    ];
...
function formatHumanReadableDate_20(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
interface IApplicationActionsProps_2 {
      app: ImmersionApp;
    }

function DropdownMenu_14({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_14({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface IDetailDialogProps_2 {
      app: ImmersionApp | null;
      isOpen: boolean;
      onClose: () => void;
    }

interface AcademicDetail_2 {
      highestQualification: string;
...
interface ImmersionApplicationDetail_2 extends ImmersionApp {
      academicDetails?: AcademicDetail_2[];
...
interface ImmersionParticipantProfile_2 {
...
function formatHumanReadableDate_21(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
interface DeleteConfirmationDialogProps_2 {
      isOpen: boolean;
      onClose: () => void;
      onConfirm: () => void;
      isDeleting?: boolean;
...
function AlertDialog_5({
      ...props
    }: React_3.ComponentProps<typeof AlertDialogPrimitive.Root>) {
      return <AlertDialogPrimitive.Root data-slot="alert-dialog" {...props} />;
    }

function DeleteConfirmationDialog_6({
      isOpen,
      onClose,
      onConfirm,
      isDeleting = false,
    ...
    }: DeleteConfirmationDialogProps_2) {
      return (
        <AlertDialog_5 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent>
            <AlertDialogHeader className="flex flex-col items-center text-center">
              <AlertDialogMedia className="bg-destructive/10 text-destructive dark:bg-destructive/20 dark:text-destructive mx-auto">
                <Trash2Icon className="size-4" />
              </AlertDialogMedia>
              <AlertDialogTitle className="w-full text-center sm:col-start-auto sm:text-center">
                {title}
              </AlertDialogTitle>
            ...
            <AlertDialogFooter className="flex w-full flex-row gap-3 sm:justify-stretch">
              ...
              <AlertDialogAction
                ...
                onClick={onConfirm}
                disabled={isDeleting}
                className="flex flex-1 cursor-pointer items-center justify-center gap-2 font-semibold"
              >
                {isDeleting ? (
                  <>
...
interface InstructorReg extends InstructorRegistrationResponse {
      name?: string;
      email?: string;
      status?: string;
      isApproved?: boolean;
...
function downloadCSV_2(headers: string[], rows: string[][], filename: string) {
        const BOM = "\uFEFF";
      const content = [
        headers.map(h => `"${h.replace(/"/g, '""')}"`).join(","),
        ...rows.map(row => row.map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(","))
...
const STATUS_FILTERS_4: TAlertFilter_9[] = [
  { label: "Pending", value: "PENDING", count: null },
  { label: "Approved", value: "APPROVED", count: null },
  { label: "Rejected", value: "REJECTED", count: null },
];

const InstructorRegistrationsSidebarFilters = () => {
  return (
    <SidebarFiltersHeader className="mr-0 px-3">
      <SidebarCheckBoxFilter
        header="status"
        filters={STATUS_FILTERS_4}
        filter_name="Status"
      />
    </SidebarFiltersHeader>
  );
...
function formatHumanReadableDate_22(date?: string | null): string {
  if (!date) return "N/A";
  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) return "N/A";
  return parsed.toLocaleDateString("en-IN", {
...
interface IInstructorRegActionsProps {
  reg: InstructorReg;
}

function DropdownMenu_15({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
  return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
}

function DropdownMenuTrigger_15({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
  return (
    <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface IDetailDialogProps_3 {
  reg: InstructorReg;
  isOpen: boolean;
  onClose: () => void;
}

function formatHumanReadableDate_23(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
interface RecruitReg {
      ...
      academics?: RecruitAcademic[];
      experiences?: RecruitExperience[];
    }

function downloadCSV_3(headers: string[], rows: string[][], filename: string) {
        const BOM = "\uFEFF";
      const content = [
        headers.map(h => `"${h.replace(/"/g, '""')}"`).join(","),
        ...rows.map(row => row.map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(","))
...
const GENDER_FILTERS: TAlertFilter_10[] = [
  { label: "Male", value: "Male", count: null },
  { label: "Female", value: "Female", count: null },
  { label: "Transgender", value: "Transgender", count: null },
];

const RecruitRegistrationsSidebarFilters = () => {
  return (
    <SidebarFiltersHeader className="mr-0 px-3">
      <SidebarCheckBoxFilter
        header="gender"
...
function formatHumanReadableDate_24(date?: string | null): string {
  if (!date) return "N/A";
  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) return "N/A";
  return parsed.toLocaleDateString("en-IN", {
...
interface IRecruitRegActionsProps {
  reg: RecruitReg;
}

function DropdownMenu_16({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
  return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
}

function DropdownMenuTrigger_16({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
  return (
    <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface IDetailDialogProps_4 {
  reg: RecruitReg;
  isOpen: boolean;
  onClose: () => void;
}

function formatHumanReadableDate_25(date?: string | null): string {
  if (!date) return "N/A";
  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) return "N/A";
  return parsed.toLocaleDateString("en-IN", {
...
const DetailDialog_4: React_3.FunctionComponent<IDetailDialogProps_4> = ({
  reg,
  isOpen,
  onClose,
}) => {
  ...
  return (
    <ResponsiveDialog
      ...
      title="Recruit Registration Profile"
    >
      <div className="max-h-[80vh] space-y-6 overflow-y-auto pr-2 text-sm">
        {}
        <div className="border-border/60 bg-muted/20 flex flex-col items-center gap-4 rounded-2xl border p-4 sm:flex-row">
          <div className="border-border/80 bg-muted relative h-20 w-20 shrink-0 overflow-hidden rounded-xl border">
            {reg.photoUrl ? (
              <img
                src={reg.photoUrl}
                alt={reg.fullName}
          ...
          <div className="min-w-0 flex-1 space-y-1.5 text-center sm:text-left">
            ...
            <p className="text-muted-foreground truncate text-xs">
              {reg.user?.email || reg.email || "No Email"}
            </p>
            <div className="flex flex-wrap justify-center gap-1.5 pt-0.5 sm:justify-start">
              <Badge
                variant="outline"
                className="border-amber-500/20 bg-amber-500/5 px-2 py-0 text-[10px] font-medium text-amber-600"
              >
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {}
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div>
                ...
                <span className="text-foreground font-medium">
                  {reg.dob ? formatHumanReadableDate_25(reg.dob) : "N/A"}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground block text-[10px]">
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <Phone_2 className="size-3.5" /> Contact Details
            </h4>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <MapPin_2 className="size-3.5" /> Current Address
            </h4>
            <div className="text-xs leading-relaxed">
              <p className="text-foreground font-medium">
                {reg.currAddressLocal || "N/A"}
              </p>
              <p className="text-muted-foreground mt-1">
                {[
                  reg.currAddressDistrict,
                  reg.currAddressState,
                  reg.currAddressCountry,
                ]
                  .filter(Boolean)
                  .join(", ")}
              </p>
              <p className="text-muted-foreground mt-0.5">
                PIN Code: {reg.currAddressPinCode || "N/A"}
              </p>
            </div>
          </div>
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <MapPin_2 className="size-3.5" /> Permanent Address
            </h4>
            <div className="text-xs leading-relaxed">
              <p className="text-foreground font-medium">
                {reg.permAddressLocal || "N/A"}
              </p>
              <p className="text-muted-foreground mt-1">
                {[
              ...
              <p className="text-muted-foreground mt-0.5">
                PIN Code: {reg.permAddressPinCode || "N/A"}
              </p>
            </div>
          </div>
        </div>
        ...
        {}
        {reg.academics && reg.academics.length > 0 && (
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <BookOpen className="size-3.5" /> Academic History
            </h4>
            <div className="space-y-4">
              {reg.academics.map((acad, idx) => (
                <div
                  key={acad.id || idx}
                  className="border-border/40 flex flex-col justify-between gap-4 border-b pb-3 last:border-b-0 last:pb-0 sm:flex-row"
                >
                  <div className="space-y-1">
                    <span className="text-foreground font-semibold">
                      {acad.qualification} ({acad.subject})
                    </span>
                    <p className="text-muted-foreground text-xs">
                      {acad.schoolInstitute} | {acad.boardUniversity}
                    </p>
                    {acad.certificateUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-primary mt-1 flex items-center gap-1 text-[10px] hover:underline"
                      >
                        View Certificate <ExternalLink size={10} />
                      </a>
                    )}
                  </div>
                  <div className="flex flex-row gap-4 text-left text-xs sm:flex-col sm:text-right">
                    <div>
                    ...
                    <div>
                      ...
                      <span className="text-foreground font-medium">
                        {acad.passingDivision} ({acad.passingMarks})
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {}
        {reg.experiences && reg.experiences.length > 0 && (
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <Briefcase_2 className="size-3.5" /> Work Experience
            </h4>
            <div className="space-y-4">
              {reg.experiences.map((exp, idx) => (
                <div
                  key={exp.id || idx}
                  className="border-border/40 flex flex-col justify-between gap-4 border-b pb-3 last:border-b-0 last:pb-0 sm:flex-row"
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-xs">
                      Location: {exp.postingLocation} | Nature:{" "}
                      {exp.natureOfWork}
                    </p>
                    {exp.certUrl && (
                      <a
                        ...
                        className="text-primary mt-1 flex items-center gap-1 text-[10px] hover:underline"
                      >
                        View Experience Certificate <ExternalLink size={10} />
                      </a>
                    )}
                  </div>
                  <div className="flex flex-row gap-4 text-left text-xs sm:flex-col sm:text-right">
                    <div>
                      ...
                      <span className="text-foreground font-medium">
                        {formatHumanReadableDate_25(exp.startDate)} -{" "}
                        {exp.endDate
                          ? formatHumanReadableDate_25(exp.endDate)
                          : "Present"}
                      </span>
                    </div>
                    <div>
        ...
        <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
          <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
            <FileSignature className="size-3.5" /> Attachments & Documents
          </h4>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {reg.resumeUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="border-border/40 bg-muted/5 hover:bg-primary/5 hover:border-primary/30 flex items-center justify-between rounded-xl border p-3 font-semibold transition-all"
              >
                <span>Curriculum Vitae (Resume)</span>
                <ExternalLink className="text-primary size-3.5 shrink-0" />
              </a>
            )}
            {reg.signatureUrl && (
              <div className="space-y-1.5">
                ...
                <div className="border-border/60 bg-muted/30 flex h-12 w-40 items-center justify-center rounded-lg border p-1">
                  <img
                    src={reg.signatureUrl}
                    alt="Signature"
...
interface StudentReg extends StudentRegistrationResponse {
      name?: string;
      email?: string;
      status?: string;
      institution?: string;
...
function downloadCSV_4(headers: string[], rows: string[][], filename: string) {
        const BOM = "\uFEFF";
      const content = [
        headers.map(h => `"${h.replace(/"/g, '""')}"`).join(","),
        ...rows.map(row => row.map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(","))
...
const STATUS_FILTERS_5: TAlertFilter_11[] = [
  { label: "Pending", value: "PENDING", count: null },
  { label: "Approved", value: "APPROVED", count: null },
  { label: "Rejected", value: "REJECTED", count: null },
];

const StudentRegistrationsSidebarFilters = () => {
  return (
    <SidebarFiltersHeader className="mr-0 px-3">
      <SidebarCheckBoxFilter
        header="status"
        filters={STATUS_FILTERS_5}
        filter_name="Status"
      />
    </SidebarFiltersHeader>
  );
...
function formatHumanReadableDate_26(date?: string | null): string {
  if (!date) return "N/A";
  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) return "N/A";
  return parsed.toLocaleDateString("en-IN", {
...
interface IStudentRegActionsProps {
  reg: StudentReg;
}

function DropdownMenu_17({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
  return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
}

function DropdownMenuTrigger_17({
  ...props
}: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
  return (
    <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface IDetailDialogProps_5 {
  reg: StudentReg;
  isOpen: boolean;
  onClose: () => void;
}

function formatHumanReadableDate_27(date?: string | null): string {
  if (!date) return "N/A";
  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) return "N/A";
  return parsed.toLocaleDateString("en-IN", {
...
const DetailDialog_5: React_3.FunctionComponent<IDetailDialogProps_5> = ({
  reg,
  isOpen,
  onClose,
}) => {
    ...
    >
      <div className="max-h-[80vh] space-y-6 overflow-y-auto pr-2 text-sm">
          ...
          <div className="min-w-0 flex-1 space-y-1.5 text-center sm:text-left">
            ...
            <div className="flex flex-wrap justify-center gap-1.5 pt-0.5 sm:justify-start">
              <Badge
                ...
                className="px-2 py-0 text-[10px] font-medium"
              >
                Goal: {reg.internshipGoal?.replace("_", " ") || "Not Specified"}
              </Badge>
              <Badge
                variant="outline"
                className="border-amber-500/20 bg-amber-500/5 px-2 py-0 text-[10px] font-medium text-amber-600"
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div>
                ...
                <span className="text-foreground font-medium">
                  {reg.dob ? formatHumanReadableDate_27(reg.dob) : "N/A"}
                </span>
              </div>
              <div className="col-span-2">
                <span className="text-muted-foreground block text-[10px]">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <MapPin_2 className="size-3.5" /> Local Address
            </h4>
            <div className="text-xs leading-relaxed">
              <p className="text-foreground font-medium">
                {reg.localAddressLocal || "N/A"}
              </p>
              <p className="text-muted-foreground mt-1">
                {[
              ...
              <p className="text-muted-foreground mt-0.5">
                PIN Code: {reg.localAddressPinCode || "N/A"}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {reg.academics.map((acad, idx) => (
                ...
                >
                  <div className="space-y-1">
                    <span className="text-foreground font-semibold">
                      {acad.qualification} ({acad.stream || acad.subject})
                    </span>
                    <p className="text-muted-foreground text-xs">
                      {acad.instituteName || acad.universityName}
                    </p>
                  </div>
                  <div className="flex flex-row gap-4 text-left text-xs sm:flex-col sm:text-right">
                    <div>
        ...
        {reg.skills && reg.skills.length > 0 && (
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
              <Award className="size-3.5" /> Technical Skills & Certifications
            </h4>
            <div className="flex flex-wrap gap-2">
              {reg.skills.map((sk, idx) => (
                <div
                  key={idx}
                  className="bg-muted/50 border-border/65 rounded-xl border px-3 py-2 text-xs"
                >
                  ...
                  {sk.certifyingBody && (
                    <div className="text-muted-foreground text-[10px]">
                      Certified by {sk.certifyingBody}{" "}
                      {sk.certYear ? `(${sk.certYear})` : ""}
                        </div>
                      )}
                    </div>
                  ))}
            ...
            <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
              <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                <FileSignature className="size-3.5" /> Signed Documents
              </h4>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                {reg.signatureUrl && (
                  <div className="space-y-1.5">
                    ...
                    <div className="border-border/60 bg-muted/30 flex h-16 w-40 items-center justify-center rounded-lg border p-2">
                      <img
                        src={reg.signatureUrl}
                        alt="Signature"
                        className="max-h-full max-w-full object-contain dark:invert"
...
export default function RegistrationsPage({
  ...
  searchParams: Promise<{ tab?: string }>;
    ...
    const DetailDialog_2: React_3.FunctionComponent<IDetailDialogProps_2> = ({
      app,
      isOpen,
      onClose,
    }) => {
      const [selectedMentorId, setSelectedMentorId] = React_3.useState("");

        const { data: detailResponse, isLoading: isDetailLoading } =
        SuperAdminDataHooks.useImmersionApplicationDetail(app?.id || "", {
          enabled: isOpen && !!app?.id,
        });

      const fullData = detailResponse?.data as
        | {
            application?: ImmersionApplicationDetail_2;
            profile?: ImmersionParticipantProfile_2;
          }
        | undefined;
      const application = fullData?.application;
      ...
      const instructors = (instructorsResponse?.data ||
        []) as InstructorBasicInfo[];

      const { mutate: assignMentor, isPending: isAssigning } =
        SuperAdminDataHooks.useAssignImmersionMentor();

      React_3.useEffect(() => {
        if (application) {
          setSelectedMentorId(application.mentorId || application.mentor?.id || "");
        }
      }, [application]);
      ...
      const getAddressString = (
        ...
        if (!addr) return "N/A";
        const parts = [
          addr.localArea || addr.currLocalArea || addr.permLocalArea,
          addr.district || addr.currDistrict || addr.permDistrict,
          addr.state || addr.currState || addr.permState,
          addr.country || "India",
          addr.pinCode || addr.currPinCode || addr.permPinCode,
        ].filter(Boolean);
        return parts.join(", ");
      };
      ...
      return (
        <ResponsiveDialog
          isOpen={isOpen}
          setIsOpen={(o: boolean) => !o && onClose()}
          title={
            <span className="flex items-center gap-1.5 text-sm font-bold">
              <Info className="text-primary size-4" /> Immersion Application Specs
            </span>
          }
          description="Detailed application breakdown for student participant cohort enrollment."
        >
          <div className="max-h-[75vh] space-y-6 overflow-y-auto pr-1 pt-2 text-xs">
            {isDetailLoading ? (
              <div className="space-y-4">
              ...
              <div className="text-muted-foreground py-6 text-center">
                Failed to retrieve full application details.
              </div>
            ) : (
              <>
                {}
                <div className="border-border/60 bg-muted/20 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4">
                  <div>
                    <h4 className="text-foreground text-sm font-bold leading-snug">
                      {application.user?.name || "Applicant"}
                    </h4>
                    <span className="text-muted-foreground mt-0.5 block text-[10px] font-semibold">
                      Applied On:{" "}
                      {application.submittedAt
                        ? formatHumanReadableDate_21(application.submittedAt)
                        : formatHumanReadableDate_21(application.createdAt)}
                    </span>
                  </div>
                  <Badge
                    variant="outline"
                    className={
                      application.status === "APPROVED"
                        ? "border-emerald-500/20 bg-emerald-500/10 font-bold text-emerald-600"
                        : application.status === "REJECTED"
                          ? "border-rose-500/20 bg-rose-500/10 font-bold text-rose-600"
                ...
                {}
                <div className="border-border/50 bg-background flex items-start gap-3 rounded-xl border p-3.5">
                ...
                <div className="space-y-2.5">
                  <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                    <User className="text-muted-foreground size-3.5" /> Personal
                    Profile
                  </h5>
                  <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <div className="bg-muted/5 border-border/40 rounded-xl border p-3">
                ...
                <div className="space-y-2.5">
                  <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                    <MapPin_2 className="text-muted-foreground size-3.5" /> Address
                    Details
                  </h5>
                  <div className="space-y-2">
                    <div className="bg-muted/5 border-border/40 rounded-xl border p-3.5">
                      <span className="text-muted-foreground block text-[9px] font-bold uppercase tracking-wider">
                        Current Address
                      </span>
                      <p className="text-foreground mt-1 font-medium leading-relaxed">
                        {getAddressString(
                          profile
                            ? {
                                localArea: profile.currLocalArea,
                                district: profile.currDistrict,
                                state: profile.currState,
                                country: profile.currCountry,
                                pinCode: profile.currPinCode,
                              }
                            : null,
                        )}
                      </p>
                    ...
                    <div className="bg-muted/5 border-border/40 rounded-xl border p-3.5">
                      ...
                      <p className="text-foreground mt-1 font-medium leading-relaxed">
                        {getAddressString(
                          profile
                            ? {
                                localArea: profile.permLocalArea,
                                district: profile.permDistrict,
                ...
                {application.academicDetails &&
                  application.academicDetails.length > 0 && (
                    <div className="space-y-2.5">
                      <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                        <GraduationCap_2 className="text-muted-foreground size-3.5" />{" "}
                        Academic History
                      </h5>
                      <div className="border-border/40 overflow-hidden rounded-xl border">
                        <table className="w-full border-collapse text-left text-[11px]">
                          <thead>
                            <tr className="bg-muted/30 border-border/40 text-muted-foreground border-b font-semibold">
                              <th className="p-2.5">Qualification</th>
                              <th className="p-2.5">Stream/Specialization</th>
                              <th className="p-2.5">University/Institute</th>
                              <th className="p-2.5 text-center">Year</th>
                              <th className="p-2.5 text-right">Score/Percentage</th>
                            </tr>
                          </thead>
                          <tbody className="divide-border/30 divide-y">
                            {application.academicDetails.map(
                              (ac: AcademicDetail_2, idx: number) => (
                                <tr
                                  key={idx}
                                  className="hover:bg-muted/5 text-foreground/80 font-medium"
                                >
                                  <td className="text-foreground p-2.5 font-bold">
                                    {ac.highestQualification}
                                  </td>
                                  <td className="p-2.5">
                                    {ac.specialization || "N/A"}
                                  </td>
                                  <td className="p-2.5">{ac.universityName}</td>
                ...
                <div className="space-y-2.5">
                  <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                    <FileText className="text-muted-foreground size-3.5" /> Document
                    Attachments
                  </h5>
                  <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
                    ...
                    {application.educationCertUrl && (
                      <a
                        href={application.educationCertUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="border-border/40 bg-muted/5 hover:bg-primary/5 hover:border-primary/30 flex items-center justify-between rounded-xl border p-3 font-semibold transition-all"
                      >
                ...
                {application.remarks && (
                  <div className="space-y-1.5">
                    ...
                    <p className="text-muted-foreground bg-muted/5 border-border/40 rounded-xl border p-3 leading-relaxed">
                      {application.remarks}
                    </p>
                  </div>
                )}
                ...
                {}
                <div className="border-border/50 bg-muted/10 flex flex-col gap-2 rounded-xl border p-3.5">
                  <div className="flex items-center gap-2">
                    <User className="text-primary size-4 shrink-0" />
                    <span className="text-muted-foreground block text-[9px] font-bold uppercase tracking-wider">
                      Assign Instructor as Mentor
                    </span>
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <select
                      ...
                      className="border-border/80 bg-background text-foreground focus:border-primary focus:ring-primary h-9 flex-1 rounded-lg border px-3 text-xs font-medium transition-all focus:outline-none focus:ring-1"
                    >
                    ...
                    <Button
                      size="sm"
                      variant={selectedMentorId ? "default" : "destructive"}
                      disabled={
                        isAssigning ||
                        selectedMentorId ===
                          (application.mentorId || application.mentor?.id || "")
                      }
                      onClick={() =>
                        assignMentor({
                          id: app.id,
    ...
    const ApplicationActions_3: React.FunctionComponent<
      IApplicationActionsProps_2
    > = ({ app }) => {
      const router = useRouter();
      ...
      const { mutate: updateStatus } =
        SuperAdminDataHooks.useUpdateImmersionStatus();

      const { mutate: deleteApp, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteImmersionApplication();

      const { mutate: downloadPDF, isPending: isDownloading } =
        SuperAdminDataHooks.useDownloadImmersionRegistration({
          onSuccess: (blob) => {
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `immersion_application_${app.id}.pdf`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
      ...
      return (
        ...
        >
          {status === "PENDING" && (
            <div className="mr-2 flex items-center gap-1">
              <Button
                ...
                size="icon"
                onClick={() =>
                  updateStatus({ id: app.id, payload: { status: "APPROVED" } })
                }
                className="h-8 w-8 text-emerald-500 hover:bg-emerald-50"
                title="Approve"
              >
                <Check_2 className="size-3.5" />
              </Button>
              <Button
                variant="ghost"
                size="icon"
                onClick={() =>
                  updateStatus({ id: app.id, payload: { status: "REJECTED" } })
                }
          ...
          <DropdownMenu_14
            ...
            modal={false}
          >
            <DropdownMenuTrigger_14 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <MoreVertical className="text-muted-foreground h-4 w-4" />
              </Button>
            </DropdownMenuTrigger_14>
            <DropdownMenuContent align="end" className="w-40 text-xs">
              ...
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
                  router.push(
                    `/super-admin/registrations/edit/immersion/${app.id}`,
                  );
                }}
                className="flex cursor-pointer items-center justify-between"
              >
              ...
              >
                <span>Delete</span>
                <span className="bg-destructive/10 text-destructive dark:bg-destructive/20 dark:text-destructive inline-flex items-center justify-center rounded-md p-1">
                  <Trash2 size={12} />
                </span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          ...
          <DeleteConfirmationDialog_6
            isOpen={deleteOpen}
            onClose={() => setDeleteOpen(false)}
            onConfirm={() =>
              deleteApp(app.id, { onSuccess: () => setDeleteOpen(false) })
            }
            isDeleting={isDeleting}
            applicantName={app.user?.name}
            title="Delete Immersion Registration?"
          />
        </div>
      );
    };

    const columns_9 = (page: number = 1, limit: number = 10): ColumnDef_2<ImmersionApp>[] => [
      {
        id: "serialNumber",
        header: "S.No.",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant="outline"
            className="rounded-md text-[9px] font-bold uppercase"
          >
            {row.original.immersion?.title || "N/A"}
          </Badge>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const acad = row.original.academicDetails?.[0];
          if (!acad)
            return <span className="text-muted-foreground italic">N/A</span>;
          return (
            <div>
              <div className="text-foreground text-xs font-medium">
                {acad.qualification || acad.highestQualification || "N/A"}
              </div>
              <div className="text-muted-foreground text-[10px]">
                {acad.universityName || acad.stream || ""}
              </div>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-muted-foreground text-xs font-medium">
            {row.original.submittedAt
              ? formatHumanReadableDate_20(row.original.submittedAt)
              : formatHumanReadableDate_20(row.original.createdAt)}
          </div>
        ),
      },
      {
        id: "actions",
        header: () => <div className="text-right">Actions</div>,
        cell: ({ row }) => <ApplicationActions_3 app={row.original} />,
        enableSorting: false,
        enableHiding: false,
      },
    ];

    const ImmersionRegistrationsListingTable = () => {
      const searchParams = useSearchParams();

        const page = Number(searchParams?.get("page")) || 1;
      const limit = Number(searchParams?.get("size")) || 10;
      const search = (searchParams?.get("filter") || "").toLowerCase();
      const status = searchParams?.get("status")?.split(",")[0] || undefined;

      const [meta, setMeta] = useState<TMeta_19>({ total: 0, limit, page });
      const [isExporting, setIsExporting] = useState(false);

      const { data: response, isLoading } =
        SuperAdminDataHooks.useImmersionApplications();

      const allApplications = useMemo<ImmersionApp[]>(
      ...
      const filteredList = useMemo(() => {
        return allApplications.filter((app) => {
          ...
          const matchesSearch =
            ...
            app.immersion?.title?.toLowerCase().includes(search);
          return matchesStatus && matchesSearch;
        });
      }, [allApplications, search, status]);
      ...
      const handleExport = () => {
        try {
          ...
          const rows = filteredList.map((item, index) => {
            const acad = item.academicDetails?.[0];
            const qual = acad ? (acad.qualification || acad.highestQualification || "N/A") : "N/A";
          ...
          downloadCSV(headers, rows, "Immersion_Applications.csv");
      ...
      const sidebarContent = useMemo(
        () => <ImmersionApplicationsSidebarFilters />,
        [],
      );
          ...
          <div className="w-full min-w-0">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">
                Total Registered Users: <strong className="text-foreground font-semibold">{meta.total}</strong>
              </span>
            </div>

            <DataTable
              isLoading={isLoading}
              columns={columns_9(page, limit)}
              data={paginatedList}
              meta={meta}
              setMeta={setMeta}
              toolbar={
...
function Cell({
  ...
  return (
    ...
    const DetailDialog_3: React_3.FunctionComponent<IDetailDialogProps_3> = ({
      ...
      const documents = [
        {
          ...
          url: reg.identityProofUrl,
          name: reg.identityProofName,
        },
        {
          label: "Education Certificate",
          url: reg.educationCertUrl,
          name: reg.educationCertName,
        },
        {
          label: "Experience Certificate",
          url: reg.experienceCertUrl,
          name: reg.experienceCertName,
        },
      ].filter((d) => d.url);
      ...
      return (
        <ResponsiveDialog
          isOpen={isOpen}
          setIsOpen={(o) => !o && onClose()}
          title="Instructor Registration Profile"
        >
          <div className="max-h-[80vh] space-y-6 overflow-y-auto pr-2 text-sm">
            {}
            <div className="border-border/60 bg-muted/20 flex flex-col items-center gap-4 rounded-2xl border p-4 sm:flex-row">
              ...
              <div className="min-w-0 flex-1 space-y-1.5 text-center sm:text-left">
                ...
                <div className="flex flex-wrap justify-center gap-1.5 pt-0.5 sm:justify-start">
                  ...
                  {reg.currentOrganization && (
                    <Badge
                      variant="outline"
                      className="border-primary/20 bg-primary/5 text-primary px-2 py-0 text-[10px] font-medium"
                    >
                      {reg.currentOrganization}
                    </Badge>
                  )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <Cell label="Father / Spouse" value={reg.fatherSpouseName} />
                  <Cell label="Gender" value={reg.gender} />
                  <Cell
                    label="Date of Birth"
                    value={reg.dob ? formatHumanReadableDate_23(reg.dob) : undefined}
                  />
                  <Cell label="Availability" value={reg.availability} />
                </div>
              </div>
              ...
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                ...
                <div className="grid grid-cols-1 gap-2 text-xs">
                  <Cell label="Mobile Phone Number" value={reg.mobileNo} />
                  <Cell label="Alternate Mobile" value={reg.alternateMobileNo} />
                  <Cell label="Email Address" value={email} />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                ...
                <div className="text-xs leading-relaxed">
                  <p className="text-foreground font-medium">
                    {reg.currentAddressLocal || "N/A"}
                  </p>
                  <p className="text-muted-foreground mt-1">
                    {[
                      reg.currentAddressDistrict,
                      reg.currentAddressState,
                      reg.currentAddressCountry,
                    ]
                      .filter(Boolean)
                      .join(", ")}
                  </p>
                  <p className="text-muted-foreground mt-0.5">
                    PIN Code: {reg.currentAddressPinCode || "N/A"}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                  <Briefcase_2 className="size-3.5" /> Professional
                </h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <Cell label="Organization" value={reg.currentOrganization} />
                  <Cell label="Designation" value={reg.currentDesignation} />
                  <Cell label="Total Experience" value={reg.totalWorkExperience} />
                  <Cell label="Teaching Exp." value={reg.teachingExperience} />
                  <Cell
                    label="Internship Exp."
                    value={reg.internshipExperience}
              ...
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                  <Users_2 className="size-3.5" /> Mentorship
                </h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <Cell
                    label="Areas"
                    value={reg.mentorshipAreas}
                    className="col-span-2"
                  />
                  <Cell
                    label="Preferred Level"
                    value={reg.preferredInternLevel?.join(", ")}
                  />
                  <Cell label="Max Interns" value={reg.maxInterns} />
                  <Cell
                    label="Mode"
                    value={reg.mentorshipMode?.join(", ")}
                    className="col-span-2"
                  />
                </div>
              </div>
            ...
            {}
            {reg.selfIntroduction && (
              <div className="border-border/50 bg-background space-y-2 rounded-xl border p-4">
                <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                  <User className="size-3.5" /> Self Introduction
                </h4>
            ...
            {}
            {reg.qualifications && reg.qualifications.length > 0 && (
              <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
                <h4 className="text-muted-foreground flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                  <GraduationCap_2 className="size-3.5" /> Qualifications
                </h4>
                <div className="space-y-4">
                  {reg.qualifications.map((q, idx) => (
                    <div
                      key={q.id || idx}
                      className="border-border/40 flex flex-col justify-between gap-4 border-b pb-3 last:border-b-0 last:pb-0 sm:flex-row"
                    >
                ...
                <div className="flex flex-wrap gap-2">
                  {documents.map((doc) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-muted/50 border-border/65 hover:bg-muted flex items-center gap-2 rounded-xl border px-3 py-2 text-xs transition-colors"
                    >
    ...
    const InstructorRegActions: React_3.FunctionComponent<
      IInstructorRegActionsProps
    > = ({ reg }) => {
      const router = useRouter();
      const [dropdownOpen, setDropdownOpen] = React_3.useState(false);
      const [viewOpen, setViewOpen] = React_3.useState(false);
      const [deleteOpen, setDeleteOpen] = React_3.useState(false);

      const { mutate: deleteReg, isPending: isDeleting } =
      ...
      const { mutate: downloadPDF, isPending: isDownloading } =
        SuperAdminDataHooks.useDownloadInstructorRegistration({
          onSuccess: (blob) => {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `instructor_registration_${reg.id}.pdf`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
      ...
      return (
        ...
        >
          <DropdownMenu_15
            ...
            modal={false}
          >
            <DropdownMenuTrigger_15 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <MoreVertical className="text-muted-foreground h-4 w-4" />
              </Button>
            ...
            <DropdownMenuContent align="end" className="w-44 text-xs">
              ...
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
                  router.push(
                    `/super-admin/registrations/edit/instructor/${reg.id}`,
                  );
                }}
                className="flex cursor-pointer items-center justify-between"
              >
          ...
          <DeleteConfirmationDialog_6
            ...
            onClose={() => setDeleteOpen(false)}
            onConfirm={() =>
              deleteReg(reg.id, { onSuccess: () => setDeleteOpen(false) })
            }
            isDeleting={isDeleting}
            applicantName={reg.fullName || reg.name || reg.user?.name}
            title="Delete Instructor Registration?"
          />
        </div>
      );
    };
    ...
    const columns_10 = (page: number = 1, limit: number = 10): ColumnDef_2<InstructorReg>[] => [
        ...
        cell: ({ row }) => (
          <div>
            ...
            <div className="text-muted-foreground mt-0.5 text-[10px] font-medium">
              {row.original.email || row.original.user?.email || "N/A"}
            </div>
          </div>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const qual =
            row.original.qualifications?.[0] || (row.original as any).qualification;
          if (!qual)
            return <span className="text-muted-foreground italic">N/A</span>;
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const parts = [
            row.original.currentAddressLocal || row.original.address,
            row.original.currentAddressDistrict,
            row.original.currentAddressState,
          ].filter(Boolean);
      ...
      },
      {
        ...
        header: "Submitted",
        cell: ({ row }) => formatHumanReadableDate_22(row.original.createdAt),
      },
      {
        id: "actions",
        header: () => <div className="text-right">Actions</div>,
        cell: ({ row }) => <InstructorRegActions reg={row.original} />,
        enableSorting: false,
        enableHiding: false,
      },
    ];

    const InstructorRegistrationsListingTable = () => {
      const searchParams = useSearchParams();
      ...
      const [meta, setMeta] = useState<TMeta_20>({ total: 0, limit, page });
      const [isExporting, setIsExporting] = useState(false);
      ...
      const list: InstructorReg[] = response?.data || [];

      useEffect(() => {
        const total = (response as { meta?: { total?: number } })?.meta?.total;
        setMeta((prev) => ({
          ...prev,
          page,
          limit,
      ...
      const handleExport = async () => {
        try {
          setIsExporting(true);
          const res = await SuperAdminService.getInstructorRegistrations({
            page: 1,
            limit: 10000,
            search,
            status,
          ...
          const rows = dataList.map((item: any, index: number) => {
            const addressParts = [
              item.currentAddressLocal || item.address,
              item.currentAddressDistrict,
              item.currentAddressState
            ].filter(Boolean);
            const qual = item.qualifications?.[0] || item.qualification;
            const highestQual = typeof qual === "string" ? qual : qual?.highestQualification || "N/A";
            const spec = (qual && typeof qual !== "string") ? qual.specialization || "N/A" : "N/A";
            return [
              String(index + 1),
              item.fullName || item.name || item.user?.name || "N/A",
          ...
          downloadCSV_2(headers, rows, "Instructor_Registrations.csv");
          toast.success("Data exported successfully!");
        } catch (error) {
          console.error(error);
          toast.error("Failed to export data");
      ...
      const sidebarContent = useMemo(
        () => <InstructorRegistrationsSidebarFilters />,
        [],
      );
...
const RecruitRegActions: React_3.FunctionComponent<
  IRecruitRegActionsProps
> = ({ reg }) => {
  const router = useRouter();
  const [dropdownOpen, setDropdownOpen] = React_3.useState(false);
  const [viewOpen, setViewOpen] = React_3.useState(false);
  ...
  const { mutate: deleteReg, isPending: isDeleting } =
    SuperAdminDataHooks.useDeleteRecruitRegistration();

  const { mutate: downloadPDF, isPending: isDownloading } =
    SuperAdminDataHooks.useDownloadRecruitRegistration({
      onSuccess: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `recruit_registration_${reg.id}.pdf`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
      ...
      return (
        ...
        >
          <DropdownMenu_16
            ...
            modal={false}
          >
            <DropdownMenuTrigger_16 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <MoreVertical className="text-muted-foreground h-4 w-4" />
              </Button>
            </DropdownMenuTrigger_16>
            <DropdownMenuContent align="end" className="w-40 text-xs">
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
              ...
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
                  router.push(`/super-admin/registrations/edit/recruit/${reg.id}`);
                }}
                className="flex cursor-pointer items-center justify-between"
              >
                <span>Edit details</span>
          ...
          <DeleteConfirmationDialog_6
            ...
            isDeleting={isDeleting}
            applicantName={reg.fullName || reg.user?.name}
            title="Delete Recruit Registration?"
          />
        </div>
      );
    };

    const columns_11 = (page: number = 1, limit: number = 10): ColumnDef_2<RecruitReg>[] => [
      {
        id: "serialNumber",
        header: "S.No.",
        cell: ({ row }) => (page - 1) * limit + row.index + 1,
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            <div className="text-foreground font-semibold">
              {row.original.fullName || row.original.user?.name || "N/A"}
            </div>
            <div className="text-muted-foreground mt-0.5 text-[10px] font-medium">
              {row.original.user?.email || row.original.email || "N/A"}
            </div>
          </div>
        ),
      },
      ...
      },
      {
        ...
        header: "Education / Qualification",
        cell: ({ row }) => {
          const acad = row.original.academics?.[0];
          if (!acad)
            return <span className="text-muted-foreground italic">N/A</span>;
          return (
            <div>
              <div className="text-foreground text-xs font-medium">
                {acad.qualification || "N/A"}
              </div>
              <div className="text-muted-foreground text-[10px]">
                {acad.schoolInstitute || acad.boardUniversity || "N/A"}
              </div>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const parts = [
            row.original.currAddressLocal || row.original.permAddressLocal,
            row.original.currAddressDistrict,
            row.original.currAddressState,
          ].filter(Boolean);
          return parts.length > 0 ? (
            parts.join(", ")
          ) : (
      ...
      },
      {
        accessorKey: "createdAt",
        header: "Applied On",
        cell: ({ row }) => formatHumanReadableDate_24(row.original.createdAt),
      },
      {
        id: "actions",
        header: () => <div className="text-right">Actions</div>,
        cell: ({ row }) => <RecruitRegActions reg={row.original} />,
        enableSorting: false,
        enableHiding: false,
      },
    ];

    const RecruitRegistrationsListingTable = () => {
      const searchParams = useSearchParams();

        const page = Number(searchParams?.get("page")) || 1;
      const limit = Number(searchParams?.get("size")) || 10;
      const search = searchParams?.get("filter") || undefined;
      const gender = searchParams?.get("gender")?.split(",")[0] || undefined;

      const [meta, setMeta] = useState<TMeta_21>({ total: 0, limit, page });
      const [isExporting, setIsExporting] = useState(false);

      const { data: response, isLoading } =
        SuperAdminDataHooks.useRecruitRegistrations({
          page,
          limit,
          search,
          gender,
      ...
      const list: RecruitReg[] = response?.data || [];
      ...
      const handleExport = async () => {
        try {
          setIsExporting(true);
          const res = await SuperAdminService.getRecruitRegistrations({
            page: 1,
            limit: 10000,
            search,
            gender,
          ...
          const rows = dataList.map((item: any, index: number) => {
            const addressParts = [
              item.currAddressLocal || item.permAddressLocal,
              item.currAddressDistrict,
              item.currAddressState
            ].filter(Boolean);
            const acad = item.academics?.[0];
            const qualification = acad ? (acad.qualification || "N/A") : "N/A";
            return [
              String(index + 1),
          ...
          downloadCSV_3(headers, rows, "Recruit_Registrations.csv");
          toast.success("Data exported successfully!");
        } catch (error) {
          console.error(error);
          toast.error("Failed to export data");
      ...
      const sidebarContent = useMemo(
        () => <RecruitRegistrationsSidebarFilters />,
        [],
      );
    ...
    const StudentRegActions: React_3.FunctionComponent<
      IStudentRegActionsProps
    > = ({ reg }) => {
      const router = useRouter();
      const [dropdownOpen, setDropdownOpen] = React_3.useState(false);
      const [viewOpen, setViewOpen] = React_3.useState(false);
      ...
      const { mutate: deleteReg, isPending: isDeleting } =
        SuperAdminDataHooks.useDeleteStudentRegistration();

      const { mutate: downloadPDF, isPending: isDownloading } =
        SuperAdminDataHooks.useDownloadStudentRegistration({
          onSuccess: (blob) => {
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `student_registration_${reg.studentId || reg.id}.pdf`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
      ...
      return (
        ...
        >
          <DropdownMenu_17
            ...
            modal={false}
          >
            <DropdownMenuTrigger_17 asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                <MoreVertical className="text-muted-foreground h-4 w-4" />
              </Button>
            </DropdownMenuTrigger_17>
            <DropdownMenuContent align="end" className="w-40 text-xs">
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
              ...
              <DropdownMenuItem
                onClick={() => {
                  setDropdownOpen(false);
                  router.push(`/super-admin/registrations/edit/student/${reg.id}`);
                }}
                className="flex cursor-pointer items-center justify-between"
              >
                <span>Edit details</span>
    ...
    const columns_12 = (page: number = 1, limit: number = 10): ColumnDef_2<StudentReg>[] => [
      {
        id: "serialNumber",
        header: "S.No.",
        cell: ({ row }) => (page - 1) * limit + row.index + 1,
        ...
        cell: ({ row }) => {
          ...
          return (
            <div>
              ...
              <div className="text-muted-foreground text-[10px]">
                {acad.instituteName || acad.universityName || "N/A"}
              </div>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const parts = [
            row.original.localAddressLocal || row.original.localAddress,
            row.original.localAddressDistrict,
            row.original.localAddressState,
          ].filter(Boolean);
          return parts.length > 0 ? (
            parts.join(", ")
          ) : (
      ...
      },
      {
        accessorKey: "createdAt",
        header: "Applied On",
        cell: ({ row }) => formatHumanReadableDate_26(row.original.createdAt),
      },
      {
        id: "actions",
        header: () => <div className="text-right">Actions</div>,
        cell: ({ row }) => <StudentRegActions reg={row.original} />,
        enableSorting: false,
        enableHiding: false,
      },
    ];

    const StudentRegistrationsListingTable = () => {
      const searchParams = useSearchParams();
      ...
      const [meta, setMeta] = useState<TMeta_22>({ total: 0, limit, page });
      const [isExporting, setIsExporting] = useState(false);

      const { data: response, isLoading } =
        SuperAdminDataHooks.useStudentRegistrations({
          page,
          limit,
          search,
          status,
      ...
      const list: StudentReg[] = response?.data || [];
      ...
      const handleExport = async () => {
        try {
          setIsExporting(true);
          const res = await SuperAdminService.getStudentRegistrations({
            page: 1,
            limit: 10000,
            search,
            status,
          ...
          const rows = dataList.map((item: any, index: number) => {
            const addressParts = [
              item.localAddressLocal || item.localAddress,
              item.localAddressDistrict,
              item.localAddressState
            ].filter(Boolean);
            return [
              String(index + 1),
              item.fullName || item.name || item.user?.name || "N/A",
          ...
          downloadCSV_4(headers, rows, "Student_Registrations.csv");
          toast.success("Data exported successfully!");
        } catch (error) {
          console.error(error);
          toast.error("Failed to export data");
      ...
      const sidebarContent = useMemo(
        () => <StudentRegistrationsSidebarFilters />,
        [],
      );
  ...
  const activeTab = params?.tab || "student";
    ...
    <main className="space-y-4">
      <PageHeaderLayout>
        <div className="flex w-full flex-col gap-4">
          <Heading
            title="Registrations & Onboarding"
            description="Manage and oversee submissions for students, instructors, immersions, and recruiters."
          />
        </div>
      ...
      <Suspense fallback={null}>
        {activeTab === "student" && <StudentRegistrationsListingTable />}
        {activeTab === "instructor" && <InstructorRegistrationsListingTable />}
        {activeTab === "immersion" && <ImmersionRegistrationsListingTable />}
        {activeTab === "recruit" && <RecruitRegistrationsListingTable />}
      </Suspense>
    </main>
  );
}
