...
import { Code, Compass, Edit, GraduationCap as GraduationCap_2, MapPin as MapPin_2, Save, User, Target } from "lucide-react";
...
interface ImmersionApplication {
  id: string;
  status?: string;
  remarks?: string | null;
  submittedAt?: string | null;
  ...
  immersion?: { title?: string; location?: string | null };
  academicDetails?: AcademicDetail_3[];
}

interface ImmersionProfile {
...
interface IEditImmersionFormProps {
  application: ImmersionApplication;
  profile?: ImmersionProfile;
}
...
const Field = ({
  ...
  <div className="bg-muted/5 border-border/40 rounded-xl border p-3">
    ...
    <span className="text-foreground mt-0.5 block text-xs font-semibold">
      {value || "N/A"}
    </span>
  </div>
);

function formatHumanReadableDate_32(date?: string | null): string {
  if (!date) return "N/A";
...
function SelectValue_24({
  ...props
}: React_3.ComponentProps<typeof SelectPrimitive.Value>) {
  return <SelectPrimitive.Value data-slot="select-value" {...props} />;
}

const EditImmersionForm_2: React_3.FunctionComponent<
  IEditImmersionFormProps
> = ({ application, profile }) => {
  ...
  const onSubmit = (values: { status: string; remarks: string }) => {
    updateStatus({
      id: application.id,
      payload: {
  ...
  const address = (parts: (string | null | undefined)[]) =>
  ...
  return (
    <ResponsiveDialog
      ...
      title="Edit Immersion Application"
      description="Review the applicant's registration and record the admissions decision."
    >
      <div className="max-h-[80vh] space-y-6 overflow-y-auto pr-1 text-xs">
        {}
        ...
        <div className="space-y-2.5">
          <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
            <User className="text-muted-foreground size-3.5" /> Personal Profile
          </h5>
          <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
            <Field
              label="Full Name"
              value={profile?.fullName || application.user?.name}
            />
            <Field label="Email ID" value={application.user?.email} />
            <Field label="Mobile Number" value={profile?.mobileNumber} />
            <Field label="Alt Mobile" value={profile?.alternateMobileNo} />
            <Field label="Gender" value={profile?.gender} />
            <Field
              label="Date of Birth"
              value={
                profile?.dateOfBirth
                  ? formatHumanReadableDate_32(String(profile.dateOfBirth))
                  : undefined
              }
            />
            <Field
                  ...
                  {academics.map((ac, idx) => (
                    <tr key={idx} className="text-foreground/80 font-medium">
                      <td className="text-foreground p-2.5 font-bold">
                        {ac.highestQualification || ac.qualification || "N/A"}
                      </td>
                      <td className="p-2.5">
                        {ac.specialization || ac.stream || "N/A"}
                      </td>
                      <td className="p-2.5">{ac.universityName || "N/A"}</td>
                      <td className="p-2.5 text-center">
                        {ac.yearOfCompletion || ac.sessionYear || "N/A"}
                      </td>
                      <td className="p-2.5 text-right font-bold text-emerald-600 dark:text-emerald-400">
                        {ac.percentage || ac.gradeDivision || "N/A"}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="border-primary/20 bg-primary/5 space-y-4 rounded-xl border p-4">
          <h5 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
            <Save className="text-primary size-3.5" /> Admissions Decision
          </h5>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue_24 placeholder="Select Status" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Admin Remarks / Comments</FormLabel>
                    <FormControl>
                      <Input
                        placeholder="Provide remarks or reason for decision..."
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
...
interface IEditInstructorFormProps {
  reg: any;
}
...
function SelectValue_25({
  ...props
}: React_3.ComponentProps<typeof SelectPrimitive.Value>) {
  return <SelectPrimitive.Value data-slot="select-value" {...props} />;
}

const EditInstructorForm_2: React_3.FunctionComponent<
  IEditInstructorFormProps
> = ({ reg }) => {
  const router = useRouter();
  ...
  const form = useForm({
    defaultValues: {
      ...
      currentOrganization: reg.currentOrganization || "",
      currentDesignation: reg.currentDesignation || "",
      totalWorkExperience: reg.totalWorkExperience || "",
      teachingExperience: reg.teachingExperience || "",
      internshipExperience: reg.internshipExperience || "",
      mentorshipAreas: reg.mentorshipAreas || "",
      preferredInternLevel: reg.preferredInternLevel || "",
      maxInterns: reg.maxInterns || 5,
      mentorshipMode: reg.mentorshipMode || "Hybrid",
      availability: reg.availability || "",
      selfIntroduction: reg.selfIntroduction || "",
      currentAddressLocal: reg.currentAddressLocal || "",
      currentAddressDistrict: reg.currentAddressDistrict || "",
      currentAddressState: reg.currentAddressState || "",
      currentAddressPinCode: reg.currentAddressPinCode || "",
      permAddressLocal: reg.permAddressLocal || "",
      permAddressDistrict: reg.permAddressDistrict || "",
      permAddressState: reg.permAddressState || "",
      permAddressPinCode: reg.permAddressPinCode || "",
    },
  });

  const { mutate: updateReg, isPending } =
    SuperAdminDataHooks.useUpdateInstructorRegistration({
      onSuccess: () => {
        router.back();
        router.refresh();
      },
  ...
  return (
    <ResponsiveDialog
      ...
      title="Edit Instructor Registration Profile"
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue_25 placeholder="Select Gender" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
          ...
          <div className="border-t pt-4">
            <h4 className="mb-2 text-sm font-semibold">Current Address</h4>
            <div className="grid grid-cols-2 gap-4">
              <FormField
                control={form.control}
                name="currentAddressLocal"
                render={({ field }) => (
                  <FormItem>
          ...
          <div className="border-t pt-4">
            <h4 className="mb-2 text-sm font-semibold">Permanent Address</h4>
            <div className="grid grid-cols-2 gap-4">
              <FormField
                control={form.control}
                name="permAddressLocal"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Local Address</FormLabel>
                    <FormControl>
...
interface IEditRecruitFormProps {
  reg: any;
}

const formatDob = (dateStr: string | null | undefined): string => {
  if (!dateStr) return "";
...
const mapReligionBack = (r: string | null | undefined): string => {
  if (!r) return "Other";
...
const mapBloodGroupBack = (b: string | null | undefined): string => {
  if (!b) return "O+";
  const map: Record<string, string> = {
    A_POS: "A+",
    A_NEG: "A-",
...
function SelectValue_26({
  ...props
}: React_3.ComponentProps<typeof SelectPrimitive.Value>) {
  return <SelectPrimitive.Value data-slot="select-value" {...props} />;
}

const EditRecruitForm_2: React_3.FunctionComponent<
  IEditRecruitFormProps
> = ({ reg }) => {
  const router = useRouter();
  ...
  const form = useForm({
    defaultValues: {
      ...
      dob: formatDob(reg.dob),
      gender: reg.gender || "Male",
      mobileNo: reg.mobileNo || "",
      email: reg.email || "",
      maritalStatus: reg.maritalStatus || "Unmarried",
      nationality: reg.nationality || "Indian",
      gotra: reg.gotra || "",
      religion: mapReligionBack(reg.religion),
      category: reg.category || "General",
      bloodGroup: mapBloodGroupBack(reg.bloodGroup),
      hobby: reg.hobby || "",
      languagesKnown: reg.languagesKnown || "",
      physicallyChallenged: reg.physicallyChallenged ? "Yes" : "No",
      aadharNo: reg.aadharNo || "",
      currAddressLocal: reg.currAddressLocal || "",
      currAddressDistrict: reg.currAddressDistrict || "",
      currAddressState: reg.currAddressState || "",
      currAddressPinCode: reg.currAddressPinCode || "",
      permAddressLocal: reg.permAddressLocal || "",
      permAddressDistrict: reg.permAddressDistrict || "",
      permAddressState: reg.permAddressState || "",
      permAddressPinCode: reg.permAddressPinCode || "",
  ...
  const { mutate: updateReg, isPending } =
    SuperAdminDataHooks.useUpdateRecruitRegistration({
      onSuccess: () => {
        router.back();
        router.refresh();
      },
  ...
  const onSubmit = (values: any) => {
    updateReg({
      ...
      payload: {
        ...
        localAddress: {
          local: values.currAddressLocal,
          district: values.currAddressDistrict,
          state: values.currAddressState,
          country: "India",
          pinCode: values.currAddressPinCode,
        },
        permanentAddress: {
          local: values.permAddressLocal,
          district: values.permAddressDistrict,
  ...
  return (
    <ResponsiveDialog
      ...
      title="Edit Recruit Registration Profile"
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue_26 placeholder="Select Gender" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      <SelectItem value="Male">Male</SelectItem>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue_26 placeholder="Select Category" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
...
function SelectValue_27({
  ...props
}: React_3.ComponentProps<typeof SelectPrimitive.Value>) {
  return <SelectPrimitive.Value data-slot="select-value" {...props} />;
}

const EditStudentForm_2: React_3.FunctionComponent<
...
> = ({ reg }) => {
  ...
  const form = useForm({
    defaultValues: {
      ...
      mobileNo: reg.mobileNo || "",
      aadharNo: reg.aadharNo || "",
      internshipGoal: reg.internshipGoal || "Skill Enhancement",
      localAddressLocal: reg.localAddressLocal || "",
      localAddressDistrict: reg.localAddressDistrict || "",
      localAddressState: reg.localAddressState || "",
      localAddressPinCode: reg.localAddressPinCode || "",
      sameAsLocal: reg.sameAsLocal || false,
      permAddressLocal: reg.permAddressLocal || "",
      permAddressDistrict: reg.permAddressDistrict || "",
      permAddressState: reg.permAddressState || "",
      permAddressPinCode: reg.permAddressPinCode || "",
  ...
  const { mutate: updateReg, isPending } =
    SuperAdminDataHooks.useUpdateStudentRegistration({
      onSuccess: () => {
        router.back();
        router.refresh();
      },
  ...
  const onSubmit = (values: any) => {
    updateReg({
      ...
      payload: {
        ...
        localAddress: {
          local: values.localAddressLocal,
          district: values.localAddressDistrict,
          state: values.localAddressState,
          country: "India",
          pinCode: values.localAddressPinCode,
        },
        permanentAddress: {
          local: values.permAddressLocal,
          district: values.permAddressDistrict,
  ...
  return (
    <ResponsiveDialog
      ...
      description="Update candidate profiles and onboarding registration details."
    >
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="space-y-4 pb-4 pr-2 pt-2 text-xs"
        >
          {}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue_27 placeholder="Select Gender" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectItem value="Male">Male</SelectItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="col-span-1 sm:col-span-2">
                    <FormLabel>Internship Goal</FormLabel>
                    <Select_27
                      onValueChange={field.onChange}
                      value={field.value}
                      defaultValue={field.value}
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue_27 placeholder="Select Goal" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
          ...
          <div className="border-border/50 bg-background space-y-3 rounded-xl border p-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <FormField
                control={form.control}
                name="localAddressLocal"
                render={({ field }) => (
                  <FormItem className="col-span-1 sm:col-span-2">
                    <FormLabel>Flat / Street / Locality</FormLabel>
                    <FormControl>
...
export default function EditRegistrationPage() {
  ...
  if (type === "student") {
    return <StudentEditWrapper regId={regId} />;
  }
  if (type === "instructor") {
    return <InstructorEditWrapper regId={regId} />;
  }
  if (type === "immersion") {
    return <ImmersionEditWrapper regId={regId} />;
  }
  if (type === "recruit") {
    return <RecruitEditWrapper regId={regId} />;
  }

  return (
    <div className="p-6 text-sm text-red-500">Invalid registration type</div>
  );
}

function StudentEditWrapper({ regId }: { regId: string }) {
  const { data, isLoading } =
    SuperAdminDataHooks.useStudentRegistrationDetail(regId);
  if (isLoading)
    return (
      <div className="text-muted-foreground animate-pulse p-6 text-sm">
  ...
  if (!data?.data)
    return (
      <div className="text-muted-foreground p-6 text-sm">
        Student registration profile not found.
      </div>
    );
  return <EditStudentForm_2 reg={data.data} />;
}

function InstructorEditWrapper({ regId }: { regId: string }) {
  const { data, isLoading } =
    SuperAdminDataHooks.useInstructorRegistrationDetail(regId);
  if (isLoading)
    return (
      <div className="text-muted-foreground animate-pulse p-6 text-sm">
  ...
  if (!data?.data)
    return (
      <div className="text-muted-foreground p-6 text-sm">
        Instructor registration profile not found.
      </div>
    );
  return <EditInstructorForm_2 reg={data.data} />;
}

function ImmersionEditWrapper({ regId }: { regId: string }) {
  const { data, isLoading } =
    SuperAdminDataHooks.useImmersionApplicationDetail(regId);
  if (isLoading)
    return (
      <div className="text-muted-foreground animate-pulse p-6 text-sm">
        Loading Immersion Application...
      </div>
    );
  const detail = data?.data as any;
  const application = detail?.application ?? detail;
  if (!application)
    return (
      <div className="text-muted-foreground p-6 text-sm">
  ...
  return (
    <EditImmersionForm_2 application={application} profile={detail?.profile} />
  );
}

function RecruitEditWrapper({ regId }: { regId: string }) {
  const { data, isLoading } =
    SuperAdminDataHooks.useRecruitRegistrationDetail(regId);
  if (isLoading)
    return (
      <div className="text-muted-foreground animate-pulse p-6 text-sm">
        Loading Recruit Info...
      </div>
    );
  if (!data?.data)
    return (
      <div className="text-muted-foreground p-6 text-sm">
        Recruit registration profile not found.
      </div>
    );
  return <EditRecruitForm_2 reg={data.data} />;
}
