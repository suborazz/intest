...
import { Activity as Activity_2, AlertTriangle, Award, BookOpen, Calendar, Check as Check_2, Clock as Clock_2, CreditCard, DollarSign, Download, Edit, ExternalLink, Eye, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, LucideIcon, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, School, Send, ShieldCheck, Sparkles as Sparkles_2, Trash2, Trash2Icon, User, Users as Users_2, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
import { ChevronLeftIcon, ChevronRightIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, CopyIcon, Loader2Icon, XIcon, SearchIcon, ChevronsLeftIcon, ChevronsRightIcon, ArrowUpIcon, FilterIcon, FilterXIcon, TableIcon, CopyCheckIcon, LoaderIcon } from "lucide-react";
...
const TYPE_TO_STATUS: Record<string, "ACTIVE" | "UPCOMING" | "COMPLETED"> = {
      active: "ACTIVE",
      upcoming: "UPCOMING",
      completed: "COMPLETED",
    };
...
interface ImmersionProgramDetail {
      id: string;
      title: string;
      description?: string;
      status?: string;
...
function formatImmersionCode_13(
      code?: string | null,
...
const IMMERSION_TABS: TSectionItems_4[] = [
      {
        id: "active",
        label: "Active Immersions",
        href: "active",
        link: "/super-admin/immersions?type=active",
        icon: Activity_2,
      },
      {
        id: "upcoming",
        label: "Upcoming Immersions",
        href: "upcoming",
        link: "/super-admin/immersions?type=upcoming",
        icon: Clock_2,
      },
      {
        id: "completed",
        label: "Completed Immersions",
        href: "completed",
        link: "/super-admin/immersions?type=completed",
        icon: Check_2,
      },
    ];
...
interface ImmersionProgramDetail_3 {
      id: string;
      title: string;
      description?: string;
      status?: string;
...
function formatImmersionCode_12(
      code?: string | null,
      programTitle?: string | null,
    ): string {
      if (!code) return "IM2026XX10001";
...
interface ImmersionProgramDetail_2 {
      id: string;
      title: string;
      description?: string;
      status?: string;
...
interface ImmersionActionsProps {
      program: ImmersionProgramDetail_2;
    }

function DropdownMenu_9({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_9({
      ...props
    }: React.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
interface ImmersionDetailDialogProps {
      isOpen: boolean;
      onClose: () => void;
      program: ImmersionProgramDetail | null;
    }

const ImmersionDetailDialog: React_3.FC<ImmersionDetailDialogProps> = ({
      isOpen,
      onClose,
      program,
    }) => {
        ...
        <ResponsiveDialog
          ...
          title={
            <span className="flex items-center gap-1.5 text-sm font-bold">
              <Info className="text-primary size-4" /> Program Catalog Details
            </span>
          }
          description="View full specifications of the published immersion program catalog."
        >
          <div className="max-h-[70vh] space-y-5 overflow-y-auto pr-1 pt-2 text-xs">
            {}
            <div className="border-border/60 bg-muted/20 space-y-2 rounded-xl border p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <Badge className="bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider">
                  {program.category?.name || "Immersion Program"}
                </Badge>
                <Badge
                  variant="outline"
                  className={
              ...
              <h3 className="text-foreground text-sm font-bold leading-snug">
                {program.title}
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="border-border/50 bg-background flex items-start gap-2.5 rounded-xl border p-3">
                <MapPin_2 className="text-muted-foreground mt-0.5 size-4 shrink-0" />
                <div>
              ...
              <div className="border-border/50 bg-background flex items-start gap-2.5 rounded-xl border p-3">
                ...
                <div>
                  ...
                  <span className="mt-0.5 block font-extrabold text-emerald-600 dark:text-emerald-400">
                    {program.fees === 0
                      ? "Free"
            ...
            <div className="space-y-1.5">
              <h4 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                <BookOpen className="text-muted-foreground size-3.5" /> Description
              </h4>
              <p className="text-muted-foreground bg-muted/5 border-border/40 whitespace-pre-line rounded-xl border p-3.5 leading-relaxed">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <h4 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                  <Award className="text-muted-foreground size-3.5" /> Facilities
                  Provided
                </h4>
                <div className="border-border/40 bg-muted/5 min-h-[80px] rounded-xl border p-3">
                  {parseList(program.facilities).length > 0 ? (
                    <ul className="text-muted-foreground list-inside list-disc space-y-1">
                      {parseList(program.facilities).map((item, idx) => (
                        <li key={idx}>{item}</li>
                      ))}
                    </ul>
                  ) : (
              ...
              <div className="space-y-2">
                <h4 className="text-foreground flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider">
                  <Sparkles_2 className="text-muted-foreground size-3.5" /> Program
                  Benefits
                </h4>
                <div className="border-border/40 bg-muted/5 min-h-[80px] rounded-xl border p-3">
                  {parseList(program.benefits).length > 0 ? (
                    <ul className="text-muted-foreground list-inside list-disc space-y-1">
                      {parseList(program.benefits).map((item, idx) => (
                        <li key={idx}>{item}</li>
                      ))}
                    </ul>
                  ) : (
...
interface DeleteImmersionDialogProps {
      isOpen: boolean;
      onClose: () => void;
      onConfirm: () => void;
      isDeleting?: boolean;
...
function AlertDialog_3({
      ...props
    }: React_3.ComponentProps<typeof AlertDialogPrimitive.Root>) {
      return <AlertDialogPrimitive.Root data-slot="alert-dialog" {...props} />;
    }

function DeleteImmersionDialog_2({
      isOpen,
      onClose,
      onConfirm,
      isDeleting = false,
      programTitle,
    }: DeleteImmersionDialogProps) {
      return (
        <AlertDialog_3 open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
            <AlertDialogHeader className="flex flex-col items-center text-center">
              ...
              <AlertDialogDescription className="text-center">
                ...
                applications associated with this program might be affected.
...
export default function SuperAdminImmersionsPage() {
    ...
    const ImmersionsSidebarFilters = () => {
      const { data: categoriesRes } =
        SuperAdminDataHooks.useAdminImmersionCategories();

      const categoryFilters: TAlertFilter_7[] = (categoriesRes?.data || []).map(
    ...
    const ImmersionActions: React.FC<ImmersionActionsProps> = ({
      program,
    }) => {
      ...
      const { mutate: deleteImmersion, isPending } =
        SuperAdminDataHooks.useDeleteImmersion();

      const handleDeleteConfirm = () => {
        deleteImmersion(program.id, {
        ...
        >
          <DropdownMenu_9>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem asChild className="cursor-pointer font-semibold">
                <Link href={`/super-admin/immersions/edit/${program.id}`}>
    ...
    const columns_6 = (): ColumnDef<ImmersionProgramDetail_3>[] => [
      {
        ...
        header: "Immersion Program ID",
        cell: ({ row }) => {
          const code = formatImmersionCode_12(row.original.id, row.original.title);
          return (
            <span className="inline-flex shrink-0 items-center rounded-md border border-emerald-200/70 bg-emerald-50/80 px-2 py-0.5 font-mono text-[11px] font-bold tracking-tight text-emerald-700 transition-colors dark:border-emerald-800/50 dark:bg-emerald-950/40 dark:text-emerald-300">
              {code}
            </span>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col">
            <span className="text-foreground max-w-[200px] truncate text-xs font-bold leading-snug">
              {row.getValue("title")}
            </span>
      ...
      },
      {
        ...
        header: "Fees",
        cell: ({ row }) => {
          const val = row.getValue("fees") as number;
          return (
      ...
      {
        id: "actions",
        cell: ({ row }) => <ImmersionActions program={row.original} />,
      },
    ];
    ...
    const CreateCategoryDialog = ({
      ...
      const { mutate: createCategory, isPending } =
        SuperAdminDataHooks.useCreateImmersionCategory({
          onSuccess: () => {
            setName("");
            onClose();
          },
      ...
      return (
        <ResponsiveDialog
          isOpen={isOpen}
          setIsOpen={(open) => !open && onClose()}
          title="Create Immersion Category"
          description="Add a new domain category for immersion program organization."
        >
    ...
    const ImmersionsListingTable = () => {
      const searchParams = useSearchParams();

      const activeType = searchParams?.get("type") || "active";
      const targetStatus = TYPE_TO_STATUS[activeType] || "ACTIVE";
      ...
      const [meta, setMeta] = useState<TMeta_12>({ total: 0, limit, page });
      ...
      const allImmersions = useMemo<ImmersionProgramDetail[]>(
        () => (response?.data as ImmersionProgramDetail[]) || [],
        [response],
      );

      const filteredList = useMemo(() => {
        return allImmersions.filter((item) => {
          const matchesStatus = item.status === targetStatus;
          const formattedId = formatImmersionCode_13(item.id, item.title);
          const matchesSearch =
            !search ||
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [allImmersions, search, searchParams, targetStatus]);
      ...
      const sidebarContent = useMemo(() => <ImmersionsSidebarFilters />, []);
        ...
        <>
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
              <Heading
                title="Immersion Programs Catalog"
                description="Manage your available immersion programs — view, publish new offerings, edit, or delete them."
              />
              <div className="flex w-full flex-row gap-1.5 sm:w-auto sm:items-center">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setCatOpen(true)}
                  className="h-8 w-full flex-1 cursor-pointer justify-center rounded-lg border-dashed text-xs font-semibold sm:w-auto sm:flex-initial"
                >
                ...
                <Button
                  size="sm"
                  asChild
                  className="h-8 w-full flex-1 cursor-pointer rounded-lg text-xs font-semibold sm:w-auto sm:flex-initial"
                >
                  <Link
                    href="/super-admin/immersions/new"
            ...
            <Sections sections={IMMERSION_TABS} activeHref={activeType} />
          </PageHeaderLayout>
