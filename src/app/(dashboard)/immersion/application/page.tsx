...
import React_5 from "react";
import { AlertTriangle, Award, Calendar, ClipboardList, CreditCard, Download, ExternalLink, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, RefreshCw, School, Search as Search_2, Send, ShieldCheck, Trash2, User, AlertCircle as AlertCircle_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Search, XCircle as XCircle_2 } from "lucide-react";
...
const STATUS_FILTERS: TAlertFilter_5[] = [
      { label: "Draft (Unpaid)", value: "DRAFT", count: null },
...
const ApplicationsSidebarFilters = () => {
      return (
...
interface ApplicationsTableProps {
      applications: EmersionApplicationData[];
      isLoading: boolean;
    }
...
function formatImmersionCode_9(
      code?: string | null,
...
function generateApplicationCode_2(
      id: string,
      createdAt?: string | Date | null,
    ): string {
        const date = createdAt ? new Date(createdAt) : new Date();
...
export default function EmersionApplicationPage() {
    function useAuth() {
    ...
    function ApplicationsTable({
      ...
      const [meta, setMeta] = useState<TMeta_6>({ total: 0, limit, page });

      const filteredList = useMemo(() => {
        return applications.filter((app) => {
          const matchesStatus = !status || app.status === status;
          const matchedStatic = IMMERSION_PROGRAMS.find(
            (p) => p.id === app.immersionId,
          );
          const title = (
            app.immersion?.title ||
            matchedStatic?.title ||
          ...
          const location = (
            app.immersion?.location ||
            matchedStatic?.location ||
            ""
          ).toLowerCase();

          const formattedProgId = formatImmersionCode_9(
            app.immersionId || app.id,
            app.immersion?.title || matchedStatic?.title || "",
          );
          const formattedAppId = generateApplicationCode_2(
            app.id,
          ...
          const matchesSearch =
            ...
            (app.immersionId && app.immersionId.toLowerCase().includes(search)) ||
            formattedProgId.toLowerCase().includes(search) ||
            formattedAppId.toLowerCase().includes(search);
      ...
      const tableColumns = useMemo(() => columns(userName), [userName]);
          ...
          <DataTable
            ...
            toolbar={
              <div className="flex items-center gap-2">
                ...
                <DataTableSearch
                  searchKey="filter"
                  placeholder="Search by ID or program name"
                  small
                />
              </div>
            }
    ...
    function EmersionApplicationContent_2() {
      const router = useRouter();
      ...
      const sidebarContent = React_3.useMemo(
          ...
          <div className="bg-card/85 border-border/50 mx-auto my-12 flex max-w-md flex-col items-center space-y-4 rounded-2xl border p-6 text-center shadow-lg backdrop-blur-md">
            ...
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground text-xs leading-normal">
                Could not retrieve your application. Please try again.
              </p>
            </div>
            <Button
              onClick={() => refetch()}
          ...
          <div className="bg-card/85 border-border/50 animate-in fade-in slide-in-from-bottom-6 duration-400 relative mx-auto mt-12 max-w-2xl overflow-hidden rounded-3xl border p-8 shadow-xl backdrop-blur-md">
            ...
            <div className="flex flex-col items-center space-y-6 text-center">
              <div className="bg-primary/10 text-primary border-primary/20 flex size-16 items-center justify-center rounded-2xl border shadow-sm">
                <ClipboardList className="size-9 animate-pulse" />
              </div>
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground mx-auto max-w-md text-sm leading-relaxed">
                  You haven&apos;t applied to any immersion program yet. Browse
                  available programs and apply with a single click.
                </p>
              </div>
              <Button
                id="btn-browse-emersion-programs"
                onClick={() => router.push("/immersion/programs")}
                className="bg-primary text-primary-foreground hover:bg-primary/95 h-11 w-full max-w-xs gap-2 rounded-xl font-semibold shadow-md transition-all active:scale-[0.98]"
              >
                <Search_2 className="size-4" />
                Browse Programs
              </Button>
            </div>
          </div>
        ...
        <div className="mx-auto space-y-6 p-4 px-0">
          <PageHeaderLayout>
            <Heading
              ...
              description="Track the status of your immersion program application."
            />
          </PageHeaderLayout>
  ...
  return (
    <main className="min-h-full">
      <h1 className="sr-only">Immersion Program Application</h1>
      <EmersionApplicationContent_2 />
    </main>
  );
}
