...
import { ArrowRight as ArrowRight_2, Briefcase as Briefcase_2, Calendar, Check as Check_2, Compass, Copy, Download, Edit, Edit2, GraduationCap as GraduationCap_2, HeartHandshake, Mail as Mail_2, MapPin as MapPin_2, Phone as Phone_2, Printer, RefreshCw, ShieldCheck, User, UserCircle, Users as Users_2, Building } from "lucide-react";
...
interface EmersionProfileData_2 {
      id: string;
      userId: string;
      emersionId?: string;
...
interface EmersionProfileHeaderProps {
      profile: EmersionProfileData_2;
      onEdit: () => void;
    }

function generateImmersionParticipantId_3(sequence: number): string {
      const currentYear = new Date().getFullYear();
      const formattedSeq = String(sequence).padStart(5, "0");
      return `IMM-${currentYear}-${formattedSeq}`;
    }

const getInitials_2 = (name: string): string => {
      if (!name) return "EU";
      ...
      if (parts.length === 1) {
        ...
        return word.length >= 2
          ...
          : (word[0] + "U").toUpperCase();
      }
      return (parts[0][0] + parts[parts.length - 1][0]).toUpperCase();
    };

interface EmersionProfileData_3 {
      id: string;
      userId: string;
      emersionId?: string;
...
interface EmersionProfilePersonalProps {
      profile: EmersionProfileData_3;
      email?: string;
    }

function EmersionProfilePersonal({
      profile,
      email,
    }: EmersionProfilePersonalProps) {
      const details = [
        {
          label: "Full Name",
        ...
        },
        {
          ...
          value: profile.fatherSpouseName,
          icon: Users_2,
          color: "text-emerald-600 bg-emerald-500/10",
        },
        {
        ...
        },
        {
          ...
          value: profile.alternateMobileNo || "—",
          icon: Phone_2,
          color: "text-teal-600 bg-teal-500/10",
        },
        {
      ...
      return (
        <div className="bg-card border-border/60 rounded-2xl border p-4 sm:p-5 print:border-0 print:p-0">
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-3.5 sm:grid-cols-2 print:grid-cols-2">
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    ...
                    <span className="text-foreground block text-sm font-medium leading-tight print:text-xs">
                      {item.value || "—"}
                    </span>
                  </div>
                </div>
...
interface EmersionProfileAddressProps {
      currentAddressLocal: string;
      currentAddressDistrict: string;
      currentAddressState: string;
      currentAddressCountry: string;
      ...
      return (
        <div className="bg-card border-border/60 group relative overflow-hidden rounded-2xl border p-4 sm:p-5 print:border-0 print:p-0">
          ...
          <h4 className="text-foreground mb-3 flex items-center gap-2 text-sm font-bold print:mb-1.5">
            <div
              className={`rounded-lg p-1.5 ${iconColor} print:bg-transparent print:p-0`}
            >
              <Icon className="size-3.5" />
            </div>
            {label}
...
function EmersionProfileAddress(props: EmersionProfileAddressProps) {
      const {
        currentAddressLocal,
        currentAddressDistrict,
        currentAddressState,
      ...
      return (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 print:grid-cols-2">
          <AddressBlock
            label="Current Address Coordinate"
            local={currentAddressLocal}
            district={currentAddressDistrict}
            state={currentAddressState}
            country={currentAddressCountry}
            pinCode={currentAddressPinCode}
            icon={MapPin_2}
            iconColor="text-emerald-600 bg-emerald-500/10 dark:bg-emerald-500/20 dark:text-emerald-400"
          />
          {sameAsCurrentAddress ? (
            <div className="bg-card border-border/60 flex flex-col items-center justify-center rounded-2xl border border-dashed p-4 text-center print:items-start print:justify-start print:border-0 print:p-0 print:text-left">
              <h4 className="text-foreground mb-1.5 flex items-center gap-2 text-sm font-bold">
              ...
              <p className="text-muted-foreground text-xs font-semibold print:text-xs">
                Same as Current Address
              </p>
            </div>
          ) : (
            <AddressBlock
              label="Permanent Address Coordinate"
              local={permAddressLocal}
              district={permAddressDistrict}
              state={permAddressState}
              country={permAddressCountry}
              pinCode={permAddressPinCode}
              icon={Compass}
              iconColor="text-teal-600 bg-teal-500/10 dark:bg-teal-500/20 dark:text-teal-400"
            />
          )}
        </div>
      );
...
interface EmersionProfileApplicationProps {
      app: EmersionApplicationData;
    }
...
function qualificationLabel(q: string): string {
      const map: Record<string, string> = {
        ...
        UNDER_POST_GRADUATE: "Post Graduate (Pursuing)",
        POST_GRADUATE_PASS_OUT: "Post Graduate (Pass Out)",
        UNDER_MPHIL: "M.Phil (Pursuing)",
        MPHIL_PASS_OUT: "M.Phil (Pass Out)",
        UNDER_PHD: "Ph.D (Pursuing)",
...
function EducationSection({
      ...
      academics: EmersionApplicationData["academicDetails"];
    }) {
      return (
        <div className="bg-card border-border/60 rounded-2xl border p-4 sm:p-5 print:border-0 print:p-0">
          <h3 className="text-foreground border-border/40 mb-4 flex items-center gap-2 border-b pb-2 text-base font-bold print:pb-1.5">
            <div className="rounded-lg bg-emerald-500/10 p-1.5 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 print:bg-transparent print:p-0">
              <GraduationCap_2 className="size-3.5" />
            </div>
                    ...
                    academics.map((ac, idx) => (
                      ...
                      >
                        <TableCell className="text-foreground p-3 font-semibold">
                          {qualificationLabel(ac.qualification)}
                        </TableCell>
                        <TableCell className="text-muted-foreground p-3">
                          <span className="text-foreground text-xs font-semibold">
                            {ac.stream || "—"}
                          </span>
                          {ac.subject && ac.subject !== ac.stream && (
...
function presenceLabel(p: string | null | undefined): string {
      if (p === "FULL_TIME") return "Offline / On Campus";
      if (p === "PART_TIME") return "Online";
      if (p === "HYBRID") return "Hybrid";
      return "—";
    }

function workTypeLabel(w: string | null | undefined): string {
      const map: Record<string, string> = {
        GOVT_JOB: "Job in Government Sector",
        NGO_JOB: "Job in NGO / Non-Govt Sector",
...
function AvailabilitySection({ app }: { app: EmersionApplicationData }) {
      const details = [
        {
          label: "Presence during Immersion",
          value: presenceLabel(app.presenceType),
          icon: MapPin_2,
          color: "text-emerald-600 bg-emerald-500/10",
        },
        {
          label: "Comfortable with Field Visits",
          value:
            app.fieldVisitsComfort === true
              ? "Yes"
              : app.fieldVisitsComfort === false
                ? "No"
                : "—",
          icon: HeartHandshake,
          color: "text-teal-600 bg-teal-500/10",
        ...
        {
          label: "Current Work / Occupation",
          value: workTypeLabel(app.workType),
          icon: Briefcase_2,
          color: "text-emerald-600 bg-emerald-500/10",
        },
      ];
...
function EmergencyContactSection({ app }: { app: EmersionApplicationData }) {
      const details = [
        {
          label: "Contact Name",
        ...
        {
          label: "Relationship",
          value: app.emergencyRelationship,
          icon: HeartHandshake,
          color: "text-teal-600 bg-teal-500/10",
        },
        {
          label: "Emergency Mobile",
          value: app.emergencyMobile,
          icon: Phone_2,
          color: "text-teal-600 bg-teal-500/10",
        },
      ];
...
function EmersionProfileApplication({
      app,
    }: EmersionProfileApplicationProps) {
      return (
        <>
          <EducationSection academics={app.academicDetails} />
          <AvailabilitySection app={app} />
...
export default function EmersionProfilePage() {
    function useAuth() {
    ...
    class IMMERSION {
      private static readonly IMMERSION_URL = "/immersion";

      static readonly DASHBOARD = `${this.IMMERSION_URL}/dashboard`;
      static readonly APPLICATION = `${this.IMMERSION_URL}/application`;
      static readonly STATUS = `${this.IMMERSION_URL}/status`;
      static readonly PROFILE = `${this.IMMERSION_URL}/profile`;
    }

    function EmersionProfileHeader({
      profile,
      onEdit,
    }: EmersionProfileHeaderProps) {
      const { user } = useAuth();
      const [copied, setCopied] = useState(false);
      const [imgError, setImgError] = useState(false);

      const rawId = profile.emersionId ?? profile.userId;
      const numPart = rawId.replace(/\D/g, "");
      const seq = parseInt(numPart.slice(-5), 10) || 0;

      const emersionId =
        profile.emersionId && profile.emersionId.startsWith("IMM-")
          ? profile.emersionId
          : user?.registrationNo || generateImmersionParticipantId_3(seq);

      const copyId = () => {
        if (typeof navigator !== "undefined") {
          navigator.clipboard.writeText(emersionId);
          setCopied(true);
          toast.success("Immersion ID copied to clipboard!");
      ...
      return (
        <div className="bg-card border-border/60 relative overflow-hidden rounded-2xl border shadow-lg backdrop-blur-md print:rounded-none print:border-0 print:shadow-none">
          {}
          ...
          <div className="relative flex flex-col items-start justify-between gap-4 px-5 pb-5 pt-3 sm:flex-row sm:items-end sm:px-6 sm:pb-6 print:px-0 print:pb-2 print:pt-4">
            <div className="z-10 flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row sm:items-end">
              {}
              <div className="border-card bg-muted z-10 -mt-12 flex size-24 shrink-0 items-center justify-center overflow-hidden rounded-full border-4 sm:-mt-16 sm:size-28 print:mt-0 print:size-20 print:border-2">
                {profile.photoUrl && !imgError ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-emerald-500/10 text-xl font-bold uppercase tracking-wide text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400">
                    {getInitials_2(profile.fullName)}
                  </div>
                )}
              </div>
            ...
            <div className="z-10 flex w-full shrink-0 items-center justify-center gap-2 sm:w-auto print:hidden">
              <Button
                variant="outline"
                onClick={handlePrint}
                className="border-border/80 hover:border-foreground/20 hover:bg-muted h-9 flex-1 gap-1.5 rounded-lg px-4 text-xs font-medium transition-all active:scale-[0.98] sm:flex-initial"
              >
                <Printer className="text-muted-foreground size-3.5" />
                Download PDF
              </Button>
              <Button
                id="btn-edit-emersion-profile"
                onClick={onEdit}
                className="bg-primary hover:bg-primary/95 text-primary-foreground h-9 flex-1 gap-1.5 rounded-lg px-4 text-xs font-medium transition-all active:scale-[0.98] sm:flex-initial"
              >
    ...
    function EmersionProfileContent_2() {
      const { user } = useAuth();
      ...
      const isLoading = isProfileLoading || isAppLoading;
      ...
      if (isError || !profileResponse?.success || !profileResponse.data) {
        return (
          <div className="bg-card/85 border-border/50 animate-in fade-in slide-in-from-bottom-6 duration-400 relative mx-auto mt-12 max-w-2xl overflow-hidden rounded-3xl border p-8 shadow-xl backdrop-blur-md">
            ...
            <div className="flex flex-col items-center space-y-6 text-center">
              <div className="bg-primary/10 text-primary border-primary/20 flex size-16 items-center justify-center rounded-2xl border shadow-sm">
                <UserCircle className="size-9 animate-pulse" />
              </div>
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground mx-auto max-w-md text-sm leading-relaxed">
                  Your immersion participant profile has not been created yet.
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  size="sm"
                  onClick={() => refetch()}
                  className="h-10 gap-1.5 rounded-xl px-4 font-semibold"
                >
                  <RefreshCw className="size-4" />
                  Retry
                </Button>
                <Link href={IMMERSION.APPLICATION}>
                  <Button className="h-10 gap-1.5 rounded-xl px-4 font-semibold">
                    Submit Application
                    <ArrowRight_2 className="size-4" />
                  </Button>
                </Link>
              </div>
            </div>
      ...
      let app: EmersionApplicationData | null = null;
      if (Array.isArray(applications) && applications.length > 0) {
        const generalApps = (applications as EmersionApplicationData[])
        ...
        app =
          generalApps[0] ??
          pickPrimaryImmersionApplication(
            applications as EmersionApplicationData[],
          );
      }

      const handleEditSubmission = () => {
        router.push("/immersion/registration?edit=true");
      };

      return (
        <div className="animate-in fade-in mx-auto max-w-7xl space-y-5 px-4 py-5 duration-300 md:px-6">
          {}
          <EmersionProfileHeader profile={profile} onEdit={handleEditSubmission} />

          {}
          <EmersionProfilePersonal profile={profile} email={user?.email} />

          {}
          <EmersionProfileAddress
            currentAddressLocal={profile.currentAddressLocal}
            currentAddressDistrict={profile.currentAddressDistrict}
            currentAddressState={profile.currentAddressState}
            currentAddressCountry={profile.currentAddressCountry}
            currentAddressPinCode={profile.currentAddressPinCode}
            permAddressLocal={profile.permAddressLocal}
            permAddressDistrict={profile.permAddressDistrict}
            permAddressState={profile.permAddressState}
            permAddressCountry={profile.permAddressCountry}
            permAddressPinCode={profile.permAddressPinCode}
            sameAsCurrentAddress={profile.sameAsCurrentAddress}
          />

          {}
          {app && <EmersionProfileApplication app={app} />}
          ...
          <div className="hidden items-center justify-between border-t-2 border-emerald-600 bg-white pb-1 pt-3 print:fixed print:bottom-0 print:left-0 print:right-0 print:flex">
            ...
            <p className="text-muted-foreground text-[10px]">
              System-generated participant profile.
            </p>
          </div>
        </div>
      );
  ...
  return (
    <main className="min-h-full">
      <h1 className="sr-only">Immersion Participant Profile</h1>
      <EmersionProfileContent_2 />
    </main>
  );
}
