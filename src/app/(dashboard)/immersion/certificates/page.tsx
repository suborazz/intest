...
interface EmersionCertificate_2 {
      id: string;
      certificateNo: string;
      studentId?: string;
      issuedAt: string;
...
function formatImmersionCode_3(
      code?: string | null,
...
interface EmersionCertificate_3 {
      id: string;
      certificateNo: string;
      studentId?: string;
      issuedAt: string;
...
function formatHumanReadableDate_3(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
interface ActionCellProps {
      certificate: EmersionCertificate_3;
...
export default function ImmersionCertificatesPage() {
    function useAuth() {
    ...
    const ActionCell: React_3.FC<ActionCellProps> = ({
      certificate,
      participantName,
    }) => {
    ...
    const getColumns = (
      participantName: string,
    ): ColumnDef<EmersionCertificate_3>[] => [
      {
        id: "slNo",
        header: "Sl No.",
        cell: ({ row }) => (
        ...
        cell: ({ row }) => (
          <div className="flex flex-col">
            <span className="text-foreground text-xs font-bold">
              {row.original.program?.title || "Immersion Program"}
            </span>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-xs">
            {row.original.program?.location || "Remote"}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-xs">
            {row.original.program?.period || "N/A"}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-xs font-medium">
            {formatHumanReadableDate_3(row.original.issuedAt)}
          </span>
        ),
      },
      {
    ...
    function CertificatesList_3() {
      ...
      const [meta, setMeta] = useState<TMeta>({ total: 0, limit, page });

      const { data, isLoading, isError } =
        EmersionDataHooks.useMyEmersionCertificates();

      const allCertificates = useMemo<EmersionCertificate_2[]>(
        () => data?.data ?? [],
        [data],
      );
      ...
      const filtered = useMemo(() => {
        ...
        return allCertificates.filter((c) => {
          const formattedId = c.program?.id
            ? formatImmersionCode_3(c.program.id, c.program.title)
            : "";
          return (
            c.certificateNo?.toLowerCase().includes(search) ||
            c.program?.id?.toLowerCase().includes(search) ||
            formattedId.toLowerCase().includes(search) ||
            c.program?.title?.toLowerCase().includes(search) ||
            c.program?.location?.toLowerCase().includes(search)
          );
        });
      ...
      const columns = useMemo(() => getColumns(participantName), [participantName]);
        ...
        <div className="space-y-6">
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-4">
              <Heading
                ...
                description="Your verified digital certificates from completed immersion programs."
              />
            </div>
          </PageHeaderLayout>
          ...
          {allCertificates.length === 0 && !isLoading ? (
            <NoDataFound_2
              ...
              description="Complete your immersion program to earn and download a verified digital certificate."
            />
              ...
              <DataTable
                ...
                toolbar={
                  <div className="flex w-full items-center gap-2">
                    <DataTableSearch
                      searchKey="filter"
                      placeholder="Search by certificate number or program..."
                      small
                    />
                  </div>
                }
