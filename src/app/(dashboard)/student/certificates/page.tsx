...
import { AlertTriangle, Award, Calendar, CreditCard, Download, ExternalLink, Eye, FolderOpen, GraduationCap as GraduationCap_2, Info, Loader2, Mail as Mail_2, MapPin as MapPin_2, MessageSquare, MoreHorizontal, Phone as Phone_2, Plus, Printer, School, Send, ShieldCheck, Trash2, User, AlertCircle as AlertCircle_2, Award as Award_2, CheckCircle2 as CheckCircle2_2, Clock, FileText as FileText_2, Globe, Mail, Search, XCircle as XCircle_2 } from "lucide-react";
...
function formatHumanReadableDate_8(date?: string | null): string {
      if (!date) return "N/A";
      const parsed = new Date(date);
      if (isNaN(parsed.getTime())) return "N/A";
      return parsed.toLocaleDateString("en-IN", {
...
interface ActionCellProps_2 {
      certificate: MyCertificate_5;
      studentName: string;
    }

function DropdownMenu_2({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Root>) {
      return <DropdownMenuPrimitive.Root data-slot="dropdown-menu" {...props} />;
    }

function DropdownMenuTrigger_2({
      ...props
    }: React_3.ComponentProps<typeof DropdownMenuPrimitive.Trigger>) {
      return (
        <DropdownMenuPrimitive.Trigger suppressHydrationWarning
...
export default function CertificatesPage() {
    ...
    const ActionCell_2: React_3.FC<ActionCellProps_2> = ({
      certificate,
      studentName,
    }) => {
      ...
      return (
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <DropdownMenu_2 open={menuOpen} onOpenChange={setMenuOpen} modal={false}>
            <DropdownMenuTrigger_2 asChild>
              <Button
                variant="ghost"
                size="icon"
    ...
    const getColumns_3 = (studentName: string): ColumnDef<MyCertificate_5>[] => [
      {
        id: "slNo",
        header: "Sl No.",
        cell: ({ row }) => (
        ...
        cell: ({ row }) => (
          <div className="flex max-w-[180px] flex-col">
            <span
              ...
              title={row.original.internship?.title || "N/A"}
            >
              {row.original.internship?.title || "N/A"}
            </span>
            {row.original.internship?.companyName && (
              <span
                className="text-muted-foreground truncate text-[10px]"
                title={row.original.internship.companyName}
              >
                {row.original.internship.companyName}
              </span>
            )}
      ...
      },
      {
        ...
        header: "Internship Location",
        cell: ({ row }) => {
          const mode = row.original.internship?.mode;
          const location = row.original.internship?.location;
          const isOnline = mode === "ONLINE";
          const display = isOnline ? "Online / Remote" : location || "N/A";
          return (
            <span
              className="text-foreground block max-w-[150px] truncate text-xs font-medium"
              title={display}
            >
              {display}
            </span>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const grade =
            row.original.grade || (row.original as any).certificate?.grade;
          console.log(
            "CELL GRADE VALUE RESOLVED:",
            grade,
            "from original:",
            row.original,
          );
          ...
          return (
            <span className="inline-flex w-fit items-center rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
              {grade}
            </span>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const credits =
            ...
            row.original.internship?.credits ||
            (row.original as any).certificate?.credits;
          console.log("CELL CREDITS VALUE RESOLVED:", credits);
          if (!credits) {
            return <span className="text-muted-foreground text-xs">—</span>;
          }
          return (
            <span className="inline-flex w-fit items-center rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
              {credits}
            </span>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className="text-muted-foreground block max-w-[100px] truncate text-xs"
            title={row.original.internship?.duration || "N/A"}
          >
            {row.original.internship?.duration || "N/A"}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-xs font-medium">
            {formatHumanReadableDate_8(row.original.issuedAt)}
          </span>
        ),
      },
      {
        ...
        cell: () => (
          <span
            className="text-foreground block max-w-[130px] truncate text-xs font-bold"
            title={studentName}
          >
            {studentName}
          </span>
      ...
      },
      {
        ...
        header: "Action",
        cell: ({ row }) => (
          <ActionCell_2 certificate={row.original} studentName={studentName} />
        ),
      },
    ];

    function CertificatesList_4() {
      ...
      const [meta, setMeta] = useState<TMeta_3>({ total: 0, limit, page });

      const { data, isLoading, isError } = StudentDataHook.useMyCertificates();

      const allCertificates = useMemo<MyCertificate_4[]>(
        () => data?.data ?? [],
        [data],
      );
      ...
      const filtered = useMemo(() => {
        ...
        return allCertificates.filter((c) => {
          const formattedId = c.internship?.id
            ? formatInternshipCode(c.internship.id, c.internship.companyName)
            : "";
          return (
            c.certificateNo?.toLowerCase().includes(search) ||
            c.internship?.id?.toLowerCase().includes(search) ||
            formattedId.toLowerCase().includes(search) ||
            c.internship?.title?.toLowerCase().includes(search) ||
            c.internship?.companyName?.toLowerCase().includes(search)
          );
        });
      ...
      useEffect(() => {
        setMeta((prev) => ({ ...prev, page, limit, total: filtered.length }));
      }, [filtered.length, page, limit]);

      const columns = useMemo(() => getColumns_3(studentName), [studentName]);
          ...
          <PageHeaderLayout>
            <div className="flex w-full flex-col gap-4">
              <Heading
                ...
                description="Your verified digital certificates from completed internships."
              />
            </div>
          </PageHeaderLayout>

          {allCertificates.length === 0 && !isLoading ? (
            <NoDataFound_2
              title="No Certificates Yet"
              description="Complete your enrolled internship programs to earn and download verified digital certificates."
              action={
                <Button asChild>
                  <Link href="/student/id-card">View ID Cards</Link>
                </Button>
              }
            />
            ...
            <NoDataFound_2
              ...
              description="No certificates match your search. Try a different search query."
            />
          ) : (
              ...
              <DataTable
                ...
                toolbar={
                  <div className="flex w-full items-center gap-2">
                    <DataTableSearch
                      searchKey="filter"
                      placeholder="Search by ID, certificate number or program..."
                      small
                    />
                  </div>
                }
