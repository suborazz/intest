...
type PaymentStatus_2 = "PENDING" | "COMPLETED" | "FAILED" | "REFUNDED";
...
interface PaymentColumnsProps_2 {
...
const PAYMENT_STATUS_FILTERS: TAlertFilter_4[] = [
...
interface ReceiptDialogProps_2 {
...
export default function PaymentsPage() {
    ...
    function PaymentStatusBadge_2({ status }: { status: PaymentStatus }) {
      switch (status) {
        case "COMPLETED":
          return (
            <Badge className="gap-1 rounded-full border border-emerald-500/20 bg-emerald-500/10 font-semibold text-emerald-600 hover:bg-emerald-500/15 dark:text-emerald-400">
    ...
    const getColumns_4 = ({
      onViewReceipt,
    }: PaymentColumnsProps_2): ColumnDef_2<any>[] => [
      {
        id: "details",
        ...
        cell: ({ row }) => (
          <div className="py-1">
            <div className="text-foreground font-semibold">
              {row.original.internship?.title || "Internship Enrollment"}
            </div>
      ...
      },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => <PaymentStatusBadge_2 status={row.original.status} />,
      },
      {
        id: "receipt",
    ...
    function ReceiptDialog_2({ paymentId, open, onClose }: ReceiptDialogProps_2) {
      const {
        data: response,
        isLoading,
        isError,
    ...
    function PaymentsTable_2() {
      const searchParams = useSearchParams();
      const page = Number(searchParams?.get("page")) || 1;
      const limit = Number(searchParams?.get("size")) || 10;
      const search = searchParams?.get("filter") || undefined;
      const status =
        (searchParams?.get("status")?.split(",")[0] as PaymentStatus_2) || undefined;

      const [receiptPaymentId, setReceiptPaymentId] = useState<string | null>(null);
      const [meta, setMeta] = useState<TMeta_4>({ total: 0, limit, page });

      const { data: paymentsData, isLoading } = StudentDataHook.useMyPayments({
        page,
        limit,
        search,
        status,
      ...
      const columns = useMemo(
        () => getColumns_4({ onViewReceipt: setReceiptPaymentId }),
        [],
      );

      const sidebarContent = useMemo(() => <PaymentsSidebarFilters />, []);
        ...
        <div className="space-y-6">
          <PageHeaderLayout>
            <Heading
              ...
              description="Monitor your transactions, receipts, and order statuses."
            />
          </PageHeaderLayout>
