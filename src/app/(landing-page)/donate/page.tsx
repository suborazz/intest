...
import { AlertCircle, Heart, Receipt, Search } from "lucide-react";
import React_2, { useState } from "react";
import { Loader2, QrCode, Heart as Heart_2 } from "lucide-react";
...
const ZDonateForm = z_2
  .object({
    ...
    amount: z_2.string().refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
      message: "Please enter a valid amount greater than 0.",
    }),
    anythingElse: z_2.string().optional(),
  })
  .refine(
    (data) => {
      if (data.certificate80G === "yes" && (!data.panNumber || data.panNumber.trim() === "")) {
        return false;
      }
      return true;
    },
    {
      message: "PAN Card number is mandatory for issuing an 80G tax certificate.",
      path: ["panNumber"],
    }
  );
...
interface PublicDonationRecord {
    id: string;
    donorName: string;
    amount: number;
    address?: string;
...
export default function DonatePage() {
    ...
    type TDonateForm = z_2.infer<typeof ZDonateForm>;

    const ZDonateForm = z_3
      .object({
        name: z_3.string().min(2, { message: "Name must be at least 2 characters." }),
        mobile: z_3.string().regex(/^[0-9]{10}$/, {
          message: "Mobile number must be exactly 10 digits.",
        }),
        email: z_3.string().email({ message: "Please enter a valid email address." }),
        certificate80G: z_3.enum(["yes", "no"], {
          message: "Please select if you want an 80G certificate.",
        }),
        panNumber: z_3
          .string()
        ...
        address: z_3
          .string()
          .min(5, { message: "Address must be at least 5 characters." }),
        amount: z_3.string().refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
          message: "Please enter a valid amount greater than 0.",
        }),
        anythingElse: z_3.string().optional(),
      })
      .refine(
        (data) => {
          if (data.certificate80G === "yes" && (!data.panNumber || data.panNumber.trim() === "")) {
  ...
  const { data: rawRecords = [], isLoading } =
    StudentDataHook.usePublicDonations();
  const records = React_2.useMemo(() => {
    return ((rawRecords as unknown as PublicDonationRecord[]) || []).map(
      (r) => ({
        ...r,
        paymentNumber: r.razorpayPaymentId || r.paymentNumber || "N/A",
        date: r.createdAt || r.date,
        otherInfo: r.notes || r.otherInfo || "N/A",
      }),
    );
  }, [rawRecords]);
  ...
  const filteredRecords = (records || []).filter((record) => {
    ...
    const nameMatch = record.donorName?.toLowerCase().includes(query);
    const txnMatch = record.paymentNumber?.toLowerCase().includes(query);
    const addressMatch = record.address?.toLowerCase().includes(query);
    return nameMatch || txnMatch || addressMatch;
  });
    const [isProcessing, setIsProcessing] = useState(false);
    const form = useForm<TDonateForm>({
            resolver: zodResolver(ZDonateForm),
            defaultValues: {
              name: "",
              mobile: "",
              email: "",
              certificate80G: "" as "yes" | "no",
              panNumber: "",
              address: "",
              amount: "",
              anythingElse: "",
    ...
    async function onSubmit(values: TDonateForm) {
            try {
              setIsProcessing(true);

                    const response = await axios.post("/api/v1/donations/create-order", {
                amount: parseFloat(values.amount),
                donorName: values.name,
                email: values.email,
                mobile: values.mobile,
                wants80G: values.certificate80G === "yes",
                panNumber: values.panNumber || undefined,
                address: values.address,
                notes: values.anythingElse || undefined,
              });
              ...
              const orderData = response.data.data;
                ...
                toast.info("Mock payment gateway detected. Simulating payment...");
                const verifyRes = await axios.post("/api/v1/donations/verify", {
                ...
                if (verifyRes.data.success) {
                  toast.success("Thank you! Your donation was successful (Mock).");
                  form.reset();
              ...
              if (!loaded) {
                ...
                currency: "INR",
                name: "IIInternship",
                description: "Voluntary Donation / Contribution",
  ...
  return (
    <main className="bg-background relative flex min-h-screen flex-col overflow-hidden">
                    ...
                    <div className="relative mx-auto mt-12 grid w-[min(1300px,calc(100%-56px))] grid-cols-1 items-center gap-8 text-center md:mt-20 lg:grid-cols-2 lg:gap-0 lg:text-left">
                      ...
                      <div className="relative z-30 mx-auto w-full max-w-[560px] lg:mx-0">
                        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-white/90 backdrop-blur-md">
                          <Heart_2 className="h-4 w-4 fill-emerald-300 text-emerald-300" />
                          Support Our Mission
                        </div>
                        ...
                        <p className="text-primary-foreground/85 mx-auto mt-[12px] max-w-[500px] text-left text-[0.9rem] leading-[1.6] sm:text-[1.1rem] lg:mx-0">
                          Your contribution helps us provide free internships and vital
                          educational resources to underprivileged students. Together, we can
                      ...
                      <div className="relative mt-[20px] flex justify-center lg:mt-0 lg:translate-y-[20px] lg:justify-end">
                        <img
                          src="/images/hero-donation-v2.jpg" 
                          alt="Donate"
      ...
      {}
      <section className="relative z-10 mx-auto -mt-16 mb-24 w-full max-w-[1300px] space-y-8 px-4 sm:px-6 md:-mt-32 lg:px-8">
        {}
        <div className="flex w-full justify-center">
          <div className="grid w-full grid-cols-2 rounded-2xl border border-black/10 bg-black/5 p-1 shadow-inner backdrop-blur-md sm:flex sm:w-auto dark:border-white/10 dark:bg-white/5">
            <button
              onClick={() => setActiveTab("form")}
              className={`group flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-xl px-3 py-2 text-xs font-bold transition-all duration-300 sm:px-5 sm:text-sm ${
                activeTab === "form"
            ...
            >
              <span
                className={`flex items-center justify-center rounded-lg p-1.5 transition-all duration-300 ${
                  activeTab === "form"
                    ? "bg-white/20 text-white"
                    : "text-muted-foreground bg-black/5 group-hover:bg-black/10 dark:bg-white/5 dark:group-hover:bg-white/10"
                }`}
              >
        ...
        <div className="pt-4 transition-all duration-300">
          {activeTab === "form" ? (
            <div className="grid grid-cols-1 items-stretch gap-10 lg:grid-cols-12 lg:gap-16">
              <div className="lg:col-span-7">
                <div className="flex h-full flex-col gap-8 md:mt-14">
                                        {}
                                        <div>
                                          <h2 className="bg-linear-to-r from-primary mb-4 to-emerald-600 bg-clip-text text-3xl font-extrabold tracking-tight text-transparent md:text-5xl">
                                            The Power of Giving
                                          </h2>
                                          <p className="text-muted-foreground text-base font-medium leading-relaxed">
                                            Your contribution is more than support—it is an investment in human
                                            values, moral development, and society&apos;s spiritual progress.
                                          </p>
                                        </div>
                                        ...
                                        <article className="text-foreground space-y-8">
                                          ...
                                          <div className="space-y-4">
                                            <h3 className="text-foreground relative pl-3 text-xl font-bold tracking-tight before:absolute before:bottom-1.5 before:left-0 before:top-1.5 before:w-1 before:rounded-full before:bg-amber-500 md:text-2xl">
                                              Glory and Inspiration of Donation
                                            </h3>
                                            <p className="text-muted-foreground text-base leading-relaxed">
                                              Donating not only paves the way for our spiritual progress, but it
                                              also strengthens moral, human values, and goodwill in society. The
                                              effect of donation is not limited to this birth only, but its
                                              auspicious results also benefit us in the next births, thereby
                                              providing higher speed.
                                            </p>
                                            <p className="text-muted-foreground text-base leading-relaxed">
                                              We all are bound by the bondage of karma in our lives, and with the
                                              bondage of karma, we leave this world and then take birth again. In
                                              such a situation, if we do good deeds by donating according to our
                                              capacity and contribute to the work that benefits the public and the
                                              service dedicated to society, then our name can remain immortal for
                                              ages.
                                            </p>
                                          </div>
                                          ...
                                          {}
                                          <div className="space-y-4">
                                            <h3 className="text-foreground relative pl-3 text-xl font-bold tracking-tight before:absolute before:bottom-1.5 before:left-0 before:top-1.5 before:w-1 before:rounded-full before:bg-indigo-500 md:text-2xl">
                                              Request to Rich and Virtuous Gentlemen
                                            </h3>
                                            <p className="text-muted-foreground text-base leading-relaxed">
                                              Religious-loving rich people, philanthropists, and wealthy
                                              capitalists are appealed to contribute to this sacred cause. Donate
                                              generously for the construction of a grand building of Unique
                                              Records of Universe unit under the banner of{" "}
                                              <strong className="text-foreground font-semibold">
                                                &quot;Divya Prerak Kahaniya Humanity Research Centre Trust&quot;
                                              </strong>
                                              , so that human values can be promoted and the coming generation can
                                          ...
                                          {}
                                          <div className="space-y-4">
                                            <h3 className="text-foreground relative pl-3 text-xl font-bold tracking-tight before:absolute before:bottom-1.5 before:left-0 before:top-1.5 before:w-1 before:rounded-full before:bg-emerald-500 md:text-2xl">
                                            ...
                                            <p className="text-muted-foreground text-base leading-relaxed">
                                              ...
                                              capacity. This donation will be{" "}
                                              <span className="rounded-md bg-emerald-500/10 px-2 py-0.5 font-semibold text-emerald-600 dark:text-emerald-400">
                                                Tax-free under Section 80G
                                              </span>{" "}
                                              of the Income Tax Act, Government of India.
                                            </p>
                                          </div>

                                          {}
                                          <div className="space-y-4">
                                            <h3 className="text-foreground relative pl-3 text-xl font-bold tracking-tight before:absolute before:bottom-1.5 before:left-0 before:top-1.5 before:w-1 before:rounded-full before:bg-rose-500 md:text-2xl">
                                              Donate, Earn Merit
                                            </h3>
                                            <p className="text-muted-foreground text-base leading-relaxed">
                                              Donation makes the life of any person successful, purifies the soul,
                                              and brings happiness, prosperity, progress, and peace in your life.
                                              Come forward and become a part of this noble work by making a
                                              voluntary donation as per your capacity and serve Humanity.
                                            </p>
                                          </div>
                                        </article>
                                      </div>
              ...
              <div className="lg:col-span-5">
                <div className="relative w-full overflow-hidden rounded-3xl border border-white/60 bg-white/80 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)] backdrop-blur-2xl sm:p-8 md:mt-4 lg:p-10 dark:border-white/10 dark:bg-black/60">
                                        ...
                                        <div className="relative z-10 flex h-full flex-col">
                                          <div className="mb-8 flex items-center justify-between text-center sm:text-left">
                                            <div>
                                            ...
                                            <div className="hidden h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-100 sm:flex dark:bg-emerald-900/40">
                                          ...
                                          <Form {...form}>
                                            ...
                                            >
                                              ...
                                              <FormField
                                                ...
                                                render={({ field }) => (
                                                  <FormItem>
                                                    <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                    ...
                                                    <FormControl>
                                                      <Input
                                                        ...
                                                        className="focus:ring-primary text-foreground rounded-xl border border-black/10 bg-white/50 px-4 py-6 text-xl font-bold outline-none transition-all focus:border-transparent focus:ring-2 dark:border-white/10 dark:bg-black/50"
                                              ...
                                              <FormField
                                                ...
                                                render={({ field }) => (
                                                  <FormItem>
                                                    <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                      1. Name <span className="text-destructive">*</span>
                                                    </FormLabel>
                                                    <FormControl>
                                                      <Input
                                                        placeholder="Enter Name..."
                                                        className="rounded-xl border border-black/10 bg-white/50 px-4 py-5 dark:border-white/10 dark:bg-black/50"
                                                        disabled={isProcessing}
                                                        {...field}
                                                      />
                                                    </FormControl>
                                              ...
                                              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                                ...
                                                <FormField
                                                  ...
                                                  render={({ field }) => (
                                                    <FormItem>
                                                      <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                        3. Email <span className="text-destructive">*</span>
                                                      </FormLabel>
                                                      <FormControl>
                                                        <Input
                                                          type="email"
                                              ...
                                              <FormField
                                                ...
                                                render={({ field }) => (
                                                  <FormItem className="space-y-2">
                                                    <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                      4. Do you want an 80G certificate?{" "}
                                                      <span className="text-destructive">*</span>
                                                    </FormLabel>
                                                    <Select
                                                    ...
                                                    >
                                                      <FormControl>
                                                        <SelectTrigger className="text-foreground focus:ring-primary flex h-[50px] items-center rounded-xl border border-black/10 bg-white/50 px-4 py-6 text-sm font-medium shadow-none dark:border-white/10 dark:bg-black/50">
                                              ...
                                              {form.watch("certificate80G") === "yes" && (
                                                <FormField
                                                  ...
                                                  render={({ field }) => (
                                                    <FormItem>
                                                      <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                        4a. PAN Card Number <span className="text-destructive">*</span>
                                                      </FormLabel>
                                                      <FormControl>
                                                        <Input
                                                          placeholder="Enter PAN (e.g. ABCDE1234F)..."
                                                          className="rounded-xl border border-black/10 bg-white/50 px-4 py-5 dark:border-white/10 dark:bg-black/50 uppercase"
                                                          disabled={isProcessing}
                                                          {...field}
                                              ...
                                              <FormField
                                                ...
                                                render={({ field }) => (
                                                  <FormItem>
                                                    <FormLabel className="text-foreground ml-1 text-sm font-semibold">
                                                      5. Address <span className="text-destructive">*</span>
                                                    </FormLabel>
                                                    <FormControl>
                                                      <Textarea
                                                        placeholder="Enter Address..."
                                                        className="min-h-[80px] w-full rounded-xl border border-black/10 bg-white/50 px-4 py-3 text-sm dark:border-white/10 dark:bg-black/50"
                                                        disabled={isProcessing}
                                                        {...field}
                                                      />
                                                    </FormControl>
                                                  ...
                                                  <FormItem>
                                                    ...
                                                    <FormControl>
                                                      <Textarea
                                                        ...
                                                        className="min-h-[60px] w-full rounded-xl border border-black/10 bg-white/50 px-4 py-3 text-sm dark:border-white/10 dark:bg-black/50"
                                                        disabled={isProcessing}
                                                        {...field}
                                                      />
                                                    </FormControl>
                                              ...
                                              <div className="mt-4 pt-2">
                                                <button
                                                  type="submit"
                                                  disabled={isProcessing}
                                                  className="bg-linear-to-r from-primary hover:from-primary/90 text-primary-foreground group relative flex h-12 w-full cursor-pointer items-center justify-center overflow-hidden rounded-xl border-0 to-emerald-500 text-[16px] font-bold shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:to-emerald-500/90 hover:shadow-[0_8px_20px_rgba(16,185,129,0.3)] lg:h-[54px] lg:text-[18px] disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                  ...
                                                  <span className="bg-background/20 group-hover:bg-background group-hover:text-primary absolute right-2 z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-white shadow-sm transition-colors duration-300">
                                                    <Heart className="group-hover:fill-primary h-5 w-5 transition-transform duration-300 group-hover:scale-110" />
                                                  </span>
                                                </button>

                                                <div className="border-border/50 mt-6 flex flex-col items-center justify-center gap-2 border-t pt-4">
                                                  <p className="text-muted-foreground text-xs font-semibold uppercase tracking-wider">
                                                    Or Scan to Pay
                                                  </p>
                                                  <div className="flex items-center gap-2 rounded-lg border border-black/5 bg-white/50 px-4 py-2 dark:border-white/5 dark:bg-black/50">
                                                    <QrCode className="text-primary h-5 w-5" />
                                                    <span className="text-sm font-medium">
                                                      UPI / QR Payment available on next step
                                                    </span>
                                                  </div>
                                                </div>
                                              </div>
              ...
              <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 text-sm">
                    Transparency matters. View all recent voluntary donations
                    and contributions.
                  </p>
                </div>
                ...
                <div className="relative w-full md:max-w-md">
                  ...
                  <input
                    ...
                    className="focus:ring-primary placeholder:text-muted-foreground text-foreground w-full rounded-xl border border-black/10 bg-white/50 py-2.5 pl-10 pr-4 text-sm transition-all focus:border-transparent focus:outline-none focus:ring-2 dark:border-white/10 dark:bg-black/50"
                  />
                </div>
              </div>

              {}
              <div className="w-full overflow-x-auto rounded-2xl border border-white/20 bg-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.08)] backdrop-blur-xl dark:border-white/10 dark:bg-black/40">
                {isLoading ? (
                  ...
                  <table className="w-full min-w-[900px] border-collapse text-left">
                    <thead>
                      <tr className="text-muted-foreground border-b border-black/10 bg-black/5 text-sm font-bold dark:border-white/10 dark:bg-white/5">
                        ...
                        <th className="whitespace-nowrap px-5 py-4">Address</th>
                        <th className="whitespace-nowrap px-5 py-4">
                          Amount Received
                        </th>
                        <th className="whitespace-nowrap px-5 py-4">
                    ...
                    <tbody className="divide-y divide-black/5 text-sm dark:divide-white/5">
                      {filteredRecords.length > 0 ? (
                        filteredRecords.map((record, index) => (
                          ...
                          >
                            <td className="text-muted-foreground/80 px-5 py-4 text-center font-bold">
                              {index + 1}
                            </td>
                            <td className="px-5 py-4 font-mono text-sm">
                              ...
                              record.paymentNumber === "N/A" ? (
                                ...
                                <span className="bg-primary/10 text-primary rounded-md px-2.5 py-1 font-semibold">
                                  {record.paymentNumber}
                                </span>
                              )}
                            </td>
                            <td className="text-muted-foreground/90 px-5 py-4 font-medium">
                              {record.date &&
                            ...
                            <td className="text-muted-foreground/80 max-w-[250px] break-words px-5 py-4">
                              {record.address}
                            </td>
                            <td className="px-5 py-4 text-sm font-bold text-emerald-600 dark:text-emerald-400">
                              ₹{record.amount.toLocaleString()}
                            </td>
                            <td className="text-muted-foreground/90 px-5 py-4 font-medium italic">
                              {record.otherInfo || "N/A"}
                            </td>
                          </tr>
                        ))
                      ) : (
