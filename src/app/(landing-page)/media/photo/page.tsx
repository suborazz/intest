...
export default function PhotoPage() {
    ...
    function PhotoGallery() {
      ...
      const photos = useMemo(() => {
        return ((rawPhotos as unknown as PublicPhoto[]) || []).map((p) => ({
          ...p,
      ...
      return (
        <section className="w-full">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-foreground text-lg font-bold">Latest Photos</h2>
          ...
          {photos.length === 0 ? (
            <div className="text-muted-foreground py-12 text-center font-semibold">
              No photo gallery assets available.
            </div>
          ) : (
            <div
              className={
              ...
              {photos.map((photo, index) => (
                ...
                >
                  <div
                    ...
                    className={`border-border/20 relative shrink-0 overflow-hidden rounded-2xl border shadow-sm sm:rounded-3xl cursor-pointer ${viewMode === "grid" ? "aspect-[4/3] w-full" : "min-h-[100px] w-28 sm:min-h-[160px] sm:w-64 lg:w-72"}`}
                  >
                    <img
                      src={photo.imageUrl}
                      alt={photo.title}
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300 flex items-center justify-center">
                      <span className="text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/50 px-3 py-1.5 rounded-full text-xs font-medium">
                        View Full Size
                      </span>
                    </div>
                  </div>
          ...
          {selectedPhoto && (
            ...
            >
              <button
                className="absolute top-6 right-6 text-white/80 hover:text-white transition-colors bg-white/10 hover:bg-white/20 p-2 rounded-full backdrop-blur-md focus:outline-none"
                onClick={() => setSelectedPhoto(null)}
              >
                <svg
                  className="h-6 w-6"
              ...
              <div className="relative max-h-[90vh] max-w-[95vw] overflow-hidden rounded-xl bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <img
                  src={selectedPhoto}
                  alt="Full size"
                    ...
                    <div className="relative mx-auto grid w-[min(1300px,calc(100%-56px))] grid-cols-1 items-center gap-8 text-center lg:grid-cols-2 lg:gap-0 lg:text-left">
                      ...
                      <div className="relative z-30 mx-auto w-full max-w-[560px] lg:mx-0">
                        ...
                        <p className="text-primary-foreground/85 mx-auto mt-[12px] max-w-[500px] text-left text-[0.9rem] leading-[1.6] sm:text-[1.1rem] lg:mx-0">
                          A visual journey through our events, campus drives, and vibrant
                          community.
                        </p>
                      </div>
