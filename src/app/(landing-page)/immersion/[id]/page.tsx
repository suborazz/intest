...
export default function ImmersionDetailPage() {
  ...
  const { data: res, isLoading } = StudentDataHook.usePublicImmersions();
  ...
  const rawPrograms = (res as any) || [];
  const program = rawPrograms.find((p: any) => p.id === id);
      ...
      <main className="bg-background flex min-h-screen items-center justify-center px-4 pb-24 pt-32 sm:px-6 lg:px-8">
        <div className="max-w-md rounded-2xl border border-white/60 bg-white/50 p-8 text-center shadow-xl backdrop-blur-md dark:border-white/20 dark:bg-black/40">
          ...
          <p className="text-muted-foreground mb-6 text-sm">
            The immersion program you are looking for might have expired, been
  ...
  const formattedFees =
    program.fees === 0 ? "Free" : `₹${(program.fees ?? 0).toLocaleString()}`;

  return (
    <main className="bg-background relative min-h-screen pb-24 pt-[20px]">
      {}
      <div className="pointer-events-none absolute left-[-10%] top-40 h-[400px] w-[400px] rounded-full bg-emerald-400 opacity-10 mix-blend-multiply blur-[150px] filter"></div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex min-w-[200px] flex-col gap-3 sm:flex-row md:flex-col">
              ...
              <button
                onClick={() => {
                  ...
                  const instructorText = program.instructor?.name
                    ? `\n👤 Instructor: ${program.instructor.name}`
                    : "";
                  const shareText = `Check out this Immersion Program: "${program.title}"\n📍 Location: ${program.location}\n⏳ Duration: ${program.period}${instructorText}\n\nApply Link: ${url}`;
                  if (navigator.share) {
                    navigator
                      .share({
                        title: program.title,
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1">
            <div className="space-y-6 rounded-2xl border border-white/60 bg-white/50 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)] backdrop-blur-xl dark:border-white/20 dark:bg-black/40">
              <h3 className="text-foreground border-border/50 border-b pb-2 text-lg font-bold">
                Specifications
              </h3>

              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-200/50 py-2 dark:border-zinc-800/50">
                  <span className="text-muted-foreground text-sm">
                    Program Fees
                  </span>
                  <span className="text-foreground text-base font-extrabold text-amber-600 dark:text-amber-500">
                    {formattedFees}
                  </span>
                </div>
              ...
              <div className="pt-2">
                <Link
                  href="/sign-up?role=IMMERSION_USER"
                  className="bg-linear-to-r from-primary hover:from-primary/95 text-primary-foreground flex h-11 w-full items-center justify-center rounded-xl to-emerald-500 text-sm font-bold shadow-md transition-all"
                >
                  Express Interest
                </Link>
              </div>
