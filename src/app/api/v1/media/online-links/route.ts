...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const where: Prisma.MediaOnlineLinkWhereInput = {
      deletedAt: null,
    };

    const [links, total] = await Promise.all([
      prisma.mediaOnlineLink.findMany({
        where,
        orderBy: { slNo: "asc" },
        skip,
        take: limit,
      }),
      prisma.mediaOnlineLink.count({ where }),
    ]);

    const meta = buildPaginationMeta(total, page, limit);

    return successResponse(links, { meta });
  } catch (error) {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = createMediaOnlineLinkSchema.safeParse(body);
    ...
    const link = await prisma.mediaOnlineLink.create({
      data: parsed.data,
    });

    return successResponse(link, {
      status: HTTP_2.CREATED,
      message: "Online link added successfully.",
    });
  } catch (error) {
    return handleError(error);
  }
