...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const where: Prisma.MediaPhotoWhereInput = {
      deletedAt: null,
    };
    ...
    return successResponse(photos, { meta });
  } catch (error) {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = createMediaPhotoSchema.safeParse({
      title,
      date,
      description,
    });
    ...
    if (!file || file.size === 0) {
      return errorResponse("VALIDATION_ERROR", "Photo image file is required", {
        status: HTTP_2.UNPROCESSABLE,
        details: { photo: ["Photo file is required"] },
      });
    }
