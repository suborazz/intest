...
export async function GET(
  ...
  { params }: RouteContext,
    ...
    function generateImmersionParticipantId(sequence: number): string {
  ...
  try {
    ...
    const idCard = await prisma.immersionIdCard.findFirst({
      where: {
        OR: [{ id }, { applicationId: id }, { cardNo: id }],
      },
      include: {
        application: {
          include: {
    ...
    const origin = `${proto}://${host}`;
        ...
        const rawStudentId =
      idCard.application.user.registrationNo || idCard.studentId;
    const numPart = rawStudentId.replace(/\D/g, "");
    const seq = parseInt(numPart.slice(-5), 10) || 0;
    const formattedParticipantId = rawStudentId.startsWith("IMM-")
      ? rawStudentId
      : generateImmersionParticipantId(seq);

    const pdfPayload = {
      cardNo: idCard.cardNo,
      issuedAt: idCard.issuedAt.toISOString(),
      studentId: formattedParticipantId,
      studentName:
        profile?.fullName || idCard.application.user.name || "Participant",
      studentEmail: idCard.application.user.email,
      studentMobile: profile?.mobileNumber || "N/A",
      studentAddress: address,
      photoUrl: profile?.passportPhotoUrl || "",
      internshipId: idCard.application.immersion?.id || idCard.applicationId,
      internshipTitle:
        idCard.application.immersion?.title || "Immersion Program",
      internshipLocation: idCard.application.immersion?.location || "Remote",
      origin,
    };
    ...
    return new NextResponse(new Uint8Array(fileBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="immersion_id_card_${idCard.cardNo}.pdf"`,
      },
    });
  } catch (error) {
