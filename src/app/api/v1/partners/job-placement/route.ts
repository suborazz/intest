...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const where: Prisma.JobPlacementCompanyWhereInput = {};
    ...
    const [companies, total] = await Promise.all([
      prisma.jobPlacementCompany.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take: limit,
      }),
      prisma.jobPlacementCompany.count({ where }),
    ]);

    const meta = buildPaginationMeta(total, page, limit);

    return successResponse(companies, { meta });
  } catch (error) {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (skillsRequiredRaw) {
        try {
          skillsRequired = JSON.parse(skillsRequiredRaw as string);
        } catch {
          skillsRequired = (skillsRequiredRaw as string)
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
        }
      ...
      body = {
        ...
        postOpportunity: formData.get("postOpportunity") as string,
        jobNature: formData.get("jobNature") as string,
        fieldOfEmployment: formData.get("fieldOfEmployment") as string,
        minQualification: formData.get("minQualification") as string,
        skillsRequired,
        staffStrength: formData.get("staffStrength") as string,
    ...
    const result = createJobPlacementCompanySchema.safeParse(body);
    ...
    const company = await prisma.jobPlacementCompany.create({
      data: {
        name: result.data.name,
        address: result.data.address,
        postOpportunity: result.data.postOpportunity,
        jobNature: result.data.jobNature,
        fieldOfEmployment: result.data.fieldOfEmployment,
        minQualification: result.data.minQualification,
        skillsRequired: result.data.skillsRequired,
        staffStrength: result.data.staffStrength,
    ...
    return successResponse(company, {
      message: "Job Placement Company added successfully.",
