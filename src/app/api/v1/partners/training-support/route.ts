...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const where: Prisma.TrainingSupportWhereInput = {};

    if (search) {
      where.OR = [
        { name: { contains: search, mode: "insensitive" } },
        { address: { contains: search, mode: "insensitive" } },
        { serviceProvide: { contains: search, mode: "insensitive" } },
      ];
    }

    const [partners, total] = await Promise.all([
      prisma.trainingSupport.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take: limit,
      }),
      prisma.trainingSupport.count({ where }),
    ]);

    const meta = buildPaginationMeta(total, page, limit);

    return successResponse(partners, { meta });
  } catch (error) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      body = {
        ...
        serviceProvide: formData.get("serviceProvide") as string,
    ...
    const result = createTrainingSupportSchema.safeParse(body);
    ...
    const partner = await prisma.trainingSupport.create({
      data: {
        name: result.data.name,
        address: result.data.address,
        serviceProvide: result.data.serviceProvide,
    ...
    return successResponse(partner, {
      message: "Training Support Partner added successfully.",
