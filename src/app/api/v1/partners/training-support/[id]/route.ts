...
const createTrainingSupportSchema = z.object({
  name: z.string().min(2, "Name is required").trim(),
  address: z.string().min(5, "Address is required").trim(),
  serviceProvide: z.string().min(2, "Provided service is required").trim(),
...
const updateTrainingSupportSchema =
  createTrainingSupportSchema.partial();
...
export async function GET(
  ...
  try {
    ...
    const partner = await prisma.trainingSupport.findUnique({
      where: { id },
    });

    if (!partner) {
      return errorResponse("NOT_FOUND", "Training Support Partner not found.", {
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.trainingSupport.findUnique({
      where: { id },
    });
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const serviceProvide = formData.get("serviceProvide");
      if (serviceProvide !== null)
        body.serviceProvide = serviceProvide as string;
    ...
    const result = updateTrainingSupportSchema.safeParse(body);
    ...
    const updateData: {
      ...
      ...result.data,
      serviceProvide: result.data.serviceProvide ?? undefined,
    ...
    const updated = await prisma.trainingSupport.update({
      where: { id },
      data: updateData,
    });

    return successResponse(updated, {
      message: "Training Support Partner updated successfully.",
    });
  } catch (error) {
    return handleError(error);
  }
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.trainingSupport.delete({
      where: { id },
    });
    ...
    return successResponse(
      { id },
      {
        message: "Training Support Partner deleted successfully.",
      },
    );
  } catch (error) {
    return handleError(error);
