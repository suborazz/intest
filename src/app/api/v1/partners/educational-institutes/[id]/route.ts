...
const updateEducationalInstituteSchema =
  createEducationalInstituteSchema.partial();
...
export async function GET(
  ...
  try {
    ...
    const institute = await prisma.educationalInstitute.findUnique({
      where: { id },
    });

    if (!institute) {
      return errorResponse("NOT_FOUND", "Educational Institute not found.", {
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.educationalInstitute.findUnique({
      where: { id },
    });
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const naacAccreditation = formData.get("naacAccreditation");
      if (naacAccreditation !== null)
        body.naacAccreditation = naacAccreditation as string;
    ...
    const result = updateEducationalInstituteSchema.safeParse(body);
    ...
    const updateData: {
      ...
      ...result.data,
      naacAccreditation: result.data.naacAccreditation ?? undefined,
    ...
    const updated = await prisma.educationalInstitute.update({
      where: { id },
      data: updateData,
    });
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.educationalInstitute.delete({
      where: { id },
    });
