  ...
  try {
    ...
    let isValid = false;
      ...
      try {
        ...
        if (
          ...
          isValid = true;
          console.log(
            `[RAZORPAY API] Payment verified via direct API lookup for order ${razorpayOrderId}, payment ${razorpayPaymentId}`
          );
        }
      } catch (err) {
        console.error(
          `[RAZORPAY API ERROR] Failed to fetch payment status from Razorpay:`,
          err
        );
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      return { updatedPayment, enrollment };
        ...
        await issueIdCard(payment.userId, result.enrollment.id);
      ...
      console.error("[EMAIL ERROR] Failed to send payment confirmation:", err),
    );
