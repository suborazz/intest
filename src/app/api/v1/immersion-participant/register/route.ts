...
`;
  ...
  `;
    ...
    function getImmersionWelcomeEmailTemplate(name: string): string {
    ...
    async function sendImmersionWelcomeEmail(
      email: string,
      name: string,
    ): Promise<boolean> {
    ...
    const immersionParticipantRegisterSchema = z.object({
  ...
  try {
    ...
    const parsed = immersionParticipantRegisterSchema.safeParse(body);

    if (!parsed.success) {
      return errorResponse("VALIDATION_ERROR", "Invalid registration data.", {
    ...
    const user = await prisma.$transaction(async (tx) => {
      const count = await tx.user.count({
        where: {
          role: "IMMERSION_USER",
          registrationNo: { startsWith: `IMM-${currentYear}-` },
        },
      });

      const nextSeq = count + 1;
      const formattedSeq = String(nextSeq).padStart(5, "0");
      const registrationNo = `IMM-${currentYear}-${formattedSeq}`;
    ...
    const tokens = await generateTokens(user.id, user.email, user.role);
        ...
        sendImmersionWelcomeEmail(user.email, user.name || "Participant").catch(
      (err) => {
        console.error(
          "[Immersion Register] Failed to send welcome email:",
          err,
        );
      },
    );
    ...
    return successResponse(
      { user, tokens },
      {
        message: "Immersion Participant account created successfully.",
