...
export async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    const refundEmersionPaymentSchema = z.object({
  ...
  try {
    ...
    const parsed = refundEmersionPaymentSchema.safeParse(body);
    ...
    const refundResult = await refundRazorpayPayment(
      ...
      await tx.immersionPayment.update({
        where: { id: payment.id },
        data: {
          status: "REFUNDED",
    ...
    return successResponse(
      ...
      {
        message: `Successfully processed refund of ${refundAmount} INR.`,
      },
    );
  } catch (error) {
