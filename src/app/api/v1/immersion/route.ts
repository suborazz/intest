...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (type) {
      ...
      if (types.includes("FREE") && !types.includes("PAID")) {
        where.fees = { lte: 0 };
      } else if (types.includes("PAID") && !types.includes("FREE")) {
        where.fees = { gt: 0 };
      }
    }
    ...
    return successResponse(immersions);
  } catch (error) {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const formattedCode = formatImmersionCode(immersion.id, immersion.title);
    const updatedImmersion = await prisma.immersion.update({
    ...
    return successResponse(updatedImmersion, {
      message: "Immersion program posted successfully.",
