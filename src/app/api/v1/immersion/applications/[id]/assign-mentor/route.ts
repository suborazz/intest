...
export async function PATCH(
  ...
  { params }: RouteContext,
    ...
    const assignMentorSchema = z.object({
      mentorId: z.string().optional().nullable().or(z.literal("")),
    });
  ...
  try {
    ...
    if (!parsed.success) {
      return errorResponse(
        "VALIDATION_ERROR",
        "Invalid mentor assignment data.",
        {
    ...
    if (!mentorId) {
      ...
      where: { id: mentorId, role: "INSTRUCTOR" },
      select: { id: true, name: true, email: true },
    });
    if (!mentor) {
      return errorResponse(
        "NOT_FOUND",
        "Mentor not found or is not an Instructor.",
        { status: HTTP_2.NOT_FOUND },
      );
    }
    ...
    return successResponse(updated, {
      message: `Mentor "${mentor.name ?? mentor.email}" assigned successfully.`,
    });
  } catch (error) {
