...
export async function GET(): Promise<NextResponse> {
  try {
    const categories = await prisma.immersionCategory.findMany({
      orderBy: { name: "asc" },
    });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { name } = parsed.data;
        ...
        const existing = await prisma.immersionCategory.findUnique({
      where: { name },
    });
    if (existing) {
      return errorResponse(
    ...
    const category = await prisma.immersionCategory.create({
      data: { name },
    });
