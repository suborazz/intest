  ...
  try {
    ...
    const updated = await prisma.internship.update({
      ...
      data: {
        isApproved: false,
        isActive: false,
        deletedAt: isInstructor ? null : new Date(),
      },
    });
      ...
      try {
        ...
        if (user) {
          console.log(
            `[REJECT POSTING] Internship "${internship.title}" has been rejected and soft-deleted for instructor ${user.email}.`,
          );
        }
      } catch (err) {
        console.error(
          "[EMAIL ERROR] Failed to notify instructor on rejection:",
          err,
        );
      }
    })();
    ...
    return successResponse(updated, {
      message: `Internship "${internship.title}" has been rejected and removed.`,
    });
  } catch (error) {
