...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const formattedApplications = applications.map((app) => {
      ...
      if (!address) {
        address = app.internship.location || "Remote";
      }

      return {
        ...app,
        studentName: reg?.fullName || app.student?.name || "Student",
        address,
      };
    });

    const meta = buildPaginationMeta(total, page, limit);
    return successResponse(formattedApplications, { meta });
  } catch (error) {
