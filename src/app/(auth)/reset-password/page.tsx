...
import React_2, { Suspense, useState } from "react";
...
function ResetPasswordContent() {
  ...
  const { mutate: resetPassword, isPending } = AuthDataHook.useResetPassword({
    onSuccess: () => {
    ...
    <div className="bg-background text-foreground flex min-h-screen w-full flex-col font-sans md:flex-row">
        ...
        <div className="relative z-10 hidden w-full max-w-md rounded-2xl border border-emerald-500/25 bg-emerald-950/40 p-8 text-white shadow-2xl backdrop-blur-2xl transition-all hover:border-emerald-400/40 md:block">
          <div className="flex flex-col space-y-6">
            ...
            <p className="text-lg font-medium leading-relaxed tracking-wide text-white/95">
              &ldquo;Security is our top priority. Choose a strong, unique
              password to ensure your account remains protected at all
              times.&rdquo;
            </p>
      ...
      <div className="bg-background relative z-10 -mt-6 flex w-full flex-col justify-center rounded-t-[30px] px-6 pb-8 pt-5 md:mt-0 md:w-1/2 md:rounded-none md:px-12 md:py-8 lg:px-20 xl:px-24">
        <div className="mx-auto w-full max-w-[400px]">
          ...
          {!token ? (
            <div className="border-destructive/20 bg-destructive/5 text-destructive rounded-xl border p-4 text-center text-sm font-medium">
              Error: Reset token is missing or invalid. Please request a new
              password reset link.
            </div>
          ) : (
            <Form {...form}>
...
export default function ResetPasswordPage() {
  return (
    <Suspense
      fallback={
        <div className="bg-background text-muted-foreground flex min-h-screen items-center justify-center text-sm font-medium">
          Loading...
        </div>
      }
    >
