...
const ZSignUp = z
      .object({
        ...
        agreeTerms: z.boolean().refine((val) => val === true, {
          message:
            "You must agree to the Internship Policy, Terms of Service and Privacy Policy.",
        }),
      })
...
type TSignUp = z.infer<typeof ZSignUp>;

const rolesList = [
      {
        value: "STUDENT",
        label: "Student",
        desc: "Join to apply for internships",
        icon: GraduationCap_2,
      },
      {
        value: "INSTRUCTOR",
        label: "Instructor",
        desc: "Manage classes & assignments",
        icon: Presentation_2,
      },
      {
        value: "IMMERSION_USER",
        label: "Immersion User",
        desc: "Apply for Various Immersion Programs",
        icon: Briefcase_2,
      },
      {
        value: "RECRUIT_USER",
        label: "Job Applicant",
        desc: "For Appointment to Various Posts",
        icon: Building_2,
      },
    ];
...
export default function SignUpPage() {
    ...
    const RegisterForm = () => {
      ...
      const hasValidRoleParam = !!(
        roleParam &&
        ["STUDENT", "INSTRUCTOR", "IMMERSION_USER", "RECRUIT_USER"].includes(
          roleParam,
        )
      );
      ...
      useEffect(() => {
        if (
          ...
          form.setValue("role", roleParam as any);
        }
      }, [roleParam, form]);

      const { mutate: register, isPending } = AuthDataHook.useRegister({
        onSuccess: () => {
        ...
        <>
          ...
          <Form {...form}>
                      ...
                      {rolesList.map((r) => {
                        ...
                        return (
                          <button
                            key={r.value}
                            type="button"
                            disabled={hasValidRoleParam && !isSelected}
                            onClick={() => field.onChange(r.value)}
                            className={`flex flex-col items-center justify-center rounded-xl border p-2.5 text-center outline-none transition-all duration-200 ${
                              isSelected
                                ? "border-primary bg-primary/[0.03] text-primary shadow-sm"
                                : hasValidRoleParam && !isSelected
                          ...
                          >
                            <Icon
                              className={`size-4.5 mb-1 ${isSelected ? "text-primary animate-pulse" : "text-muted-foreground/80"}`}
                            />
                            <span className="text-[12px] font-bold tracking-tight">
                              {r.label}
                            </span>
                            <span className="text-muted-foreground mt-0.5 line-clamp-1 text-[8.5px] leading-none">
                              {r.desc}
                            </span>
                          </button>
                        );
            ...
            <FormField
              ...
              name="agreeTerms"
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-2 space-y-0 pt-1.5">
                  <FormControl>
                    <Checkbox
                      checked={field.value}
        ...
        <div className="relative z-10 hidden w-full max-w-md rounded-2xl border border-emerald-500/25 bg-emerald-950/40 p-8 text-white shadow-2xl backdrop-blur-2xl transition-all hover:border-emerald-400/40 md:block">
          <div className="flex flex-col space-y-6">
            ...
            <p className="text-lg font-medium leading-relaxed tracking-wide text-white/95">
              &ldquo;Join our global ecosystem connecting students, institutes,
              and mentors. We ensure verified profiles, structured internships,
